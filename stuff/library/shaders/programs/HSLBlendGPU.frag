...
void main( void )
{
	...
	vec4 s_frag = texture2D(inputImage[0], texPos);
	vec4 d_frag = texture2D(inputImage[1], texPos);
	...
	gl_FragColor.rgb = mix(d_pix, o_pix, alpha);
	gl_FragColor.a = d_frag.a;

	// Premultiplication
	gl_FragColor.rgb *= d_frag.a;
}
