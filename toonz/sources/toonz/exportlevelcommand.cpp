...
struct VectorThicknessTransformer {
		...
		: m_vi(vi), m_mutexLocker(vi->getMutex())
	{
		...
		double thickPoly[2] = {
			tcg::numeric_ops::lerp(thickTransform[0][0], thickTransform[1][0], param),
			tcg::numeric_ops::lerp(thickTransform[0][1], thickTransform[1][1], param)};

		if (thickPoly[0] == 0.0 && thickPoly[1] == 1.0)
			return;
...
TRasterImageP ImageExporter::exportedImage(TToonzImageP ti)
{
	...
	// Convert the frame's colormap to fullcolor
	{
		...
		TRop::convert(dstTile, srcTile, ti->getPalette(), // May be an in-place conversion, if
...
TImageP ImageExporter::exportedImage(const TFrameId &fid, TXshLevelType outImgType)
{
