...
// TODO: Avoid rebuilding the preview as long as parameters are untouched. Use a flag associated to
...
void PreviewToggleCommand::enable()
{
	...
	TPaletteHandle *ph = TApp::instance()->getPaletteController()->getCurrentCleanupPalette();
	ret = ret && connect(ph, SIGNAL(colorStyleChanged()), &m_timer, SLOT(start()));
...
void PreviewToggleCommand::disable()
{
	...
	// Cleanup palette changes all falls under post-processing stuff. And do not involve the model.
	TPaletteHandle *ph = TApp::instance()->getPaletteController()->getCurrentCleanupPalette();
	ret = ret && disconnect(ph, SIGNAL(colorStyleChanged()), &m_timer, SLOT(start()));
	ret = ret && disconnect(ph, SIGNAL(paletteChanged()), &m_timer, SLOT(start()));
	assert(ret);
...
void PreviewToggleCommand::onPreviewDataChanged()
{
	...
	if (sl) {
		if (!(sl->getFrameStatus(fid) & TXshSimpleLevel::CleanupPreview)) {
			...
			sl->setFrameStatus(fid, sl->getFrameStatus(fid) | TXshSimpleLevel::CleanupPreview);
		}
...
void PreviewToggleCommand::postProcess()
{
	...
	TRasterImageP preview(TCleanupper::instance()->processColors(transformed->getRaster()));
	...
	if (m_sl) {
		...
		for (i = 0; i < fidsCount; ++i) {
			...
			if (status & TXshSimpleLevel::CleanupPreview) {
				...
				// This is currently done hard here - should be skipped to m_sl, though...
...
void CameraTestToggleCommand::disable()
{
	...
	TApp::instance()->getCurrentTool()->setTool(QString::fromStdString(m_oldTool->getName()));
	m_oldTool = 0;
}
...
void CameraTestTool::drawClosestFieldCamera(double pixelSize)
{
	...
	if (areAlmostEqual(zoom, 1.0, 1e-2))
		return;
	...
	if (m_scaling != eNoScale) {
		...
		if (e.isShiftPressed()) {
			...
			if (fabs(delta.x) > fabs(delta.y)) {
				if (!cp->m_offx_lock)
					cp->m_offx += -delta.x * (2.0 / Stage::inch);
			} else {
				if (!cp->m_offy_lock)
					cp->m_offy += -delta.y * (2.0 / Stage::inch);
			}
		} else {
			if (!cp->m_offx_lock)
				cp->m_offx += -dp.x * (2.0 / Stage::inch);
			if (!cp->m_offy_lock)
				cp->m_offy += -dp.y * (2.0 / Stage::inch);
		}
	}
