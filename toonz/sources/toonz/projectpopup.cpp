...
QModelIndex ProjectDirModel::index(int row, int column, const QModelIndex &parent) const
{
	if (column != 0)
		return QModelIndex();
	...
	: Dialog(TApp::instance()->getMainWindow(), isModal, false, "Project")
{
	...
	m_topLayout->setSpacing(10);
	{
		...
		{
			upperLayout->addWidget(m_choosePrjLabel, 0, 0, Qt::AlignRight | Qt::AlignVCenter);
			upperLayout->addWidget(m_chooseProjectCombo, 0, 1);

			upperLayout->addWidget(m_prjNameLabel, 1, 0, Qt::AlignRight | Qt::AlignVCenter);
...
void ProjectPopup::updateChooseProjectCombo()
{
	...
	for (int i = 0; i < m_projectPaths.size(); i++) {
		if (TProjectManager::instance()->getCurrentProjectPath() == m_projectPaths[i]) {
...
void ProjectPopup::updateFieldsFromProject(TProject *project)
{
	...
	for (i = 0; i < m_useScenePathCbs.size(); i++) {
		...
		Qt::CheckState cbState = project->getUseScenePath(folderName) ? Qt::Checked : Qt::Unchecked;
...
/*! \class ProjectSettingsPopup
		\brief The ProjectSettingsPopup class provides a dialog to change current project settings.
	...
	: ProjectPopup(false)
{
	...
	for (i = 0; i < m_useScenePathCbs.size(); i++) {
		CheckBox *cb = m_useScenePathCbs[i].second;
		connect(cb, SIGNAL(stateChanged(int)), this, SLOT(onUseSceneChekboxChanged(int)));
	}

	connect(m_chooseProjectCombo, SIGNAL(activated(int)), this, SLOT(onChooseProjectChanged(int)));
}
...
void ProjectSettingsPopup::onChooseProjectChanged(int index)
{
	...
	TProject *projectP = TProjectManager::instance()->getCurrentProject().getPointer();
...
OpenPopupCommandHandler<ProjectSettingsPopup> openProjectSettingsPopup(MI_ProjectSettings);

//=============================================================================
/*! \class ProjectCreatePopup
		\brief The ProjectCreatePopup class provides a modal dialog to create a new project.
...
{
	if (!IoCmd::saveSceneIfNeeded(QObject::tr("Create project")))
		return;
	...
	if (projectName.isAbsolute()) {
		error(tr("Bad project name: '%1' looks like an absolute file path").arg(m_nameFld->text()));
		return;
	}
...
OpenPopupCommandHandler<ProjectCreatePopup> openProjectCreatePopup(MI_NewProject);
