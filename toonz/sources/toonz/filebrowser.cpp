...
public:
	FrameCountTask(const TFilePath &path,
...
FileBrowser::FileBrowser(QWidget *parent, Qt::WindowFlags flags, bool noContextMenu, bool multiSelectionEnabled)
#else
FileBrowser::FileBrowser(QWidget *parent, Qt::WFlags flags, bool noContextMenu, bool multiSelectionEnabled)
#endif
	: QFrame(parent), m_folderName(0), m_itemViewer(0)
{
	...
	m_itemViewer = new DvItemViewer(box, noContextMenu, multiSelectionEnabled, DvItemViewer::Browser);
	...
	bool ret = connect(m_folderTreeView, SIGNAL(currentNodeChanged()), itemViewPlayDelegate, SLOT(resetPlayWidget()));
	//if the current forder is changed in the folder tree, then update in the item view
	ret = ret && connect(m_folderTreeView, SIGNAL(currentNodeChanged()), this, SLOT(onTreeFolderChanged()));

	ret = ret && connect(m_itemViewer, SIGNAL(clickedItem(int)), this, SLOT(onClickedItem(int)));
	ret = ret && connect(m_itemViewer, SIGNAL(selectedItems(const std::set<int> &)), this, SLOT(onSelectedItems(const std::set<int> &)));
	...
	if (Preferences::instance()->isSVNEnabled())
		ret = ret && connect(VersionControl::instance(), SIGNAL(commandDone(const QStringList &)), this, SLOT(onVersionControlCommandDone(const QStringList &)));

	// if the folderName is edited, move the current folder accordingly
	ret = ret && connect(m_folderName, SIGNAL(editingFinished()), this, SLOT(onFolderEdited()));

	// folder history
	ret = ret && connect(m_folderTreeView, SIGNAL(currentNodeChanged()), this, SLOT(storeFolderHistory()));
	ret = ret && connect(buttonBar, SIGNAL(folderBack()), this, SLOT(onBackButtonPushed()));
...
void FileBrowser::storeFolderHistory()
{
	...
	if (m_indexHistoryList[m_currentPosition] == currentModelIndex)
		return;
	...
	//If the next history item is different from the current one, then replace with the new one
	else {
...
{
	struct locals {
		static inline bool itemLess(int aIdx, int bIdx, FileBrowser &fb, DataType dataType)
		{
			return (fb.compareData(dataType, aIdx, bIdx) > 0);
		}

		static inline bool indexLess(int aIdx, int bIdx, const std::vector<int> &vec)
		{
	...
	if (dataType != getCurrentOrderType()) {
		...
		std::vector<int> new2OldIdx(boost::make_counting_iterator(0),
		...
		std::vector<Item>(boost::make_permutation_iterator(m_items.begin(), new2OldIdx.begin()),
		...
		FileSelection *fs = static_cast<FileSelection *>(m_itemViewer->getPanel()->getSelection());

		if (!fs->isEmpty()) {
			std::vector<int> old2NewIdx(boost::make_counting_iterator(0),
										boost::make_counting_iterator(int(m_items.size())));
			...
			tcg::substitute(newSelectedIndices, tcg::permuted_range(
													old2NewIdx,
													fs->getSelectedIndices() | ba::filtered(
																				   boost::bind(std::less<int>(), _1, int(old2NewIdx.size())))));
	...
	if (isDiscendentOrder() != isDiscendent) {
		...
		if (!fs->isEmpty()) {
			int iCount = int(m_items.size()),
				lastIdx = iCount - 1;

			std::vector<int> newSelectedIndices;
			tcg::substitute(newSelectedIndices, fs->getSelectedIndices() | ba::filtered(boost::bind(std::less<int>(), _1, iCount)) | ba::transformed(boost::bind(locals::complement, _1, lastIdx)));
...
void FileBrowser::setFilterTypes(const QStringList &types)
{
	m_filter = types;
}
			...
			if (tmpIndex.isValid()) {
				...
				if (node) {
					...
					if (TFileStatus(childFolderPath).isLink())
						m_items.push_back(Item(childFolderPath, true, true, QString::fromStdWString(node->getName())));
					else
						m_items.push_back(Item(childFolderPath, true, false, QString::fromStdWString(node->getName())));
				}
			}
		}
	} else
	...
	if (fp != TFilePath()) {
		...
		for (it = all_files.begin(); it != all_files.end(); it++) {
			...
			if (levelName.isLevelName()) {
				...
				//TODO: とりあえず残すが、FileInfoの取得に時間がかかるようならオプション化も検討 2015/12/28 shun_iwasawa
				QFileInfo fileInfo(QString::fromStdWString(it->getWideString()));
				//Update level infos
				if (levelItem.m_creationDate.isNull() || (fileInfo.created() < levelItem.m_creationDate))
					levelItem.m_creationDate = fileInfo.created();
				if (levelItem.m_creationDate.isNull() || (fileInfo.lastModified() > levelItem.m_modifiedDate))
...
void FileBrowser::setUnregisteredFolder(const TFilePath &fp)
{
	if (fp != TFilePath()) {
		...
		// get both the folder & file list by readDirectory and readDirectory_Dir_ReadExe
		TFilePathSet folders;
		TFilePathSet files;
		//for updating m_multiFileItemMap
		TFilePathSet all_files;

		if (fpStatus.doesExist() && fpStatus.isDirectory() && fpStatus.isReadable()) {
			try {
				TSystem::readDirectory(files, all_files, fp);
	...
	// for all items in the folder, retrieve the file names(m_name) from the paths(m_path)
	refreshData();
...
int FileBrowser::getItemCount() const
{
	return m_items.size();
}
...
//!Frame count needs a special access function for viewable types - for they are
...
QVariant FileBrowser::getItemData(int index, DataType dataType, bool isSelected)
{
	...
	if (dataType == Name) {
		...
		// parent folder icons
		if (item.m_path == m_folder.getParentDir()) {
			static QPixmap folderUpPixmap(":Resources/folderup_icon.png");
			return scalePixmapKeepingAspectRatio(folderUpPixmap, iconSize, QColor(Qt::transparent));
		}
		// folder icons
		else if (item.m_isFolder) {
			if (item.m_isLink) {
				static QPixmap linkIcon(":Resources/link_icon.png");
				return scalePixmapKeepingAspectRatio(linkIcon, iconSize, QColor(Qt::transparent));
			} else {
				static QPixmap folderIcon(":Resources/folder_icon.png");
				return scalePixmapKeepingAspectRatio(folderIcon, iconSize, QColor(Qt::transparent));
			}
		}
	...
	if (dataType == CreationDate)
		return item.m_creationDate;
	if (dataType == ModifiedDate)
		return item.m_modifiedDate;
...
{
	return 0 <= index && index < (int)m_items.size() && m_items[index].m_path.getType() == "tnz";
}
...
void FileBrowser::renameItem(int index, const QString &newName)
{
	...
	if (renameFile(newFp, newName)) {
		...
		if (newFp.getType() == "tlv" || newFp.getType() == "tzp" || newFp.getType() == "tzu") {
			...
			if (paletteIndex >= 0) {
				...
				m_items[paletteIndex].m_name = QString::fromStdWString(palettePath.getLevelNameW());
		...
		if (fp.getType() == "tnz") {
			// ho cambiato il folder _files. Devo aggiornare il folder che lo contiene nel tree view
...
QMenu *FileBrowser::getContextMenu(QWidget *parent, int index)
{
	...
	if (files.size() == 1 && files[0].getType() == "tnz") {
		...
		ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(previewScreenSaver()));
		menu->addAction(action);
		action = new QAction(tr("Install Screensaver"), menu);
		ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(installScreenSaver()));
		menu->addAction(action);
	}
#endif
	...
	for (i = 0; i < (int)files.size(); i++) {
		...
		if (areResources && !TFileType::isResource(type))
			areResources = false;
		if (!areScenes && TFileType::isScene(type))
			areScenes = true;
	}
	...
	if (areResources) {
		...
		ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(loadResources()));
		menu->addAction(action);
		menu->addSeparator();
	}
	...
	if (!clickedFile.isEmpty() && (clickedFile.getType() == "tnz" || clickedFile.getType() == "tab")) {
		menu->addSeparator();
	...
	if (i == files.size()) {
		...
		for (j = 0; j < files.size(); j++)
			if (files[j].getType() == "tzp" || files[j].getType() == "tzu")
				break;
		if (j == files.size())
	...
	if (files.size() == 1 && files[0].getType() != "tnz") {
		...
		ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(renameAsToonzLevel()));
		menu->addAction(action);
	}
#ifdef LEVO
	...
	if (files.size() == 2 && (files[0].getType() == "tif" || files[0].getType() == "tiff" || files[0].getType() == "png" ||
		...
		ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(convertToPaintedTlv()));
		menu->addAction(action);
	}
	if (areFullcolor) {
		QAction *action = new QAction(tr("Convert to Unpainted TLV"), menu);
		ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(convertToUnpaintedTlv()));
		menu->addAction(action);
		menu->addSeparator();
	}
#endif
	...
	if (node) {
		...
		DvItemListModel::Status status = (DvItemListModel::Status)m_itemViewer->getModel()->getItemData(index, DvItemListModel::VersionControlStatus).toInt();
		...
		if (status == DvItemListModel::VC_ReadOnly ||
			...
			if (status == DvItemListModel::VC_ReadOnly) {
				...
				ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(editVersionControl()));
				...
				if (fileType == "tlv" || fileType == "pli" || path.getDots() == "..") {
					...
					ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(editFrameRangeVersionControl()));
				}
			} else {
				action = vcMenu->addAction(tr("Edit"));
				ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(updateAndEditVersionControl()));
			}
		}

		if (status == DvItemListModel::VC_Modified) {
			action = vcMenu->addAction(tr("Put..."));
			ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(putVersionControl()));

			action = vcMenu->addAction(tr("Revert"));
			ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(revertVersionControl()));
		}

		if (status == DvItemListModel::VC_ReadOnly ||
			status == DvItemListModel::VC_ToUpdate) {
			action = vcMenu->addAction(tr("Get"));
			ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(getVersionControl()));

			if (status == DvItemListModel::VC_ReadOnly) {
				action = vcMenu->addAction(tr("Delete"));
				ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(deleteVersionControl()));
			}
			...
			if (files.size() == 1) {
				...
				TFilePath path = files.at(0);
				if (path.getDots() == "..")
					ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(getRevisionVersionControl()));
				else
					ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(getRevisionHistory()));
		...
		if (status == DvItemListModel::VC_Edited) {
			...
			ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(unlockVersionControl()));
		}
		...
		if (status == DvItemListModel::VC_Locked && files.size() == 1) {
			...
			ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(showLockInformation()));
		}
		...
		if (status == DvItemListModel::VC_PartialLocked) {
			...
			if (files.size() == 1) {
				...
				action = vcMenu->addAction(tr("Edit Info"));
				ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(showFrameRangeLockInfo()));
			}
			...
			if (files.size() == 1) {
				...
				ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(unlockFrameRangeVersionControl()));
			...
			if (files.size() == 1) {
				action = vcMenu->addAction(tr("Put..."));
				ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(putFrameRangeVersionControl()));

				action = vcMenu->addAction(tr("Revert"));
				ret = ret && connect(action, SIGNAL(triggered()), this, SLOT(revertFrameRangeVersionControl()));
			}
		}
...
void FileBrowser::startDragDrop()
{
	...
	for (int i = 0; i < (int)files.size(); i++) {
		if (TSystem::doesExistFileOrLevel(files[i]))
			urls.append(QUrl::fromLocalFile(QString::fromStdWString(files[i].getWideString())));
	}
	if (urls.isEmpty())
		return;
	...
	QPixmap dropThumbnail = scalePixmapKeepingAspectRatio(icon, iconSize, Qt::transparent);
...
bool FileBrowser::dropMimeData(QTreeWidgetItem *parent, int index, const QMimeData *data, Qt::DropAction action)
{
	return false;
}
...
void FileBrowser::onTreeFolderChanged()
{
	...
	if (fileFolderNode)
		emit treeFolderChanged(fileFolderNode->getPath());
}
...
void FileBrowser::onDataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight)
{
	onTreeFolderChanged();
}
...
bool FileBrowser::drop(const QMimeData *mimeData)
{
	...
	if (mimeData->hasFormat(CastItems::getMimeFormat())) {
		...
		if (scene->isUntitled()) {
			bool ok;
			QString sceneName = QInputDialog::getText(this, tr("Save Scene"),
	...
	: Dialog(TApp::instance()->getMainWindow(), true, true, "RenameAsToonz")
{
	...
	if (frames == -1)
		...
		lbl = new QLabel(QString(tr("Creating an animation level of %1 frames").arg(frames)));
...
bool parsePathName(const QString &fullpath, QString &parentPath, QString &name, QString &format)
{
	...
	index--;
	if (!filename.at(index).isDigit())
		return false;

	while (index >= 0 && filename.at(index).isDigit())
		index--;
...
void getLevelFiles(const QString &parentPath, const QString &name, const QString &format, QStringList &pathIn)
{
...
QString getFrame(const QString &filename)
{
	...
	int to, from;
	to = from = index;
	while (from >= 0 && filename.at(from).isDigit())
		from--;
...
void renameSingleFileOrToonzLevel(const QString &fullpath)
{
	...
	RenameAsToonzPopup popup(QString::fromStdWString(fpin.withoutParentDir().getWideString()));
...
void FileBrowser::convertToUnpaintedTlv()
{
	...
	bool ok;
	QString autoclose = QInputDialog::getItem(this, tr("Convert To Unpainted Tlv"), "", sl, 0, false, &ok);
	if (!ok)
		return;
...
void FileBrowser::convertToPaintedTlv()
{
	...
	sl << "Apply Autoclose                      "
	   << "Don't Apply Autoclose                        ";
	bool ok;
	QString autoclose = QInputDialog::getItem(this, tr("Convert To Painted Tlv"), "", sl, 0, false, &ok);
	if (!ok)
		return;

	QApplication::setOverrideCursor(Qt::WaitCursor);

	Convert2Tlv *converter = new Convert2Tlv(filePaths[0], filePaths[1], TFilePath(), -1, -1, autoclose == sl.at(0), TFilePath(), 0, 0, 0);
	...
	ProgressDialog pb("Generating level " + toQString(converter->m_levelOut), "Cancel", 0, count);
	pb.show();
...
void FileBrowser::getExpandedFolders(DvDirModelNode *node, QList<DvDirModelNode *> &expandedNodes)
{
	if (!node)
		return;
	QModelIndex newIndex = DvDirModel::instance()->getIndexByNode(node);
	if (!m_folderTreeView->isExpanded(newIndex))
		return;
...
{
	TFilePath originalFolder(m_folder); //setFolder is invoked by Qt throughout the following...
...
void FileBrowser::previewScreenSaver()
{
	...
	if (!fs)
		return;
	fs->getSelectedFiles(files);
	if (files.size() != 1 || files[0].getType() != "scr")
		return;
...
void FileBrowser::showEvent(QShowEvent *)
{
	...
	// Refresh SVN
	DvDirVersionControlNode *vcNode = dynamic_cast<DvDirVersionControlNode *>(m_folderTreeView->getCurrentNode());
	if (vcNode)
		m_folderTreeView->refreshVersionControl(vcNode);
}
...
void FileBrowser::selectNone()
{
	m_itemViewer->selectNone();
}
...
{
	QDateTime modifiedDate = QFileInfo(QString::fromStdWString(fp.getWideString())).lastModified();
...
calculateTask:
	...
	connect(task, SIGNAL(finished(TThread::RunnableP)), this, SIGNAL(calculatedFrameCount()));
	connect(task, SIGNAL(exception(TThread::RunnableP)), this, SIGNAL(calculatedFrameCount()));
...
inline void FrameCountReader::stopReading()
{
...
void FrameCountTask::run()
{
	...
	if (frameCount > 0) {
		...
		//Seems that tlv reads sometimes may fail, returning invalid frame counts (typically 0).
...
OpenFloatingPanel openBrowserPane(MI_OpenFileBrowser, "Browser", QObject::tr("File Browser"));
