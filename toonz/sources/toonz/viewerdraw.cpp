...
inline void glVertex(const T3DPointD &p)
{
...
void ViewerDraw::drawCameraMask(SceneViewer *viewer)
{
	...
	if (cameraRect.overlaps(bounds)) {
		...
		if (bounds.x0 <= x0)
			tglFillRect(bounds.x0, bounds.y0, x0, bounds.y1);
		if (x1 <= bounds.x1)
		...
		if (x0 < bounds.x1 && x1 > bounds.x0) {
			...
			if (y1 < bounds.y1)
				tglFillRect(xa, y1, xb, bounds.y1);
		}
	} else {
		tglFillRect(bounds);
	}
	...
	bool gridEnabled)
{
	...
	int hGuideCount = 0;
	if (vr)
		vGuideCount = vr->getGuideCount();
	if (hr)
		hGuideCount = hr->getGuideCount();
	...
	clipRect -= TPoint((clipRect.x0 + clipRect.x1) / 2, (clipRect.y0 + clipRect.y1) / 2);
...
void ViewerDraw::draw3DCamera(unsigned long flags, double zmin, double phi)
{
	...
	double ztable = 0;
	if (ztable < znear)
		cageZ.push_back(ztable);
	if (zmin < znear)
		cageZ.push_back(zmin);
	...
	for (int i = 0; i < m; i++) {
		...
		for (int j = 0; j < 4; j++)
			cage[i][j] = make3dPoint(cameraCenter + sc * (cameraCorners[j] - cameraCenter), z);
	}
...
void ViewerDraw::drawCamera(unsigned long flags, double pixelSize)
{
	...
	if (!CleanupPreviewCheck::instance()->isEnabled() && subcamera) {
		...
		if (previewSubRect.getLx() > 0 && previewSubRect.getLy() > 0) {
			...
			glVertex2d(stagePreviewSubRect.x1 - pixelSize, stagePreviewSubRect.y1 - pixelSize);
