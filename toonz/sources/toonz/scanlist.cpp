...
TFrameId ScanListFrame::getFrameId() const
{
	return m_fid;
}
...
void ScanListFrame::setRasterImage(const TRasterImageP &ras, bool isBW) const
{
	...
	if (!m_xl->getPalette()) {
		...
		m_xl->setPalette(p);
		TApp::instance()->getPaletteController()->getCurrentLevelPalette()->setPalette(p);
	}
...
int ScanList::getFrameCount() const
{
...
bool ScanList::areScannedFramesSelected()
{
	...
	for (it = cells.begin(); it != cells.end(); ++it) {
		assert(it->first);
		if (it->first->getFrame(it->second, false))
			return true;
	}
	return false;
}
