...
void TasksViewer::showEvent(QShowEvent *)
{
	startTimer();
}
...
void TasksViewer::hideEvent(QHideEvent *)
{
	stopTimer();
}
...
QWidget *TasksViewer::createToolBar()
{
	...
	add("saveas", tr("&Save Task List As"), saveToolbar, SLOT(saveas(bool)), tr("Save As"));
	...
	: QSplitter(parent)
{
	...
	BatchesController::instance()->setTasksTree((TaskTreeModel *)m_treeView->model());
...
{
	if (BatchesController::instance()->getController()) // si sta utilizzando la farm
...
bool containsTask(const TFarmTask::Dependencies &dependencies, TFarmTask::Id id)
{
	int i;
	for (i = 0; i < dependencies.getTaskCount(); i++)
		if (dependencies.getTaskId(i) == id)
			return true;
	return false;
}
...
void TaskSheet::update(TFarmTask *task)
{
	...
	while (m_addedBox->count())
		delete m_addedBox->takeItem(0);
	while (m_notAddedBox->count())
		delete m_notAddedBox->takeItem(0);
	...
	if (task->m_completionDate > task->m_startDate)
		m_duration->setText(QString::number(task->m_startDate.secsTo(task->m_completionDate)));
	else
		m_duration->clear();
	...
	if (task->m_isComposerTask) {
		...
		if (dynamic_cast<TFarmTaskGroup *>(task)) {
			...
			m_outputPath->setText(QString::fromStdWString(task->m_outputPath.getWideString()));
	...
	for (i = 0; i < task->m_dependencies->getTaskCount(); i++)
		new DependencyItem(bc->getTask(task->m_dependencies->getTaskId(i)), m_addedBox);

	for (i = 0; i < bc->getTaskCount(); i++) {
		TFarmTaskGroup *tg;
		TFarmTask *t = bc->getTask(i);
		if (!(tg = dynamic_cast<TFarmTaskGroup *>(t)))
			continue;
		if (tg->m_id == task->m_id)
			continue;
		if (containsTask(*task->m_dependencies, tg->m_id))
			continue;
...
void TaskSheet::updateChunks(TFarmTask *task)
{
	...
	if (t) {
		...
		for (i = 0; i < t->getTaskCount(); i++)
			BatchesController::instance()->addTask(t->getTask(i)->m_id, t->getTask(i));
...
void inline create(QTextEdit *&ret, QGridLayout *layout, QString name, int row, int span = 5)
{
	ret = new QTextEdit();
...
void inline create(LineEdit *&ret, QGridLayout *layout, QString name, int row, int span = 3)
{
	ret = new LineEdit();
	QLabel *label = new QLabel(name);
	label->setObjectName("TaskSheetItemLabel");
...
void inline create(QComboBox *&ret, QGridLayout *layout, QString name, int row, int span = 3)
{
	ret = new QComboBox();
	QLabel *label = new QLabel(name);
	label->setObjectName("TaskSheetItemLabel");
...
void inline create(QLabel *&ret, QGridLayout *layout, QString name, int row, int span = 3)
{
	ret = new QLabel();
	ret->setObjectName("TaskSheetItem");
	QLabel *label = new QLabel(name);
...
void TaskSheet::setVisible(int)
{
	if (!m_task)
		return;
	if (m_task->m_onlyVisible == (m_visible->checkState() == Qt::Checked))
		return;
...
void TaskSheet::setOverwrite(int index)
{
	if (!m_task)
		return;
	if (m_task->m_overwrite == (OverwriteBehavior)index)
		return;
...
void TaskSheet::setMultimedia(int)
{
	if (!m_task)
		return;
	if (m_task->m_multimedia == m_multimedia->currentIndex())
		return;
	if (m_task->m_completionDate.isNull() && !m_task->m_startDate.isNull()) {
		update(m_task);
		return;
...
void TaskSheet::setThreadsCombo(int)
{
	if (!m_task)
		return;
	if (m_task->m_threadsIndex == m_threadsCombo->currentIndex())
		return;
	if (m_task->m_completionDate.isNull() && !m_task->m_startDate.isNull()) {
		update(m_task);
		return;
	...
	: QScrollArea(owner)
{
	...
	layout->addWidget(m_notAddedBox, row++, 4, 1, 2, Qt::AlignLeft | Qt::AlignTop);
	...
	ret = ret && connect(m_name, SIGNAL(editingFinished()), this, SLOT(setName()));
	ret = ret && connect(m_from, SIGNAL(editingFinished()), this, SLOT(setFrom()));
	ret = ret && connect(m_to, SIGNAL(editingFinished()), this, SLOT(setTo()));
	ret = ret && connect(m_step, SIGNAL(editingFinished()), this, SLOT(setStep()));
	ret = ret && connect(m_shrink, SIGNAL(editingFinished()), this, SLOT(setShrink()));
	ret = ret && connect(m_outputPath, SIGNAL(editingFinished()), this, SLOT(setOutput()));
	ret = ret && connect(m_chunkSize, SIGNAL(editingFinished()), this, SLOT(setChunkSize()));
	ret = ret && connect(m_priority, SIGNAL(editingFinished()), this, SLOT(setPriority()));
	...
	ret = ret && connect(m_outputPath, SIGNAL(focusIn()), this, SLOT(onFocusIn()));
...
void TasksViewer::setSelected(TFarmTask *task)
{
	m_taskSheet->update(task);
}
...
{
	if (!treeModel)
		treeModel = new TaskTreeModel(this);
	setModel(treeModel);
...
void TaskTreeView::onClick(TreeModel::Item *gItem, const QPoint &pos, QMouseEvent *e)
{
...
void TaskTreeView::openContextMenu(TreeModel::Item *gItem, const QPoint &globalPos)
{
	TaskTreeModel::Item *item = static_cast<TaskTreeModel::Item *>(gItem);
	...
	if (item->getDepth() == 1) {
		...
		if (menu.isEmpty()) {
			...
			ret = ret && connect(action, SIGNAL(triggered(bool)), m, SLOT(start(bool)));
			menu.addAction(action);
			action = new QAction(tr("Stop"), this);
			ret = ret && connect(action, SIGNAL(triggered(bool)), m, SLOT(stop(bool)));
			menu.addAction(action);
			action = new QAction(tr("Remove"), this);
			menu.addAction(action);
			ret = ret && connect(action, SIGNAL(triggered(bool)), m, SLOT(remove(bool)));
			assert(ret);
		}
		menu.exec(globalPos);
	}
...
void TaskTreeModel::saveas(bool)
{
	BatchesController::instance()->saveas();
}

//------------------------------------------------------------------------------------------------------------------

void TaskTreeModel::load(bool)
{
	BatchesController::instance()->load();
}
