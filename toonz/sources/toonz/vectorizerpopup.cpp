					 ...
					 int r0, int c0, int r1, int c1)
{
	...
	for (fst = fidsSet.begin(); fst != fsEnd; ++fst)
		fids.push_back(*fst);
}
	...
	TImageP img, TPalette *palette, const VectorizerConfiguration &conf)
{
	...
	connect(&vCore, SIGNAL(partialDone(int, int)), this, SIGNAL(partialDone(int, int)));
...
void VectorizerPopup::onFinished()
{
	l_quitLoop = true;
}
...
void VectorizerPopup::updateSceneSettings()
{
	...
	if (outline) {
		...
		vParams->m_cThicknessRatioFirst = m_cThicknessRatioFirst->getValue() * 100.0;
...
void VectorizerPopup::refreshPopup()
{
	setType(getParameters()->m_isOutline);
}
...
void VectorizerPopup::updateVisibility()
{
	...
	locals.setVisible(vParams->m_isOutline ? l_outlineParamGroups : l_centerlineParamGroups,
...
{
	refreshPopup();
	disconnect(m_sceneHandle, SIGNAL(sceneSwitched()), this, SLOT(refreshPopup()));
	Dialog::hideEvent(he);
}
	...
	struct {
		...
		void saveParams(const TFilePath &fp) // May throw due to I/O failure
		{
			...
			if (TSystem::doesExistFileOrLevel(fp)) {
				...
				if (!is)
					throw TException(tr("File could not be opened for read").toStdWString());
			...
			if (!os)
				throw TException(tr("File could not be opened for write").toStdWString());
...
OpenPopupCommandHandler<VectorizerPopup> openVectorizerPopup(MI_ConvertToVectors);
