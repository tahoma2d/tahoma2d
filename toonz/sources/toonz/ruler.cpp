...
Ruler::Ruler(QWidget *parent, SceneViewer *viewer, bool vertical)
	: QWidget(parent), m_viewer(viewer), m_vertical(vertical), m_moving(false), m_hiding(false)
{
	if (vertical) {
...
int Ruler::getGuideCount() const
{
	return getGuides().size();
}
...
void Ruler::drawVertical(QPainter &p)
{
	...
	for (i = 0; i < count; i++) {
		QColor color = (m_moving && count - 1 == i ? QColor(0, 255, 255) : QColor(0, 0, 255));
		double v = guides[i];
...
void Ruler::mousePressEvent(QMouseEvent *e)
{
	if (e->button() == Qt::LeftButton) {
		...
		if (selected < 0 || minDist2 > 25) {
			...
			if (selected < count - 1)
				tswap(guides[selected], guides.back());
			// aggiorna sprop!!!!
		}
		m_moving = true;
		update();
...
void Ruler::mouseMoveEvent(QMouseEvent *e)
{
	if (m_moving) {
		...
		getGuides().back() = posToValue(e->pos());
		// aggiorna sprop!!!!
		update();
		m_viewer->update();
		return;
