	...
	: QWidget(parent), m_ph(TApp::instance()->getPaletteController()->getCurrentCleanupPalette()), m_scrollWidget(0), m_greyMode(true), m_contrastEnabled(true)
{
	...
	ret = ret && connect(m_ph, SIGNAL(colorStyleChanged()), SLOT(onColorStyleChanged()));

	ret = ret && connect(m_add, SIGNAL(clicked(bool)), SLOT(onAddClicked(bool)));
	ret = ret && connect(m_remove, SIGNAL(clicked(bool)), SLOT(onRemoveClicked(bool)));
	assert(ret);
}
...
void CleanupPaletteViewer::buildGUI()
{
	...
	for (i = 0; i < stylesCount; ++i) {
		TCleanupStyle *cs = dynamic_cast<TCleanupStyle *>(palette->getPage(0)->getStyle(i));
		if (!cs)
			continue;

		cs->enableContrast(true);
		CleanupColorField *cf = new CleanupColorField(m_scrollWidget, cs, m_ph, m_greyMode);
	...
	if (m_greyMode) {
		...
		m_scrollArea->setStyleSheet("QScrollArea { border: 1px solid rgb(190, 190, 190); }");
	}
...
void CleanupPaletteViewer::onColorStyleChanged()
{
	...
	for (i = 0; i < colorFieldsCount; ++i)
		m_colorFields[i]->updateColor();
}
...
void CleanupPaletteViewer::setContrastEnabled(bool enable)
{
	...
	for (i = 0; i < size; ++i)
		m_colorFields[i]->setContrastEnabled(enable);
}
