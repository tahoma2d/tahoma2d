...
//=============================================================================
/*! \class InsertFxPopup
		\brief The InsertFxPopup class provides a dialog to browse fx and add it to
		current scene.
	...
	: Dialog(TApp::instance()->getMainWindow(), true, false, "InsertFx"), m_folderIcon(QIcon()), m_presetIcon(QIcon()), m_fxIcon(QIcon())
{
	...
	TFilePath path = TFilePath(ToonzFolder::getProfileFolder() + "layouts" + "fxs" + "fxs.lst");
	m_presetFolder = TFilePath(ToonzFolder::getFxPresetFolder() + "presets");
	loadFx(path);
	loadMacro();

	// add 'Plugins' directory
	auto plugins = new QTreeWidgetItem((QTreeWidget *)NULL, QStringList("Plugins"));
	...
	std::map<std::string, QTreeWidgetItem *> vendors = PluginLoader::create_menu_items(
...
void InsertFxPopup::makeItem(QTreeWidgetItem *parent, std::string fxId)
{
	QTreeWidgetItem *fxItem = new QTreeWidgetItem(
		(QTreeWidget *)0, QStringList(QString::fromStdWString(TStringTable::translate(fxId))));
...
{
	updatePresets();
	connect(TApp::instance()->getCurrentFx(), SIGNAL(fxPresetSaved()), SLOT(updatePresets()));
}
