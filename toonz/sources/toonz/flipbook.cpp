...
{
	return QString::fromStdString(CommandManager::instance()->getShortcutFromId(id));
}
} // namespace
...
/*! \class FlipBook
		...
		The object is composed of grid layout \b QGridLayout which contains an image
		viewer \b ImageViewer and a double button bar. It is possible decide widget
		title and which button bar show by setting QString and bool in constructor.

		You can set level to show in FlipBook using \b setLevel(); and call 
		onDrawFrame() to show FlipBook current frame. The current frame can be
		set directly using setCurrentFrame(int index) or using slot methods connected
		to button bar.

    \sa FlipBookPool class.
*/
FlipBook::FlipBook(QWidget *parent, QString viewerTitle, UINT flipConsoleButtonMask, UCHAR flags, bool isColorModel) //, bool showOnlyPlayBackgroundButton)
	: QWidget(parent),
	  ...
	  m_flags(flags)
{
	...
	m_imageViewer = new ImageViewer(fsWidget, this, flipConsoleButtonMask == FlipConsole::cFullConsole);
	...
	mainLayout->setSpacing(0);
	{
		...
		m_flipConsole = new FlipConsole(mainLayout, flipConsoleButtonMask, true, 0, (viewerTitle == "") ? "FlipConsole" : viewerTitle, this, !isColorModel);
	...
	ret = ret && connect(parentWidget(), SIGNAL(closeButtonPressed()), this, SLOT(onCloseButtonPressed()));
	ret = ret && connect(parentWidget(), SIGNAL(doubleClick(QMouseEvent *)), this, SLOT(onDoubleClick(QMouseEvent *)));
	ret = ret && connect(&m_previewUpdateTimer, SIGNAL(timeout()), this, SLOT(performFxUpdate()));
...
//-----------------------------------------------------------------------------
/*! add freeze button to the flipbook. called from the function PreviewFxManager::openFlipBook.
...
LoadImagesPopup::LoadImagesPopup(FlipBook *flip)
	: FileBrowserPopup(tr("Load Images"), Options(WITH_APPLY_BUTTON), tr("Append"), new QFrame(0)), m_flip(flip), m_minFrame(0), m_maxFrame(1000000), m_step(1), m_shrink(1)
{
	...
	ret = ret && connect(m_fromField, SIGNAL(editingFinished()), this, SLOT(onEditingFinished()));
	ret = ret && connect(m_toField, SIGNAL(editingFinished()), this, SLOT(onEditingFinished()));
	ret = ret && connect(m_stepField, SIGNAL(editingFinished()), this, SLOT(onEditingFinished()));
	ret = ret && connect(m_shrinkField, SIGNAL(editingFinished()), this, SLOT(onEditingFinished()));

	ret = ret && connect(this, SIGNAL(filePathClicked(const TFilePath &)), SLOT(onFilePathClicked(const TFilePath &)));
...
*/
bool LoadImagesPopup::executeApply()
{
	return doLoad(true);
}
...
bool LoadImagesPopup::doLoad(bool append)
{
	...
	::viewFile(*m_selectedPaths.begin(),
			   m_from, m_to, m_step, m_shrink, 0, m_flip, append);
	...
	for (pt = m_selectedPaths.begin(); pt != m_selectedPaths.end(); ++pt) {
		RecentFiles::instance()->addFilePath(
			toQString(TApp::instance()->getCurrentScene()->getScene()->decodeFilePath((*pt))),
			RecentFiles::Flip);
	}
	return true;
}
...
void FlipBook::loadImages()
{
	...
	if (!m_loadPopup) {
		...
		m_loadPopup->setFolder(TProjectManager::instance()->getCurrentProjectPath().getParentDir());
	}
...
bool FlipBook::canAppend()
{
	...
	//  a) There is a name (in particular, an extension) representing currently held ones.
	//  b) This flipbook is not holding a preview (inappropriate and problematic).
	//  c) The level has no palette. Otherwise, appended images may have a different palette.
...
{
	if (!m_savePopup)
		m_savePopup = new SaveImagesPopup(this);

	//initialize the default path every time
	TOutputProperties *op = TApp::instance()->getCurrentScene()->getScene()->getProperties()->getOutputProperties();
...
bool FlipBook::doSaveImages(TFilePath fp)
{
	...
	if (TFileType::getInfo(fp) == TFileType::RASTER_IMAGE || ext == "pct" || ext == "pic" || ext == "pict") //pct e' un formato"livello" (ha i settings di quicktime) ma fatto di diversi frames
...
void FlipBook::saveImage()
{
	...
	for (; m_currentFrameToSave <= m_framesCount; m_currentFrameToSave++) {
		...
		savedFrames++;
		//		if (!m_pb->changeFraction(m_currentFrameToSave, TApp::instance()->getCurrentXsheet()->getXsheet()->getFrameCount()))
		//			break;
...
/*! \class FlipBookPool
		\brief The FlipBookPool class is used to store used flipbook viewers.
    ...
    This class implements the basical features to \b pop a flipbook from the pool
...
FlipBook *FlipBookPool::pop()
{
	...
	if (m_pool.empty()) {
		...
		if (m_geometryPool.empty()) {
			...
			if ((geometry & QApplication::desktop()->availableGeometry(panel)).isEmpty())
...
{
	return m_incrementalIndexing ? (m_level->getFrameCount() - 1) / m_step + 1 : (m_toIndex - m_fromIndex) / m_step + 1;
}
...
bool FlipBook::isSavable() const
{
	...
	for (int i = 0; i < m_levels.size(); i++)
		if (m_levels[i].m_fp != TFilePath() && (m_levels[i].m_fp.getType() == "tlv" || m_levels[i].m_fp.getType() == "pli"))
			return false;
...
/*! Set the level contained in \b fp to FlipBook; if level exist show in image
		...
		It's possible to change level palette, in fact if \b palette is different
		from 0 set level palette to \b palette.
*/
void FlipBook::setLevel(const TFilePath &fp, TPalette *palette, int from, int to, int step, int shrink,
						TSoundTrack *snd, bool append, bool isToonzOutput)
{
	try {
		if (!append) {
			clearCache();
		...
		if (TSystem::doesExistFileOrLevel(fp)) //is a  viewfile
		{
			...
			//m_flipConsole->enableButton(FlipConsole::eCheckBg, true);//fp.getType()!="pli");

			m_lr = TLevelReaderP(fp);

			bool supportsRandomAccess = doesSupportRandomAccess(fp, isToonzOutput);
			if (supportsRandomAccess)
				m_lr->enableRandomAccessRead(isToonzOutput);
			...
			if (!level || level->getFrameCount() == 0) {
				if (m_flags & eDontKeepFilesOpened)
					m_lr = TLevelReaderP();
				return;
			}

			// For the color model, get the reference fids from palette and delete unneeded from the table
			if (m_imageViewer->isColorModel() && palette) {
				std::vector<TFrameId> fids = palette->getRefLevelFids();

				// when loading a single-frame, standard raster image into the ColorModel, skip here.
				// If the fid == NO_FRAME(=-2), fids stores 0.
				if (!fids.empty() &&
					!(fids.size() == 1 && fids[0].getNumber() == 0)) {
					...
					for (it = level->begin(); it != level->end(); it++) {
						...
						for (i = 0; i < (int)fids.size(); i++) {
							if (fids[i].getNumber() == it->first.getNumber())
								break;
						}
						if (i == fids.size()) {
			...
			// in order to avoid that the current frame unexpectedly moves to 1 on the Color Model once editing the style
			int current = -1;
			...
			if (from == -1 && to == -1) {
				...
				//  If the level read did not support random access, *AND* the level to show was just rendered,
				//  we have to assume that no level update happened, and the from-to-step infos are lost.
			...
			if (ii)
				m_dim = TDimension(ii->m_lx / m_shrink, ii->m_ly / m_shrink);
			...
			if (ii && !m_imageViewer->isColorModel())
				m_title = m_title + "  ::  " + QString::number(ii->m_lx) + "x" + QString::number(ii->m_ly) + " Pixels";
			...
			//An old archived bug says that simulatenous open for read of the same tlv are not allowed...
		...
		} else //is a render
		{
			...
			m_title = tr("Rendered Frames  ::  From %1 To %2  ::  Step %3").arg(QString::number(from)).arg(QString::number(to)).arg(QString::number(step));
			if (shrink > 1)
...
void FlipBook::setLevel(TXshSimpleLevel *xl)
{
	try {
		...
		if (p)
			m_title = m_title + "  ::  " + QString::number(p->getImageRes().lx) + "x" + QString::number(p->getImageRes().ly) + " Pixels";
...
void FlipBook::setLevel(TFx *previewedFx, TXsheet *xsh, TLevel *level, TPalette *palette, int from, int to, int step, int currentFrame, TSoundTrack *snd)
{
	m_xl = 0;
...
TRectD FlipBook::getPreviewedImageGeometry() const
{
	...
	TRectD viewerGeomD(viewerGeom.left(), viewerGeom.top(), viewerGeom.right() + 1, viewerGeom.bottom() + 1);
	TPointD viewerCenter((viewerGeomD.x0 + viewerGeomD.x1) * 0.5, (viewerGeomD.y0 + viewerGeomD.y1) * 0.5);

	//NOTE: The above adjust() is imposed to counter the geometry removal specified in function
...
{
	if (m_previewedFx)
		m_previewUpdateTimer.start(1000); //The effective fx update will happen in 1 msec.
}
...
void FlipBook::clonePreview()
{
	...
	FlipBook *newFlip = PreviewFxManager::instance()->showNewPreview(m_previewedFx, true);
...
void FlipBook::freezePreview()
{
	...
	if (m_freezeButton)
		m_freezeButton->setPressed(true);
}
...
void FlipBook::playAudioFrame(int frame)
{
	...
	if (m_player) {
		...
		int fps = TApp::instance()->getCurrentScene()->getScene()->getProperties()->getOutputProperties()->getFrameRate();
...
TImageP FlipBook::getCurrentImage(int frame)
{
	...
	} else if (!m_levels.empty()) //is a viewfile or a previewFx
	{
		...
		int i = 0;
		//Search all subsequent levels on the flipbook and retrieve the one containing the required frame
		for (i = 0; i < m_levels.size(); i++) {
			int frameIndexesCount = m_levels[i].getIndexesCount();
			if (frameIndex > 0 && frameIndex <= frameIndexesCount)
				break;
			frameIndex -= frameIndexesCount;
		}

		if (i == m_levels.size() || frame < 0)
			return 0;
	...
	if (TImageCache::instance()->isCached(id)) {
		...
		//TImageCache::instance()->add(toString(m_poolIndex) + "lastFlipFrame", img);
	...
	if (fp != TFilePath() && !m_isPreviewFx) {
		...
		if (!m_lr)
			return 0;
		// try to get image info only when loading tlv or pli as it is quite time consuming
		if (fp.getType() == "tlv" || fp.getType() == "pli") {
			if (m_lr->getImageInfo())
				lx = oriLx = m_lr->getImageInfo()->m_lx;
		}
		...
		if (img) {
			...
			if (ri && showSub && m_loadbox != TRect() && ri->getRaster()->getLx() == oriLx) //questo serve perche' per avi e mov la setRegion e' completamente ignorata...
			...
			if ((fp.getType() == "tlv" || fp.getType() == "pli") &&
				m_shrink > 1 &&
				(lx == 0 || (ri && ri->getRaster()->getLx() == lx) || (ti && ti->getRaster()->getLx() == lx))) {
				if (ri)
...
void FlipBook::onDrawFrame(int frame, const ImagePainter::VisualSettings &vs)
{
	...
	if (m_playSound && !vs.m_drawBlankFrame)
		playAudioFrame(frame);
}

//-----------------------------------------------------------------------------

void FlipBook::swapBuffers()
{
	m_imageViewer->doSwapBuffers();
}
...
{
	m_loadbox = (m_dim.lx > 0) ? box * TRect(0, 0, m_dim.lx - 1, m_dim.ly - 1) : box;
}
...
void FlipBook::onCloseButtonPressed()
{
	...
	if (m_poolIndex >= 0)
		FlipBookPool::instance()->push(this);
}
...
void FlipBook::dragEnterEvent(QDragEnterEvent *e)
{
	...
	if (!acceptResourceDrop(mimeData->urls()) && !mimeData->hasFormat("application/vnd.toonz.drawings") &&
	...
	if (mimeData->hasFormat(CastItems::getMimeFormat())) {
		const CastItems *items =
			dynamic_cast<const CastItems *>(mimeData);
		if (!items)
			return;
...
void FlipBook::dropEvent(QDropEvent *e)
{
	...
	if (mimeData->hasUrls()) {
		foreach (QUrl url, mimeData->urls()) {
			TFilePath fp(url.toLocalFile().toStdWString());
			if (TFileType::getInfo(fp) != TFileType::UNKNOW_FILE)
				setLevel(fp);
			e->acceptProposedAction();
			return;
		}
	} else if (mimeData->hasFormat("application/vnd.toonz.drawings")) //drag-drop from film strip
	{
		TFilmstripSelection *s = dynamic_cast<TFilmstripSelection *>(TSelection::getCurrent());
		...
		setLevel(newSl);
	} else if (mimeData->hasFormat(CastItems::getMimeFormat())) //Drag-Drop from castviewer
	{
		const CastItems *items =
			dynamic_cast<const CastItems *>(mimeData);
		if (!items)
...
void FlipBook::reset()
{
	...
	if (m_playSound)
		m_flipConsole->pressButton(FlipConsole::eSound);
	if (m_player)
		m_player->stop();
...
void FlipBook::resizeEvent(QResizeEvent *e)
{
	schedulePreviewedFxUpdate();
}

//-----------------------------------------------------------------------------

void FlipBook::adaptGeometry(const TRect &interestingImgRect, const TRect &imgRect)
{
	TRectD imgRectD(imgRect.x0, imgRect.y0, imgRect.x1 + 1, imgRect.y1 + 1);
	TRectD interestingImgRectD(
		interestingImgRect.x0, interestingImgRect.y0,
		interestingImgRect.x1 + 1, interestingImgRect.y1 + 1);
	...
	adaptWidGeometry(
		TRect(tceil(interestGeomD.x0), tceil(interestGeomD.y0), tfloor(interestGeomD.x1) - 1, tfloor(interestGeomD.y1) - 1),
		TRect(tceil(imageGeomD.x0), tceil(imageGeomD.y0), tfloor(imageGeomD.x1) - 1, tfloor(imageGeomD.y1) - 1),
		true);
}
...
void FlipBook::adaptGeometryForFullPreview(const TRect &imgRect)
{
	...
	if (!panel->isFloating())
		return;
	QDesktopWidget *desk = static_cast<QApplication *>(QApplication::instance())->desktop();
	QRect screenGeom = desk->availableGeometry(panel);

	while (1) {
		TAffine toWidgetRef(m_imageViewer->getImgToWidgetAffine(imgRectD));
		TRectD imageGeomD(toWidgetRef * imgRectD);
		TRect imageGeom(tceil(imageGeomD.x0) - 1, tceil(imageGeomD.y0) - 1, tfloor(imageGeomD.x1) + 1, tfloor(imageGeomD.y1) + 1);
...
void FlipBook::adaptWidGeometry(const TRect &interestWidGeom, const TRect &imgWidGeom, bool keepPosition)
{
	TPanel *panel = static_cast<TPanel *>(parentWidget());
	if (!panel->isFloating())
		return;
	...
	QRect qgeom(interestWidGeom.x0, interestWidGeom.y0, interestWidGeom.getLx(), interestWidGeom.getLy());
	QRect interestGeom(
		m_imageViewer->mapToGlobal(qgeom.topLeft()),
		m_imageViewer->mapToGlobal(qgeom.bottomRight()));
	qgeom = QRect(imgWidGeom.x0, imgWidGeom.y0, imgWidGeom.getLx(), imgWidGeom.getLy());
	QRect imageGeom(
		m_imageViewer->mapToGlobal(qgeom.topLeft()),
		m_imageViewer->mapToGlobal(qgeom.bottomRight()));

	//qDebug("tgeom= [%d, %d] x [%d, %d]", tgeom.x0, tgeom.x1, tgeom.y0, tgeom.y1);
	//qDebug("imagegeom= [%d, %d] x [%d, %d]", imageGeom.left(), imageGeom.right(),
	...
	//Build the minimum flipbook geometry. Adjust the interesting geometry according to it.
	...
	newViewerGeom.adjust(-margins.left(), -margins.top(), -margins.right(), -margins.bottom());

	// when fx previewing in full size (i.e. keepPosition is false ),
	// try to translate geometry and keep the image inside the viewer as much as posiible
	if (keepPosition)
	...
	//qDebug("new Viewer= [%d, %d] x [%d, %d]", newViewerGeom.left(), newViewerGeom.right(),
	//  newViewerGeom.top(), newViewerGeom.bottom());

	//Calculate the pan of content image in order to compensate for our geometry change
	QPointF imageGeomCenter(
		(imageGeom.left() + imageGeom.right() + 1) * 0.5,
  ...
  QPointF newViewerGeomCenter(
	...
	//NOTE: If delta == (0,0) the image is at center. Typically happens when imageGeom doesn't intersect
	//the screen geometry.
...
void FlipBook::onDoubleClick(QMouseEvent *me)
{
	...
	TRect pixGeom(tceil(pixGeomD.x0), tceil(pixGeomD.y0), tfloor(pixGeomD.x1) - 1, tfloor(pixGeomD.y1) - 1);

	//NOTE: The previous line has ceils and floor inverted on purpose. The reason is the following:
	//As the viewer's zoom level is arbitrary, the image is likely to have a not integer geometry
	//with respect to the widget - the problem is, we cannot take the closest integer rect ENCLOSING ours,
	//or the ImageViewer class adds blank lines on image rendering.
	//So, we do the converse - take the closest ENCLOSED one - eventually to be compensated when
...
*/
void FlipBook::loadAndCacheAllTlvImages(Level level,
										int fromFrame,
										int toFrame)
{
...
//! \li The flipbook where the file is to be opened. If none, a new one is created.
//! \li Whether the level must replace an existing one on the flipbook, or it must
//! rather be appended at its end
//! \li In case the file has a movie format and it is known to be a toonz output,
//! some additional random access information may be retrieved (i.e. images may map
//! to specific frames).
void viewFile(const TFilePath &path, int from, int to, int step, int shrink,
			  TSoundTrack *snd, FlipBook *flipbook, bool append, bool isToonzOutput)
{
	//In case the step and shrink informations are invalid, load them from preferences
	if (step == -1 || shrink == -1) {
	...
	flipbook->setLevel(path, 0, from, to, step, shrink, snd, append, isToonzOutput);
}
