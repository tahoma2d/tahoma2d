	...
	: TPanel(parent)
{
	...
	connect(doublePairFld, SIGNAL(valuesChanged(bool)), SLOT(onDoubleValuesChanged(bool)));
	layout->addWidget(doublePairFld, 1);

	DoubleField *doubleFld = new DoubleField();
	doubleFld->setRange(-100, 100);
	doubleFld->setValue(50.55);
	connect(doubleFld, SIGNAL(valueChanged(bool)), SLOT(onDoubleValueChanged(bool)));
	...
	void execute()
	{
		...
		if (currentRoom) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (pane->isHidden() && pane->isFloating() && pane->getPanelType() == "Test") {
					pane->show();
					pane->raise();
					return;
				}
