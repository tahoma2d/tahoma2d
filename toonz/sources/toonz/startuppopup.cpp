...
void StartupPopup::updateProjectCB() {
    ...
    QString currentProjectPath = pm->getCurrentProjectPath().getParentDir().getQString();
    ...
    m_projectsCB->setItemData(0, pm->getSandboxProjectFolder().getQString(), Qt::ToolTipRole);

    QList<QString> recentProjects = RecentFiles::instance()->getFilesNameList(RecentFiles::Project);
    int j = 1;
    for (int i = 0; i < recentProjects.size(); i++) {
        TFilePath fp(recentProjects.at(i));
        if (pm->isProject(fp) && recentProjects.at(i) != pm->getSandboxProjectFolder().getQString()) {
