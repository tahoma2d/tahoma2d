...
public:
	DeleteFxsUndo(const QList<TFx *> &fxs)
	{
		...
		for (int i = 0; i < (int)fxs.size(); i++) {
			...
			for (j = 0; j < fx->getInputPortCount(); j++) {
				...
				int i;
				if (inputFx && !fxDag->getTerminalFxs()->containsFx(inputFx.getPointer())) {
					for (i = 0; i < (int)m_inputConnectedToXsheet.size(); i++)
						if (m_inputConnectedToXsheet[i].getPointer() == inputFx.getPointer())
							break;
			...
			for (j = 0; j < fx->getOutputConnectionCount(); j++) {
				...
				if (outFx) {
					...
					for (k = 0; k < outFx->getInputPortCount(); k++)
						if (outFx->getInputPort(k)->getFx() == fx)
							break;
	...
	void onAdd()
	{
		FxDag *fxDag = TApp::instance()->getCurrentXsheet()->getXsheet()->getFxDag();
		int i;
...
FxSchematicNodeSelection::FxSchematicNodeSelection(const FxSchematicNodeSelection &src)
