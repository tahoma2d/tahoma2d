...
void my_compute_lut(double contrast, double brightness, std::vector<int> &lut)
{
	...
	for (i = 0; i < lutSize; i++) {
		...
		if (contrast < 0.0) {
			...
			if (nvalue < 0.0)
				nvalue = 0.0;
			power = (contrast == 1.0) ? half_maxChannelValueD : 1.0 / (1.0 - contrast);
...
template <>
inline void doPix<TPixelGR8>(TPixelGR8 *outPix, const TPixelGR8 *inPix, const std::vector<int> &lut)
{
...
template <typename PIX>
void onChange(const TRasterPT<PIX> &in, const TRasterPT<PIX> &out, int contrast, int brightness)
{
...
void onChange(const TRasterP &in, const TRasterP &out, int contrast, int brightness)
{
	TRaster32P in32(in);
	if (in32) {
	...
	if (inGR16)
		onChange<TPixelGR16>(inGR16, out, contrast, brightness);
}
