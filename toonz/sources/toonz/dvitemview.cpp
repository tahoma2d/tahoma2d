...
TEnv::IntVar BrowserVersionControlStatusisVisible("BrowserVersionControlStatusisVisible", 1);
...
QString DvItemListModel::getItemDataAsString(int index, DataType dataType)
{
	...
	case ModifiedDate:
		...
		break;
	case FileSize: {
		if (getItemData(index, IsFolder).toBool())
			return QString("");

		uint64_t byteSize = value.toLongLong();

		if (byteSize < 1024)
			return QString::number(byteSize) + " bytes";
...
int DvItemListModel::compareData(DataType dataType, int firstIndex, int secondIndex)
{
	...
	case Name:
	case FileType:
		return QString::localeAwareCompare(firstValue.toString(), secondValue.toString());

	case CreationDate:
	case ModifiedDate: {
		if (firstValue.toDateTime() < secondValue.toDateTime())
			return 1;
		if (firstValue.toDateTime() == secondValue.toDateTime())
			return 0;
		if (firstValue.toDateTime() > secondValue.toDateTime())
...
void DvItemSelection::selectAll()
{
	m_selectedIndices.clear();
	int i = 0;
	for (i = 0; i < m_model->getItemCount(); i++)
...
void DvItemSelection::setModel(DvItemListModel *model)
{
	m_model = model;
}
...
{
	if (m_model)
		m_model->enableSelectionCommands(this);
}
...
ItemViewPlayWidget::PlayManager::PlayManager()
	: m_path(TFilePath()), m_currentFidIndex(0), m_pixmap(QPixmap()), m_iconSize(QSize())
{
}
...
void ItemViewPlayWidget::PlayManager::setInfo(DvItemListModel *model, int index)
{
	...
	reset();
	m_pixmap = model->getItemData(index, DvItemListModel::Thumbnail).value<QPixmap>();
	if (!m_pixmap.isNull())
		m_iconSize = m_pixmap.size();
	m_path = path;
...
bool ItemViewPlayWidget::PlayManager::setCurrentFrameIndexFromXValue(int xValue, int lenght)
{
	if (m_fids.size() == 0)
		return false;
	double d = (double)lenght / (double)(m_fids.size() - 1);
	int newIndex = tround((double)xValue / d);
	if (newIndex == m_currentFidIndex)
		return false;
...
QPixmap ItemViewPlayWidget::PlayManager::getCurrentPixmap()
{
	return m_pixmap;
}
...
ItemViewPlayWidget::ItemViewPlayWidget(QWidget *parent)
	: QWidget(parent), m_currentItemIndex(-1), m_timerId(0), m_isSliderMode(false)
{
...
void DVItemViewPlayDelegate::resetPlayWidget()
{
	m_itemViewPlay->clear();
}
...
{
	if (m_itemViewPlay->isIndexPlaying(index) && !m_itemViewPlay->isSliderMode()) // Modalita' play
	{
	...
	: QFrame(parent), m_selection(0), m_viewer(viewer), m_viewType(viewer->m_windowType == DvItemViewer::Cast ? to_enum(CastView) : to_enum(BrowserView)), m_xMargin(0), m_yMargin(0), m_itemSpacing(0), m_itemPerRow(0), m_itemSize(0, 0), m_iconSize(80, 60), m_currentIndex(-1), m_singleColumnEnabled(false), m_centerAligned(false), m_noContextMenu(noContextMenu), m_isPlayDelegateDisable(true), m_globalSelectionEnabled(true), m_multiSelectionEnabled(multiSelectionEnabled), m_missingColor(Qt::gray)
{
	...
	m_columns.push_back(std::make_pair(DvItemListModel::Name, std::make_pair(200, 1)));
}

//-----------------------------------------------------------------------------

void DvItemViewerPanel::setItemViewPlayDelegate(DVItemViewPlayDelegate *playDelegate)
{
...
void DvItemViewerPanel::addColumn(DvItemListModel::DataType dataType, int width)
{
	bool val;
	switch (dataType) {
...
void DvItemViewerPanel::setColumnWidth(DvItemListModel::DataType dataType, int width)
{
	int i;
...
void DvItemViewerPanel::setVisibility(DvItemListModel::DataType dataType, bool value)
{
	int i;
	for (i = 0; i < m_columns.size(); i++) {
		if (m_columns[i].first != dataType)
...
void DvItemViewerPanel::updateViewParameters(int panelWidth)
{
	...
	case ThumbnailView:
		...
		w = (panelWidth + m_itemSpacing - m_itemPerRow * (m_itemSize.width() + m_itemSpacing)) / 2;
		if (w > m_xMargin)
			m_xMargin = w;
		break;
	}
	if (m_centerAligned) {
		...
		int contentHeight = rowCount * (m_itemSize.height() + m_itemSpacing) - m_itemSpacing;
...
void DvItemViewerPanel::paintThumbnailItem(QPainter &p, int index)
{
	...
	int status = getModel()->getItemData(index, DvItemListModel::VersionControlStatus).toInt();

	bool isSelected = m_selection->isSelected(index);
	QRect rect = index2pos(index);
	if (!visibleRegion().intersects(rect))
		return;
	if (!getModel())
		return;
	...
	QString name = getModel()->getItemData(index, DvItemListModel::Name).toString();
	int frameCount = getModel()->getItemData(index, DvItemListModel::FrameCount).toInt();
	if (frameCount > 0) {
		QString num;
	...
	if (status != DvItemListModel::VC_None && status != DvItemListModel::VC_Missing) {
...
void DvItemViewerPanel::paintListItem(QPainter &p, int index)
{
	...
	if (!getModel())
		return;
	QPixmap icon = getModel()->getItemData(index, DvItemListModel::Icon).value<QPixmap>();
	QString name = getModel()->getItemData(index, DvItemListModel::Name).toString();
	bool isSelected = m_selection->isSelected(index);
	if (isSelected)
		p.fillRect(rect, QColor(171, 206, 255));
	p.drawPixmap(rect.topLeft(), icon);
	rect.adjust(30, 0, 0, 0);
	p.setPen(Qt::black);
	p.drawText(rect, Qt::AlignLeft | Qt::AlignVCenter, elideText(name, p.font(), rect.width()));
}
...
void DvItemViewerPanel::mousePressEvent(QMouseEvent *event)
{
	...
	if (!m_isPlayDelegateDisable) {
		...
		if (model->getItemData(index, DvItemListModel::PlayAvailable).toBool() && isSelected) {
			if (m_itemViewPlayDelegate->setPlayWidget(getModel(), index, iconRect, event->pos())) {
				update();
				return;
			}
		} else
	...
	if (0 <= index && index < getItemCount()) {
		if (0 != (event->modifiers() & Qt::ControlModifier)) {
			...
			if (!isSelected) {
				int a = index, b = index;
				while (a > 0 && !m_selection->isSelected(a - 1))
					a--;
				if (a == 0)
					a = index;
				int k = getItemCount();
				while (b < k && !m_selection->isSelected(b + 1))
					b++;
				if (b == k)
					b = index;
				int i;
...
void DvItemViewerPanel::mouseMoveEvent(QMouseEvent *event)
{
	...
	if ((event->pos() - m_startDragPosition).manhattanLength() < 20)
		return;
	// e se c'e' una selezione non vuota
	if (m_currentIndex < 0 || m_currentIndex >= getItemCount())
		return;
...
void DvItemViewerPanel::mouseDoubleClickEvent(QMouseEvent *event)
{
	...
	if (index < 0 || index >= getItemCount())
		return;
	if (!getModel()->canRenameItem(index))
		return;
...
{
	m_viewType = ListView;
	m_viewer->m_windowType == DvItemViewer::Cast ? CastView = ListView : BrowserView = ListView;
...
void DvItemViewerPanel::setTableView()
{
	...
	m_viewer->m_windowType == DvItemViewer::Cast ? CastView = TableView : BrowserView = TableView;
	emit viewTypeChange(m_viewType);
	m_viewer->updateContentSize();
	update();
}
...
void DvItemViewerPanel::exportFileList()
{
	...
	TFilePath fp;
	if (scene)
		fp = scene->decodeFilePath(project->getFolder(TProject::Extras));
	...
	if (data.open(QFile::WriteOnly)) {
		...
		for (int index = 0; index < getItemCount(); index++) {
			...
			for (int i = 0; i < (int)m_columns.size(); i++) {
				...
				if (dataType != DvItemListModel::Name && dataType != DvItemListModel::FrameCount)
					continue;
	...
	: QScrollArea(parent), m_model(0)
{
	...
	m_panel = new DvItemViewerPanel(this, noContextMenu, multiSelectionEnabled, 0);
...
void DvItemViewer::updateContentSize()
{
	...
	if (w < width())
		w = width();
	int h = m_panel->getContentHeight(w) + 20; // 20 is margin for showing the empty area
	if (h < height())
		h = height();
	m_panel->resize(w, h);
}
...
{
	if (m_panel->getSelection())
		m_panel->getSelection()->selectNone();
}
...
DvItemViewerButtonBar::DvItemViewerButtonBar(DvItemViewer *itemViewer, QWidget *parent)
	: QToolBar(parent)
{
	...
	static QPixmap backButtonEnablePM = QPixmap(":Resources/fb_history_back_enable.png");
	static QPixmap backButtonDisablePM = QPixmap(":Resources/fb_history_back_disable.png");
	static QPixmap fwdButtonEnablePM = QPixmap(":Resources/fb_history_fwd_enable.png");
	static QPixmap fwdButtonDisablePM = QPixmap(":Resources/fb_history_fwd_disable.png");
