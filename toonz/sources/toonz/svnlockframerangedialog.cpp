...
void SVNLockMultiFrameRangeDialog::onStatusRetrieved(const QString &xmlResponse)
{
	m_waitingLabel->hide();
	...
	if (info.m_userName != "" && info.m_from != 0)
		m_lockInfos.append(info);
...
{
	for (int i = m_fromLineEdit->getValue() - 1; i < m_toLineEdit->getValue(); i++)
	...
	: Dialog(TApp::instance()->getMainWindow(), true, false), m_workingDir(workingDir), m_file(file), m_hasError(false)
{
	...
	m_textLabel->setText(tr("Note: the file will be updated too. Are you sure ?"));
	...
	connect(m_unlockButton, SIGNAL(clicked()), this, SLOT(onUnlockButtonClicked()));
...
void SVNUnlockMultiFrameRangeDialog::onStatusRetrieved(const QString &xmlResponse)
{
	m_waitingLabel->hide();
...
void SVNUnlockMultiFrameRangeDialog::onUnlockDone()
{
	if (!m_hasError) {
		...
		int fileCount = m_filesToUnlock.size();
		for (int i = 0; i < fileCount; i++)
			files.append(m_filesToUnlock.at(i));
		emit done(files);
		close();
	}
}
...
void SVNMultiFrameRangeLockInfoDialog::onStatusRetrieved(const QString &xmlResponse)
{
	m_waitingLabel->hide();
