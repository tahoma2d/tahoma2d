...
void getSelectedFrames(std::map<LevelType *, std::set<TFrameId>> &frames)
{
	...
	if (TCellSelection *cellSelection = dynamic_cast<TCellSelection *>(selection)) {
	...
	if (TColumnSelection *columnSelection = dynamic_cast<TColumnSelection *>(selection)) {
		TXsheet *xsh = TApp::instance()->getCurrentXsheet()->getXsheet();
