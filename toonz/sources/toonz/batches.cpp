...
bool LoadTaskListPopup::execute()
{
	...
	if (!TSystem::doesExistFileOrLevel(fp)) {
		...
		DVGui::error(toQString(fp) + tr("It is possible to load only TNZBAT files."));
		return false;
	}
...
void TaskRunner::doRun(TFarmTask *task)
{
	...
	if (count > 1) {
		...
		for (int i = 0; i < task->getTaskCount(); i++) {
			if (task->getTask(i)->m_status == Aborted)
  ...
  commandline += " -id " + task->m_id;*/
	...
	bool statusOK = (error == QProcess::UnknownError) &&
					(process->exitCode() == 0 || task->m_successfullSteps == task->m_stepCount);
...
void BatchesController::addComposerTask(const TFilePath &_taskFilePath)
{
	...
	TFilePath outputPath = scene.decodeFilePath(sprop->getOutputProperties()->getPath());
...
//assert(it == m_tasks.end());
	...
	int count = task->getTaskCount();
	if (count == 1 && task->getTask(0) == task)
		return;
...
{
	if (mapItem.second->m_parentId.isEmpty())
...
int BatchesController::getTaskCount() const
{
	return m_tasks.size();
}
...
void BatchesController::startAll()
{
	...
	if (!m_controller) {
		...
		std::multimap<int, TFarmTask *>::reverse_iterator it2 = tasksByPriority.rbegin();
...
void notifyObserver(BatchesController::Observer *obs)
{
	obs->update();
}
}
...
void BatchesController::update()
{
	...
	for (; jtor != m_tasks.end(); ++jtor) {
		...
		if (task->m_status == Completed)
			m_farmIdsTable.erase(task->m_id);
	}
...
void MyLocalController::removeTask(const QString &id)
{
	assert(false);
}

void MyLocalController::suspendTask(const QString &id)
{
	assert(false);
}

void MyLocalController::activateTask(const QString &id)
{
	assert(false);
}

void MyLocalController::restartTask(const QString &id)
{
	assert(false);
}

void MyLocalController::getTasks(vector<QString> &tasks)
{
	assert(false);
}
	...
	FrameState state)
{
	...
	if (task->m_parentId != "") {
		TFarmTask *taskParent = BatchesController::instance()->getTask(task->m_parentId);
		assert(taskParent);
		if (state == FrameDone)
