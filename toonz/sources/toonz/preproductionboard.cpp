...
BoardButtonBar::BoardButtonBar(QWidget *parent, BoardList *fileList)
  ...
  connect(loadAction, SIGNAL(triggered()), SIGNAL(loadBoard()));
}
...
QPixmap BoardList::createThumbnail(const TFilePath &fp) {
  ...
  if (fp.getType() != "tnz") {
    if (fp.isLevelName() || !isExpandable(fp.getType())) {
      source = IconGenerator::instance()->getIcon(fp);
...
void BoardList::addFile(const QString &name, const QString &path, int atIndex) {
  ...
  if (!hasPixmap) {
    // If no pixmap, it might need time to generate. Fill w/ blank for now and
    // recheck later
  ...
  if (fp.getType() == "tnz") {
    ToonzScene scene;
    try {
      frames = scene.loadFrameCount(fp);
    } catch (...) {
    }
  ...
  // Recheck for missing pixmap in 2 seconds
  if (!hasPixmap) {
    QTimer::singleShot(1000, this, [this, lwi]() {
      TFilePath fp(lwi->data(Qt::UserRole).toString());
...
void BoardList::contextMenuEvent(QContextMenuEvent *event) {
  ...
  bool ret = ret && connect(toggleIconWrapAction, SIGNAL(triggered()), this,
  ...
  if (selectedCount == 1 && isExpandable(ext)) {
    if (fp.isLevelName()) {
      ...
      menu.addAction(action);
    } else if (fp.getQString() != QString::fromStdWString(fp.getWideName())) {
...
void BoardList::onRenameFile() {
  ...
  QString newName = QString::fromStdString(newFile.getLevelName());
...
void BoardList::onExpandSequnce() {
  ...
  for (int i = 0; i < list.size(); i++) {
    ...
    QString path = QString::fromStdWString(f.getWideString());
...
PreproductionBoard::PreproductionBoard(QWidget *parent, Qt::WindowFlags flags,
  ...
  bool ret =
      ret && connect(buttonBar, SIGNAL(newBoard()), this, SLOT(onNewBoard()));
  ret = ret &&
  ...
  ret =
      ret && connect(buttonBar, SIGNAL(loadBoard()), this, SLOT(onLoadBoard()));
...
void PreproductionBoard::onNewScene() {
  ...
  QString name = QString::fromStdString(scenePath.getLevelName());
...
void PreproductionBoard::onLoadBoard() {
