...
public:
	...
	virtual QVariant getItemData(int index, DataType dataType, bool isSelected = false) = 0;
...
class ItemViewPlayWidget : public QWidget
{
	...
	public:
		...
		/*! Increase current frame if icon is computed; return true if frame is increased. */
...
public:
	...
	void setIsPlayingCurrentFrameIndex(DvItemListModel *model, int index, int xValue, int lenght);
...
public:
	...
	const std::set<int> &getSelectedIndices() const { return m_panel->getSelectedIndices(); }
	...
	void enableGlobalSelection(bool enabled) { m_panel->enableGlobalSelection(enabled); }
	void selectNone();
