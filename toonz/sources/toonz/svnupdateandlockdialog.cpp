	...
	: Dialog(TApp::instance()->getMainWindow(), true, false), m_editSceneContentsCheckBox(0), m_workingDir(workingDir), m_files(files), m_workingRevision(workingRevision), m_hasError(false), m_sceneIconAdded(sceneIconAdded), m_updateWorkingRevisionNeeded(false)
{
	...
	connect(m_updateAndLockButton, SIGNAL(clicked()), this, SLOT(onUpdateAndLockButtonClicked()));
...
void SVNUpdateAndLockDialog::onStatusRetrieved(const QString &xmlResponse)
{
	...
	if (m_filesToEdit.size() == 0) {
		...
		m_textLabel->setText(tr("%1 items to edit.").arg(m_filesToEdit.size() - m_sceneIconAdded));
		m_lockButton->show();
...
void SVNUpdateAndLockDialog::onSceneResourcesStatusRetrieved(const QString &xmlResponse)
{
	SVNStatusReader sr(xmlResponse);
	m_status = sr.getStatus();
...
{
	m_textLabel->setText(tr("Editing %1 items...").arg(m_filesToEdit.size() - m_sceneIconAdded));
