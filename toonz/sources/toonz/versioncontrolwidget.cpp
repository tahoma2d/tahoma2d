	...
	: QWidget(parent), m_selectedRadioIndex(0)
{
	...
	connect(m_timeRadioButton, SIGNAL(clicked()), this, SLOT(onRadioButtonClicked()));
	...
	connect(m_dayRadioButton, SIGNAL(clicked()), this, SLOT(onRadioButtonClicked()));
	...
	connect(m_weekRadioButton, SIGNAL(clicked()), this, SLOT(onRadioButtonClicked()));
	...
	connect(m_dateRadioButton, SIGNAL(clicked()), this, SLOT(onRadioButtonClicked()));
...
void ConflictWidget::onRadioValueChanged()
{
	...
	if (obj)
		m_radios[obj] = obj->getValue();

	if (!m_radios.values().contains(-1))
		emit allConflictSetted();
}
...
QString DoubleRadioWidget::getText() const
{
	return m_label->text();
}
