...
// xsheet <- cells
//
void CellsMover::setCells(const std::vector<TXshCell> &cells, int r, int c) const
{
	TXsheet *xsh = getXsheet();
	for (int i = 0; i < m_colCount; i++) {
		TXshColumn *column = xsh->getColumn(c + i);
...
void CellsMover::undoMoveCells(const TPoint &pos) const
{
	...
	if (m_qualifiers & eInsertCells) {
		...
		for (int i = 0; i < m_colCount; i++)
			xsh->clearCells(r, c + i, m_rowCount);
		if (m_qualifiers & eOverwriteCells)
			setCells(m_oldCells, r, c);
	}
}
...
bool CellsMover::canMoveCells(const TPoint &pos)
{
	...
	for (int i = 0; i < m_colCount; i++) {
		for (int j = 0; j < m_rowCount; j++) {
			if (!xsh->getCell(pos.y + j, pos.x + i).isEmpty())
				return false;
			count++;
		}
	}
...
void CellsMover::restoreColumns(int c) const
{
	...
	for (int i = 0; i < m_colCount; i++) {
		...
		if (r0 <= r1)
			cellColumn->clearCells(r0, r1 - r0 + 1);
	}
}
...
public:
	...
	void undo() const
	{
		...
		if (ca != cb) {
			if (m_cellsMover.m_uffa & 2)
				m_cellsMover.emptyColumns(cb);
			if (m_cellsMover.m_uffa & 1)
				m_cellsMover.restoreColumns(ca);
		}
	...
	void redo() const
	{
		...
		if (ca != cb) {
			if (m_cellsMover.m_uffa & 1)
				m_cellsMover.emptyColumns(ca);
			if (m_cellsMover.m_uffa & 2)
				m_cellsMover.restoreColumns(cb);
		}
...
bool LevelMoverTool::isTotallyEmptyColumn(int col) const
{
	...
	if (!column->isEmpty())
		return false;
	if (column->getFx()->getOutputConnectionCount() != 0)
		return false;
...
bool LevelMoverTool::canMoveColumns(const TPoint &pos)
{
	...
	if (pos.x != m_lastPos.x) {
		...
		for (int i = 0; i < m_range.lx; i++) {
			...
			if (srcColumn && srcColumn->isLocked())
				continue;
			TXshColumn *dstColumn = xsh->getColumn(dstIndex);
			TXshColumn::ColumnType srcType = TXshColumn::eLevelType;
			if (srcColumn)
				srcType = srcColumn->getColumnType();
			if (srcType == TXshColumn::eZeraryFxType)
				return false;
			/*
...
void LevelMoverTool::onCellChange(int row, int col)
{
	...
	if (m_validPos) {
		...
		if (delta.x != 0 && (m_qualifiers & CellsMover::eMoveColumns)) {
			...
			bool emptyDst = true;
			for (int i = 0; i < colCount; i++) {
				if (!isTotallyEmptyColumn(pos.x + i))
					emptyDst = false;
			...
			if (emptySrc) {
				if (m_lastPos.x == m_startPos.x)
					m_undo->getCellsMover()->m_uffa |= 1; // first column(s) has/have been cleared
