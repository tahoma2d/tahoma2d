...
protected:
	void paintEvent(QPaintEvent *)
	{
		...
		p.drawText((width() - textWidth) * 0.5, (height() - fm.height()) * 0.5, elidedText);
	}
};
...
/*! \class MessagePanel
		\brief MessagePanel class provides a pane to view a single, centered text message.
...
void MessagePanel::setViewType(int viewType)
{
	m_viewType = viewType;
}
...
void MessagePanel::setPoolIndex(int poolIndex)
{
...
int MessagePanel::getPoolIndex() const
{
	return m_poolIndex;
}
...
void MessagePanel::setMessage(QString text)
{
...
//=============================================================================
/*! \class LogPanel
		\brief The LogPanel class provide a pane to log text messages in a QTextEdit document.
...
LogPanel::~LogPanel()
{
	TLogger::instance()->removeListener(this);
}
...
void LogPanel::onLogChanged()
{
	...
	m_messageBox->insertPlainText(QString::fromStdString(message.getText()) + QString("\n"));
}
	...
	void execute()
	{
		...
		if (currentRoom) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (pane->isHidden() && pane->isFloating() && pane->getPanelType() == "Message") {
					pane->show();
					pane->raise();
					return;
				}
