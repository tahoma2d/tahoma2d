...
void XsheetGUI::DragTool::refreshCellsArea()
{
...
void XsheetGUI::DragTool::refreshColumnsArea()
{
	getViewer()->updateColumns();
}

//-----------------------------------------------------------------------------

void XsheetGUI::DragTool::refreshRowsArea()
{
	getViewer()->updateRows();
}
		...
		if (m_modifier & Qt::ShiftModifier) {
			...
			if (r0 <= r1 && c0 <= c1) {
				...
				if (m_modifier & Qt::ControlModifier)
					getViewer()->getCellKeyframeSelection()->selectCellsKeyframes(r0, c0, r1, c1);
				else
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeSelectionTool(XsheetViewer *viewer)
{
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeLevelMoverTool(XsheetViewer *viewer)
{
...
public:
		...
		: m_firstRow(row), m_sequenceFound(false), m_base(0), m_inc(0), m_step(0), m_offset(0), m_count(0), m_sourceCells(rowCount, TXshCell()), m_invert(invert)
	{
		if (!m_invert)
			...
			// the "first row" becomes the bottom-most row for the upper-directional smart tab
	...
	void analyzeCells()
	{
		...
		for (i = 1; i < count; i++)
			if (m_sourceCells[i].m_frameId != cell.m_frameId)
				break;
		...
		// l'ultimo elemento
		for (; i < count; i++)
			if (m_sourceCells[i].m_frameId != computeFrame(i))
				break;
		// check if there are the double loops
		if (i < count) {
			TFrameId first(m_base);
			if (m_sourceCells[i].m_frameId != first)
				return;
			if (m_step > 1 && ((i + m_offset) % m_step) != 0)
				return;
			m_count = i + m_offset;
...
public:
	...
	void onCellChange(int row, int col)
	{
		...
		m_r1 = r1;
		getViewer()->getCellSelection()->selectCells(m_r0, m_c0, m_r1, m_c0 + m_colCount - 1);
	}
	...
	void onRelease(int row, int col)
	{
		...
		if (delta == 0)
			...
			if (delta > 0)
				m_undo->setCells(getViewer()->getXsheet(), m_r0, m_c0, m_r1 - m_r0 + 1, m_colCount);
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeLevelExtenderTool(XsheetViewer *viewer, bool invert)
{
...
public:
	...
	void onChange(int row)
	{
		...
		soundColumn->assignLevels(m_oldColumn.getPointer());
		soundColumn->modifyCellRange(m_firstRow, delta, m_modifierType == START_TYPE);
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeSoundLevelModifierTool(XsheetViewer *viewer)
{
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeKeyframeMoverTool(XsheetViewer *viewer)
{
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeCellKeyframeMoverTool(XsheetViewer *viewer)
{
...
public:
	...
	void onClick(const QMouseEvent *e)
	{
		...
		TPointD cellTopLeft(getViewer()->columnToX(m_startCol), getViewer()->rowToY(m_startRow));
	...
	void onChange(TPointD pos)
	{
		...
		TPointD newPos = pos - TPointD(getViewer()->columnToX(col), getViewer()->rowToY(row));
	...
	void onRelease(const QMouseEvent *event)
	{
		...
		if (m_startPos == pos && m_startRow == row && m_startCol == col)
			return;
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeNoteMoveTool(XsheetViewer *viewer)
{
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeKeyOnionSkinMaskModifierTool(XsheetViewer *viewer,
																		   bool isFos)
{
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeCurrentFrameModifierTool(XsheetViewer *viewer)
{
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makePlayRangeModifierTool(XsheetViewer *viewer)
{
...
public:
	...
	void onClick(const QMouseEvent *event)
	{
		...
		if (event->modifiers() & Qt::ControlModifier) {
			...
			while (ia > 0 && !selection->isColumnSelected(ia - 1))
				--ia;
			if (ia == 0)
				ia = col;
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeColumnSelectionTool(XsheetViewer *viewer)
{
...
void moveColumns(const std::set<int> &indices, int delta)
{
	if (indices.empty())
		return;
	if (delta < 0 && *indices.begin() + delta < 0)
		delta = -*indices.begin();
	if (delta == 0)
		return;
...
public:
	...
	{
		std::set<int> ii;
		for (std::set<int>::const_iterator it = m_indices.begin(); it != m_indices.end(); ++it)
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeColumnMoveTool(XsheetViewer *viewer)
{
...
public:
	...
	void onRelease(int row, int col)
	{
		...
		if (m_firstCol == -1)
			columnId = getViewer()->getXsheet()->getStageObjectTree()->getCurrentCameraId();
		TStageObjectId parentId(getViewer()->getObjectId(m_lastCol));
		if (m_lastCol == -1)
			parentId = getViewer()->getXsheet()->getStageObjectTree()->getCurrentCameraId();
		...
		if (m_firstCol == m_lastCol && m_firstCol != -1) {
			...
			// brutto che questa cosa sia qui. Bisognerebbe spostarlo altrove (in tnzlib)
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeColumnLinkTool(XsheetViewer *viewer)
{
...
public:
	...
	void onDrag(const QMouseEvent *event)
	{
		if (!m_enabled)
			return;
		double v = (double)(60 - (event->pos().y() - (XsheetGUI::RowHeight * 2 + 4))) / 60.0;
		TXsheet *xsh = getViewer()->getXsheet();
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeVolumeDragTool(XsheetViewer *viewer)
{
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeSoundScrubTool(XsheetViewer *viewer,
															 TXshSoundColumn *sc)
{
...
public:
	...
	void addSimpleLevel(std::pair<TXshSimpleLevelP, std::vector<TFrameId>> level)
	{
		m_levels.push_back(level);
	}
	void setSimpleLevels(std::vector<std::pair<TXshSimpleLevelP, std::vector<TFrameId>>> levels)
	{
		m_levels = levels;
	}
...
public:
	...
	void drawCellsArea(QPainter &p)
	{
		TPoint pos(getViewer()->xToColumn(m_curPos.x), getViewer()->yToRow(m_curPos.y));
		TRect rect = m_data->getLevelFrameRect();
		if (rect.isEmpty())
			return;
		rect += pos;
...
XsheetGUI::DragTool *XsheetGUI::DragTool::makeDragAndDropDataTool(XsheetViewer *viewer)
{
