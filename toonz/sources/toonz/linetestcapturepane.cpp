...
void LineTestImageViewer::enterEvent(QEvent *)
{
	setFocus();
}
...
void LineTestImageViewer::paintEvent(QPaintEvent *e)
{
	...
	if (!m_raster && !m_isViewFrameActive)
		return;
	...
	if (!m_isViewFrameActive && m_raster) {
		...
		if (m_isOnionSkinActive) {
			...
			if (currentRas) {
				...
				p.drawImage(-currentRas->getLx() * 0.5, -currentRas->getLy() * 0.5, currentImage);
	...
	if (capturePanelFieldGuideToggle.getStatus()) {
		TSceneProperties *sprop = TApp::instance()->getCurrentScene()->getScene()->getProperties();
		double f = 1;
...
ChooseCameraDialog::ChooseCameraDialog(QList<QString> cameras)
	: Dialog(TApp::instance()->getMainWindow(), true, false), m_cameraIndex(-1)
{
	...
	ret = ret && connect(okButton, SIGNAL(clicked()), this, SLOT(onOkButtonPressed()));
	QPushButton *cancelButton = new QPushButton(tr("Cancel"), this);
	ret = ret && connect(cancelButton, SIGNAL(clicked()), this, SLOT(onCancelButtonPressed()));
...
void ChooseCameraDialog::onOkButtonPressed()
{
	close();
}
	...
	: Dialog(TApp::instance()->getMainWindow()), m_defineDeviceButton(0), m_useWhiteImage(0), m_keepWhiteImage(0), m_imageWidthLineEdit(0), m_imageHeightLineEdit(0), m_contrastField(0), m_brightnessField(0)
{
	...
	ret = ret && connect(m_defineDeviceButton, SIGNAL(clicked()), this, SLOT(defineDevice()));
...
void CaptureSettingsPopup::showEvent(QShowEvent *)
{
	...
	bool ret = true;
	ret = ret && connect(camera, SIGNAL(imageSizeUpdated()), this, SLOT(updateWidgets()));
}
...
void CaptureSettingsPopup::onImageWidthEditingFinished()
{
	...
	double aspectRatio = (double)deviceResolution.ly / (double)deviceResolution.lx;
...
void CaptureSettingsPopup::onImageHeightEditingFinished()
{
	...
	TnzCamera *camera = TnzCamera::instance();
	TDimension deviceResolution = camera->getDeviceResolution();
	double aspectRatio = (double)deviceResolution.lx / (double)deviceResolution.ly;
...
FileSettingsPopup::FileSettingsPopup()
	: Dialog(TApp::instance()->getMainWindow()), m_pathField(0), m_fileFormat(0)
{
	...
	m_pathField = new DVGui::FileField(this, toQString(parameters->getFilePath()));
	m_pathField->setMaximumHeight(19);
	ret = ret && connect(m_pathField, SIGNAL(pathChanged()), this, SLOT(onPathChanged()));
	...
	ret = ret && connect(fileFormatButton, SIGNAL(pressed()), this, SLOT(openSettingsPopup()));
...
{
	getCaptureParameters()->setFilePath(TFilePath(m_pathField->getPath().toStdWString()));
}
	...
	: TPanel(parent), m_imageView(0), m_nameField(0), m_frameField(0), m_saveMode(0), m_onionSkin(0), m_incrementField(0), m_stepField(0), m_connectionCheckBox(0), m_lineColorField(0), m_captureButton(0), m_captureSettingsPopup(0), m_fileSettingsPopup(0), m_canCapture(true), m_timerId(0)
{
	...
	m_stepField = new DVGui::IntLineEdit(optionsWidget, parameters->getStep(), 1, 100);
...
bool LineTestCapturePane::capture()
{
	...
	if ((column && !column->getLevelColumn()) || (!cell.isEmpty() && cell.getChildLevel() != 0)) {
...
{
	if (!m_fileSettingsPopup)
		m_fileSettingsPopup = new FileSettingsPopup();
...
void LineTestCapturePane::onOnionSkinStateChanged(int state)
{
	...
	if (checked)
		m_viewFrame->setCheckState(Qt::Unchecked);
...
void LineTestCapturePane::onViewFrameStateChanged(int state)
{
	bool checked = state != Qt::Unchecked;
	if (checked)
		m_onionSkin->setCheckState(Qt::Unchecked);
	m_imageView->setViewFrameActive(checked);
	if (!m_connectionCheckBox->isChecked())
		m_imageView->repaint();
}
...
void LineTestCapturePane::updateFileField()
{
	...
	if (!column || column->getLevelColumn()) {
		...
		if (!cell.isEmpty()) {
			...
			TFilePath defaultPath = scene->getDefaultLevelPath(OVL_XSHLEVEL).getParentDir();
...
void LineTestCapturePane::onCaptureFinished()
{
	...
	TnzCamera *camera = TnzCamera::instance();
	IconGenerator::instance()->invalidate(camera->getCurrentLevel(), camera->getCurrentFid());
}
...
{
	if (m_captureSettingsPopup)
		m_captureSettingsPopup->updateWidgets();
	if (m_fileSettingsPopup)
		m_fileSettingsPopup->updateWidgets();
...
void LineTestCapturePane::changeWindowTitle()
{
	...
	TAffine aff = m_imageView->getViewMatrix() * m_imageView->getNormalZoomScale().inv();
