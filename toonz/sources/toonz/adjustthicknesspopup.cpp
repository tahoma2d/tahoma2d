...
struct TransformFunc {
	...
	void operator()(int s) //! Transforms the stroke thickness, and marks the specified
	...
	const int stylesSelection[], int stylesSelectionCount)
{
	...
	TransformFunc transformer = {*vi, transform};
	StylesFilter filter = {*vi, stylesSelection, stylesSelection + stylesSelectionCount};
...
TFrameId levelFid()
{
	return TApp::instance()->getCurrentFrame()->getFid();
}
...
AdjustThicknessPopup::FrameData::FrameData(const TXshSimpleLevelP &sl, int frameIdx)
	...
	: m_contentType(NONE), m_sl()
{
	struct Locals {
		...
		typedef tcg::function<int (TXshSimpleLevel::*)(const TFrameId &) const,
							  &TXshSimpleLevel::fid2index> Fid2Index;
	...
	if (sel && !sel->isEmpty()) {
		...
		else if (const TFilmstripSelection *fSel = dynamic_cast<const TFilmstripSelection *>(sel))
			locals.initialize(*fSel);
		else if (const TColumnSelection *cSel = dynamic_cast<const TColumnSelection *>(sel))
			locals.initialize(*cSel);
		else if (const StrokeSelection *sSel = dynamic_cast<const StrokeSelection *>(sel))
			locals.initialize(*sSel);
		else if (const LevelSelection *vlSel = dynamic_cast<const LevelSelection *>(sel))
...
void AdjustThicknessPopup::SelectionData::getRange(int &startIdx, int &endIdx) const
{
	...
	const double(&fromTransform)[2], const double(&toTransform)[2])
{
	struct locals {
			...
			int startIdx, int endIdx, int curIdx)
		{
			...
			transform[0] = tcg::numeric_ops::lerp(fromTransform[0], toTransform[0], relPos);
			transform[1] = tcg::numeric_ops::lerp(fromTransform[1], toTransform[1], relPos);
		}

	}; // locals

	if (!selData || !selData.m_sl)
...
{
	m_selectionData = SelectionData(TApp::instance()->getCurrentSelection()->getSelection());
}
...
{
	m_okBtn->setEnabled(bool(m_selectionData) || bool(SelectionData(m_currentFrameData)));
}
...
void AdjustThicknessPopup::onParamsChanged()
{
	schedulePreviewUpdate();
}
...
OpenPopupCommandHandler<AdjustThicknessPopup> openAdjustThicknessPopup(MI_AdjustThickness);
