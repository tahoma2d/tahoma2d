...
/*! \class LevelSettingsPopup
		\brief The LevelSettingsPopup class provides a dialog to show and change
	...
	: Dialog(TApp::instance()->getMainWindow(), false, false, "LevelSettings"), m_whiteTransp(0), m_scanPathLabel(0), m_scanPathFld(0)
{
	...
	m_topLayout->setSpacing(5);
	{
		...
		dpiLayout->setSpacing(5);
		{
			...
			dpiLayout->addWidget(m_squarePixCB, 1, 2, 1, 2, Qt::AlignRight | Qt::AlignVCenter);
			dpiLayout->addWidget(m_widthLabel, 2, 0, Qt::AlignRight | Qt::AlignVCenter);
	...
	connect(m_squarePixCB, SIGNAL(stateChanged(int)), SLOT(onSquarePixelChanged(int)));
	...
	connect(m_subsamplingFld, SIGNAL(editingFinished()), SLOT(onSubsamplingChanged()));

	/*--- ScanPathの入力に対応 ---*/
	connect(m_scanPathFld, SIGNAL(pathChanged()), SLOT(onScanPathChanged()));
	connect(m_doPremultiply, SIGNAL(stateChanged(int)), SLOT(onDoPremultiplyChanged(int)));
...
void LevelSettingsPopup::onSceneChanged()
{
	updateLevelSettings();
}
...
void LevelSettingsPopup::onCastSelectionChanged()
{
	updateLevelSettings();
}
...
void LevelSettingsPopup::onSelectionSwitched(TSelection *oldSelection, TSelection *newSelection)
{
...
void LevelSettingsPopup::updateLevelSettings()
{
	...
	/*--カラム選択の場合--*/
		...
		if (selectedLevelSize > 0)
			selectedLevel = levels[selectedLevelSize - 1];
	}
	...
	// dpi & res & resampling
	if (hasDpiEditing) {
		LevelProperties::DpiPolicy dpiPolicy = m_sl->getProperties()->getDpiPolicy();
		assert(dpiPolicy == LevelProperties::DP_ImageDpi ||
			   dpiPolicy == LevelProperties::DP_CustomDpi);
		m_dpiTypeOm->setCurrentIndex((dpiPolicy == LevelProperties::DP_ImageDpi) ? 0 : 1);
	...
	if (m_scanPathLabel)
		m_scanPathLabel->setEnabled(isTzpLevel);
	if (m_scanPathFld)
		m_scanPathFld->setEnabled(isTzpLevel);
...
void LevelSettingsPopup::onScanPathChanged()
{
	...
	newScanPath = TApp::instance()->getCurrentScene()->getScene()->codeFilePath(newScanPath);
	m_scanPathFld->setPath(QString::fromStdWString(newScanPath.getWideString()));

	if (!m_sl || m_sl->getType() != TZP_XSHLEVEL)
		return;

	/*-- 入力されたScanPathに変更がなければreturn--*/
	TFilePath oldScanPath = m_sl->getScannedPath();
	if (oldScanPath == newScanPath)
		return;

	/*-- CleanupSettingsのSaveInが今のLevelと一致しているかどうかをチェック --*/
	TFilePath settingsPath = (newScanPath.getParentDir() + newScanPath.getName()).withType("cln");
	settingsPath = TApp::instance()->getCurrentScene()->getScene()->decodeFilePath(settingsPath);
...
void LevelSettingsPopup::onDpiTypeChanged(const QString &dpiPolicyStr)
{
	...
	if (prop->getDpiPolicy() == dpiPolicy)
		return;
...
void LevelSettingsPopup::onSubsamplingChanged()
{
	...
	if (m_sl->getProperties()->getSubsampling() == (int)m_subsamplingFld->getValue())
		return;
...
void LevelSettingsPopup::onDoAntialiasChanged(int value)
{
	...
	m_sl->getProperties()->setDoAntialias(value == 0 ? 0 : m_antialiasSoftness->getValue());
	TApp::instance()->getCurrentLevel()->notifyLevelChange();
}
...
OpenPopupCommandHandler<LevelSettingsPopup> openLevelSettingsPopup(MI_LevelSettings);
...
public:
	...
	void execute()
	{
		...
		if (FileSelection *fileSelection = dynamic_cast<FileSelection *>(selection)) {
		...
		} else if (CastSelection *castSelection = dynamic_cast<CastSelection *>(selection))
...
public:
	...
	void execute()
	{
		...
		if (currentXsheet && cellSelection && !cellSelection->isEmpty()) {
			...
			simpleLevels.push_back(TApp::instance()->getCurrentLevel()->getSimpleLevel());

		if (!simpleLevels.size())
			return;
