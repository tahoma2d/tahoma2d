...
inline bool doTestConnection(const QString &hostname, const QString &ipaddr, int port)
{
	int sock;
	TTcpIpClient client;
...
void TFarmStuff::getControllerData(QString &hostName, QString &ipAddr, int &port)
{
	ControllerData data;
	...
	if (!fileExists)
		throw TFarmStuff::TMissingGRootFolder();
