...
struct Resource //!  A single resource to be loaded.
	{
	struct Path : private boost::less_than_comparable<Path> //!  Path locating a resource.
	{
	...
	const TFilePath &rootPath, const TFilePath &subPath = TFilePath())
{
	...
	{
		const QStringList &files = folderDir.entryList(QDir::Files); // Files only first
		...
		for (ft = files.begin(); ft != fEnd; ++ft) {
			...
			RsrcKey rsrcKey(Resource::Path(rootPath, relPath), int(fdt - l_formatDatas));
			rsrcMap[rsrcKey].push_back(Resource::Component(compPath, componentFunc));
		}

		// Further let found resources merge with others they recognize as 'children'
		RsrcMap::iterator rt, rEnd = rsrcMap.end();
		for (rt = rsrcMap.begin(); rt != rEnd; ++rt)
...
{
	return dstDir + (comp.m_pathFunc ? comp.m_pathFunc(comp.m_srcFp) : comp.m_srcFp);
}
...
struct import_Locals {
	...
	{
		if (!m_scene.isExternPath(rsrc.m_path.m_rootFp))
			return;

		try {
			// Build parent folder paths
			const TFilePath &srcDir = rsrc.m_path.absoluteResourcePath().getParentDir();

			switchToDst(rsrc.m_path);
			const TFilePath &dstDir = rsrc.m_path.absoluteResourcePath().getParentDir();
							   ...
							   LoadResourceArguments::ScopedBlock *sb)
{
	...
	bool import = false;
	{
		if (ba::any_of(args.resourceDatas.begin(), args.resourceDatas.end(),
