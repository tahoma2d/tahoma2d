...
void mergeRasterColumns(const vector<MatchlinePair> &matchingLevels)
{
	...
	for (i = 0; i < (int)matchingLevels.size(); i++) {
		...
		TRasterImageP match = (TRasterImageP)matchingLevels[i].m_mcell->getImage(false);
		if (!img || !match)
...
bool needTobeGrouped(const TVectorImageP &vimg)
{
	...
	if (!vimg->isStrokeGrouped(0))
		return true;

	for (int i = 1; i < vimg->getStrokeCount(); i++) {
		if (vimg->areDifferentGroup(0, false, i, false) == 0)
			return true;
	}
...
void mergeVectorColumns(const vector<MatchlinePair> &matchingLevels)
{
	...
	int i = 0;
	for (i = 0; i < (int)matchingLevels.size(); i++) {
		TVectorImageP vimg = (TVectorImageP)matchingLevels[i].m_cell->getImage(true);
		TVectorImageP vmatch = (TVectorImageP)matchingLevels[i].m_mcell->getImage(false);
		if (!vimg || !vmatch)
			throw TRopException("Cannot merge columns of different image types!");
		// img->lock();

		if (needTobeGrouped(vimg))
...
void mergeColumns(const set<int> &columns)
{
	...
	for (; it != columns.end(); ++it)
		mergeColumns(dstColumn, *it, false);
