...
bool loadFids(const TFilePath &path, TXshSimpleLevel *sl, const std::set<TFrameId> &selectedFids)
{
...
bool loadUnpaintedFids(TXshSimpleLevel *sl, const std::set<TFrameId> &selectedFids)
{
	TFilePath path = getUnpaintedLevelPath(sl);

	if (!TSystem::doesExistFileOrLevel(path)) {
		DVGui::error(QObject::tr("No cleaned up drawings available for the current selection."));
		return false;
	}
...
bool loadLastSaveFids(TXshSimpleLevel *sl, const std::set<TFrameId> &selectedFids)
{
	TFilePath path = sl->getPath();
	path = sl->getScene()->decodeFilePath(path);

	if (!TSystem::doesExistFileOrLevel(path)) {
		DVGui::error(QObject::tr("No saved drawings available for the current selection."));
		return false;
	}
...
/*--isCleanedUpが	Trueのとき： "revert to cleaned up" コマンド
					Falseのとき："revert to last saved" コマンド
--*/
void revertTo(bool isCleanedUp)
{
	...
	if (filmstripSelection) {
		...
		RevertToCommandUndo *undo = new RevertToCommandUndo(sl, selectedFids, isCleanedUp);
