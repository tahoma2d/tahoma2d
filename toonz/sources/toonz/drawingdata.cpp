...
// aggiorna gli hook: sono stati inseriti shiftRange (>0) drawings a partire da startFrameId
// quindi tutti i riferimenti a frameId >= startFrameId devono essere aggiornati

void shiftHooks(TXshSimpleLevel *sl, const TFrameId &startFrameId, int shiftRange)
{
...
{
	return "DrawingData" + QString::number(id) + "-" + QString::number(fid.getNumber());
}
						...
						const std::map<TFrameId, QString> &imageSet)
{
	...
	// es: frames=[1,3,5] selected=[3]  => devo inserire in 2 (creando un nuovo frameid).
...
void vectorize(TVectorImageP &target, const TToonzImageP &source,
			   const std::map<int, int> &styleTable, const VectorizerConfiguration &config)
{
	VectorizerCore vc;
...
void DrawingData::setLevelFrames(TXshSimpleLevel *sl, std::set<TFrameId> &frames)
{
	if (!sl || frames.empty())
		return;
	...
	for (it = frames.begin(); it != frames.end(); ++it) {
		...
		for (i = 0; i < hookCount; i++) {
			...
			if (!levelHook || levelHook->isEmpty())
				continue;

			Hook *copiedHook = m_levelHooks.getHook(i);
			if (!copiedHook)
				copiedHook = m_levelHooks.addHook();
								 ...
								 bool isRedo) const
{
	...
	for (mapIt; mapIt != m_imageSet.end(); ++mapIt) {
		...
		if (!imgPlt.getPointer()) {
			...
			if (img)
				imgPlt = img->getPalette();
		}

		if (setType != OVER_FRAMEID)
			++it;
	}
	...
	if (TToonzImageP ti = img) {
		...
		// raster -> vector
			...
			std::auto_ptr<VectorizerConfiguration> config(
				scene->getProperties()->getVectorizerParameters()->getCurrentConfiguration(0.0));
