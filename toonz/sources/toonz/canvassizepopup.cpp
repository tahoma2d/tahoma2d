...
public:
		...
		: TUndo(), m_level(level), m_fid(fid), m_oldDim(oldDim), m_newDim(newDim)
	{
		...
		if (ti) {
			...
			assert(0);
		int oldSize = oldRaster->getLx() * oldRaster->getLy() * oldRaster->getPixelSize();
		int newSize = newRaster->getLx() * newRaster->getLy() * newRaster->getPixelSize();
...
double getMeasuredLength(int pixelLength, TMeasure *measure, double dpi, const QString &unit)
{
	if (unit == "pixel")
		return pixelLength;
...
int getPixelLength(double measuredLength, TMeasure *measure, double dpi, const QString &unit)
{
	if (unit == "pixel")
		return tround(measuredLength);
...
void PeggingWidget::createButton(QPushButton **button, PeggingPositions position)
{
	*button = new QPushButton(this);
	...
	switch (position) {
	case e00:
		pix = m_topRightPix.transformed(QMatrix().rotate(-90), Qt::SmoothTransformation);
		break;
	case e01:
		pix = m_topPix;
		break;
	...
	case e12:
		...
		break;
	case e20:
		pix = m_topRightPix.transformed(QMatrix().rotate(180), Qt::SmoothTransformation);
		break;
	case e21:
...
void CanvasSizePopup::showEvent(QShowEvent *e)
{
	...
	double dimLx = getMeasuredLength(dim.lx, m_xMeasure, dpi.x, m_unit->currentText());
	double dimLy = getMeasuredLength(dim.ly, m_yMeasure, dpi.y, m_unit->currentText());
...
void CanvasSizePopup::onOkBtn()
{
	...
	TPointD dpi = m_sl->getDpi();
	QString unit = m_unit->currentText();
	int newXValue = getPixelLength(m_xSizeFld->getValue(), m_xMeasure, dpi.x, unit);
	int newYValue = getPixelLength(m_ySizeFld->getValue(), m_yMeasure, dpi.y, unit);
	...
	TPoint pos;
	if (pegPos == e00 || pegPos == e10 || pegPos == e20)
		pos.x = 0;
	...
	if (pegPos == e20 || pegPos == e21 || pegPos == e22)
		pos.y = 0;
