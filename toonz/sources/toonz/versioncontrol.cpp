...
bool getFrameRange(const TFilePath &path, unsigned int &from, unsigned int &to)
{
	// Set frame range
	if (path.getDots() == "..") {
...
void VersionControlThread::run()
{
	forever
	{
		...
		if (!userName.isEmpty() || !password.isEmpty()) {
			args << QString("--username") << userName << QString("--password") << password;
		}
		...
		env << "LC_MESSAGES=en_EN"; // Add this environment variables to fix the language to English
		...
		if (m_getStatus) {
			emit statusRetrieved(QString::fromUtf8(m_process->readAllStandardOutput().data()));
			m_getStatus = false;
		} else
...
void VersionControlManager::setFrameRange(TLevelSet *levelSet, bool deleteLater)
{
	if (levelSet->getLevelCount() == 0)
		return;
	...
	if (!m_isRunning) {
		...
		for (int i = 0; i < levelSet->getLevelCount(); i++) {
			...
			if (sl && !checkVersionControl) {
				TFilePath parentDir = sl->getScene()->decodeFilePath(sl->getPath().getParentDir());
				if (VersionControl::instance()->isFolderUnderVersionControl(toQString(parentDir)))
...
void VersionControl::showFrameRangeLockInfo(QWidget *parent, const QString &workingDir,
											const QString &file)
{
	SVNFrameRangeLockInfoDialog *dialog = new SVNFrameRangeLockInfoDialog(parent, workingDir, file);
	dialog->show();
	dialog->raise();
}
											 ...
											 const QString &sceneFileName)
{
	...
	TFilePath scenePath = TFilePath(wokingDir.toStdWString()) + sceneFileName.toStdWString();
	...
	for (it = levels.begin(); it != levels.end(); ++it) {
		...
		if (levelPath.getDots() == "..") {
			...
			QDir qDir(QString::fromStdWString(dir.getWideString()));
			QString levelName = QRegExp::escape(QString::fromStdWString(levelPath.getWideName()));
