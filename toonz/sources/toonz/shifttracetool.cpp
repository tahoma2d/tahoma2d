...
public:
	...
	void drawDot(const TPointD &center, double r, const TPixel32 &color = TPixel32::White);
		...
		if (sl) {
			...
			if (img) {
				if (TRasterImageP ri = img) {
					...
					m_box = (convert(ras->getBounds()) - ras->getCenterD()) * ri->getSubsampling();
				} else if (TToonzImageP ti = img) {
					TRasterP ras = ti->getRaster();
					m_box = (convert(ras->getBounds()) - ras->getCenterD()) * ti->getSubsampling();
...
void ShiftTraceTool::drawDot(const TPointD &center, double r, const TPixel32 &color)
{
	tglColor(color);
...
{
	if (m_ghostIndex < 0 || m_ghostIndex > 1)
		return;
	int row = m_row[m_ghostIndex];
	if (row < 0)
		return;
	...
	if (TRasterImageP ri = img) {
		TRasterP ras = ri->getRaster();
		box = (convert(ras->getBounds()) - ras->getCenterD()) * ri->getSubsampling();
	} else if (TToonzImageP ti = img) {
		TRasterP ras = ti->getRaster();
		box = (convert(ras->getBounds()) - ras->getCenterD()) * ti->getSubsampling();
	} else if (TVectorImageP vi = img) {
		box = vi->getBBox();
	} else {
		return;
	...
	glVertex2d(box.x0, box.y0);
	glEnd();
	color = m_highlightedGadget == 2000 ? TPixel32(200, 100, 100) : TPixel32::White;
	...
	if (m_curveStatus == NoCurve) {
		color = m_highlightedGadget == 2001 ? TPixel32(200, 100, 100) : TPixel32::White;
...
void ShiftTraceTool::drawCurve()
{
	...
	if (m_curveStatus == TwoPointsCurve) {
		TPixel32 color = m_highlightedGadget == 1000 ? TPixel32(200, 100, 100) : TPixel32::White;
		...
		TPixel32 color = m_highlightedGadget == 1000 ? TPixel32(200, 100, 100) : TPixel32::White;
		drawDot(m_p0, r, color);
		color = m_highlightedGadget == 1001 ? TPixel32(200, 100, 100) : TPixel32::White;
		...
		color = m_highlightedGadget == 1002 ? TPixel32(200, 100, 100) : TPixel32::White;
...
ShiftTraceTool::GadgetId ShiftTraceTool::getGadget(const TPointD &p)
{
	...
	if (0 <= m_ghostIndex && m_ghostIndex < 2) {
		...
		gadgets.push_back(std::make_pair(aff * m_center[m_ghostIndex], MoveCenterGadget));
	}
	int k = -1;
