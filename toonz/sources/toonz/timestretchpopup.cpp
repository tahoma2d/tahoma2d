...
void timeStretch(int newRange, TimeStretchPopup::STRETCH_TYPE type)
{
	...
	if (type == TimeStretchPopup::eRegion) {
		...
		int c;
		for (c = c1; c >= c0; c--)
			if (!xsheet->getColumn(c) && !xsheet->getColumn(c)->isEmpty())
				break;
	}
	assert(r1 >= r0 && c1 >= c0);
	if (r1 - r0 + 1 == newRange)
	...
	: Dialog(TApp::instance()->getMainWindow(), true, true, "TimeStretch"), m_currentStretchType(eRegion)
{
	...
	viewType << tr("Selected Cells") << tr("Selected Frame Range") << tr("Whole Xsheet");
	m_stretchType->addItems(viewType);
	connect(m_stretchType, SIGNAL(currentIndexChanged(int)), SLOT(setCurrentStretchType(int)));
...
void TimeStretchPopup::showEvent(QShowEvent *)
{
	...
	ret = ret && connect(xsheetHandle, SIGNAL(xsheetChanged()), this, SLOT(updateValues()));
	assert(ret);
...
void TimeStretchPopup::hideEvent(QHideEvent *e)
{
	...
	ret = ret && connect(selectionHandle, SIGNAL(selectionSwitched(TSelection *, TSelection *)),
						 this, SLOT(updateValues(TSelection *, TSelection *)));
	TXsheetHandle *xsheetHandle = TApp::instance()->getCurrentXsheet();
	ret = ret && disconnect(xsheetHandle, SIGNAL(xsheetChanged()), this, SLOT(updateValues()));
	assert(ret);
	Dialog::hideEvent(e);
}
//-------------------------------------------------------------------
...
{
	if (m_currentStretchType == STRETCH_TYPE(index))
		return;
