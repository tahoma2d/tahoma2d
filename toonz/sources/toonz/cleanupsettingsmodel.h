...
/*!
  In Toonz, the interface to cleanup settings follows the model/view architecture,
  where each Cleanup Settings panel is a view to a singleton model of this class.
\n\n
  The CleanupSettingsModel is responsible for handling signals that involve cleanup
  changes, such as user changes in the parameters coming from views, settings reset or
  switches from commands, and so on.
\n\n
  A cleanup operation is roughly split in 2 parts: main processing and post-processing.
\n\n
  The main processing part takes the full original image and applies geometric transforms
  and other full-image analysis and processing. This is typically slow, and is thus
...
class CleanupSettingsModel : public QObject
{
	...
	CleanupParameters m_backupParams; //!< Stores a copy of current configuration.
	...
	TFilePath m_clnPath;   //!< Path of the cln file associated with current level (if any)
...
public:
	...
	//! Prompts to save current settings in case they have been modified. Returns false if the
...
signals:
	...
	//! Emitted when the data retrievable with getPreviewData() and getCameraTestData()
	//! have been updated.
	void previewDataChanged();

	//! Emitted when the model loads (or unloads) a cln settings file. It is also invoked
	//! when a cln is saved, since saving should put us in the same situation we have after
