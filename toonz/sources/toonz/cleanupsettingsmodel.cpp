	...
	: m_listenersCount(0), m_previewersCount(0), m_cameraTestsCount(0), m_allowedActions(FULLPROCESS), m_action(NONE), m_c(-1), m_sl(0)
{
	...
	bool ret = true;
	ret = ret && connect(sceneHandle, SIGNAL(sceneSwitched()), SLOT(onSceneSwitched()));
	assert(ret);
...
void CleanupSettingsModel::getCleanupFrame(TXshSimpleLevel *&sl, TFrameId &fid)
{
...
{
	if ((objectFlag & LISTENER) && (m_listenersCount++ <= 0))
		connectSignals();
...
void CleanupSettingsModel::commitChanges()
{
	...
	// Analyze param changes
	{
		...
		if (currentParams->m_path != m_backupParams.m_path)
			action = INTERFACE;
		...
		if (currentParams->m_lineProcessingMode != m_backupParams.m_lineProcessingMode ||
			...
			currentParams->m_camera.getSize() != m_backupParams.m_camera.getSize() ||
...
void CleanupSettingsModel::commitChanges(int action)
{
	...
	if (action >= INTERFACE)
		emit modelChanged(action == POSTPROCESS);
}
...
void CleanupSettingsModel::rebuildPreview()
{
	...
	if (m_previewersCount <= 0 && m_cameraTestsCount <= 0)
		return;
...
void CleanupSettingsModel::processFrame(TXshSimpleLevel *sl, TFrameId fid)
{
	...
	bool doProcessing = (params->m_lineProcessingMode != lpNone) && (m_previewersCount > 0);
...
{
	// When the scene switches, current parameters are reverted the scene's originals.
	// So, since we store originals in CleanupParameters::GlobalParameters, we copy them there.
	CleanupParameters *params = getCurrentParameters();
	CleanupParameters::GlobalParameters.assign(params);

	// The cleanupper always uses current cleanup parameters. It has to be specified somewhere,
...
void CleanupSettingsModel::onCellChanged()
{
	...
	if (sl != m_sl) {
		...
		if (m_clnPath != clnPath) {
			...
			if (loadSettings(clnPath)) // modelChanged() emitted here, plust preview rebuild
			{
				m_clnPath = clnPath;
				rebuiltPreview = true;
...
void CleanupSettingsModel::onPaletteChanged()
{
	commitChanges(POSTPROCESS);
}
...
void CleanupSettingsModel::saveSettings(CleanupParameters *params, const TFilePath &clnPath)
{
	if (clnPath.isEmpty() || !params)
		return;
...
bool CleanupSettingsModel::loadSettings(CleanupParameters *params, const TFilePath &clnPath)
{
	assert(params);
...
void CleanupSettingsModel::promptLoad()
{
	...
	if (!loadPopup)
		loadPopup = new LoadSettingsPopup;
...
TFilePath CleanupSettingsModel::getOutputPath(TXshSimpleLevel *sl, const CleanupParameters *params)
{
	const TFilePath &inPath = getInputPath(sl); // Used just to get level name

	if (inPath.isEmpty() || !params)
		return TFilePath();
