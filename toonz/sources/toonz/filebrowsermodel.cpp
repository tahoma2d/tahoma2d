...
TFilePath getMyDocumentsPath()
{
	...
	return TFilePath((const char *)[documentsDirectory cStringUsingEncoding:NSASCIIStringEncoding]);
#else
	return TFilePath();
#endif
}
...
DvDirModelNode::DvDirModelNode(DvDirModelNode *parent, wstring name)
...
QPixmap DvDirModelNode::getPixmap(bool isOpen) const
{
	return QPixmap();
}
...
void DvDirModelNode::setTemporaryName(const wstring &newName)
{
	m_oldName = getName();
	m_name = newName;
}
...
DvDirModelFileFolderNode::DvDirModelFileFolderNode(DvDirModelNode *parent, wstring name, const TFilePath &path)
...
DvDirModelNode *DvDirModelFileFolderNode::makeChild(wstring name)
{
...
bool DvDirModelFileFolderNode::setName(wstring newName)
{
	if (isSpaceString(QString::fromStdWString(newName)))
		return true;
...
bool DvDirModelFileFolderNode::hasChildren()
{
	...
	if (m_peeks) {
		// Using QDir directly rather than DvDirModelFileFolderNode::refreshChildren() due to
		// performance issues
		...
		return true; // Not peeking nodes allow users to actively scan for sub-folders
}
...
void DvDirModelFileFolderNode::refreshChildren()
{
	...
	for (i = 0; i < (int)names.size(); i++) {
		...
		for (j = oldChildren.begin(); j != oldChildren.end() && (*j)->getName() != name; ++j) {
		}
...
void DvDirModelFileFolderNode::getChildrenNames(std::vector<wstring> &names) const
{
	TFileStatus folderPathStatus(m_path);
	if (folderPathStatus.isLink())
		return;
	...
	for (e = 0; e != eCount; ++e)
		names.push_back(entries[e].toStdWString());
}

//-----------------------------------------------------------------------------

int DvDirModelFileFolderNode::rowByName(const wstring &name) const
{
	std::vector<wstring> names;
	getChildrenNames(names);
	std::vector<wstring>::iterator it = std::find(names.begin(), names.end(), name);
	if (it == names.end())
		return -1;
	else
...
DvDirModelSceneFolderNode::DvDirModelSceneFolderNode(DvDirModelNode *parent, wstring name, const TFilePath &scenePath)
...
DvDirModelSceneFolderNode::DvDirModelSceneFolderNode(DvDirModelNode *parent, const TFilePath &path)
	: DvDirModelFileFolderNode(parent, path.withoutParentDir().getWideString(), path)
{
	try {
		ToonzScene scene;
...
bool DvDirModelSceneFolderNode::setName(wstring newName)
{
	m_name = newName;
	return true;
}
...
DvDirModelNode *DvDirModelSceneFolderNode::makeChild(wstring name)
{
	TFilePath actualFolderPath = m_folders[name];
	DvDirModelFileFolderNode *node = new DvDirModelFileFolderNode(this, name, actualFolderPath);
...
void DvDirModelSceneFolderNode::getChildrenNames(std::vector<wstring> &names) const
{
	std::map<wstring, TFilePath>::const_iterator it;
...
int DvDirModelSceneFolderNode::rowByName(const wstring &name)
{
	std::vector<wstring> names;
	getChildrenNames(names);
	std::vector<wstring>::iterator it = std::find(names.begin(), names.end(), name);
...
DvDirModelSpecialFileFolderNode::DvDirModelSpecialFileFolderNode(
	DvDirModelNode *parent, wstring name, const TFilePath &path)
...
DvDirVersionControlNode::DvDirVersionControlNode(DvDirModelNode *parent, wstring name, const TFilePath &path)
...
QList<TFilePath> DvDirVersionControlNode::getMissingFiles() const
{
	...
	while (i != m_statusMap.constEnd()) {
		...
		if (s.m_item == "missing" || s.m_item == "none" && s.m_repoStatus == "added") {
			...
			if (dots != "") {
				if (dots == "..")
					path = path.getParentDir() + path.getLevelNameW();
...
QStringList DvDirVersionControlNode::getMissingFiles(const QRegExp &filter) const
{
	QStringList list;
	QMap<QString, SVNStatus>::const_iterator i = m_statusMap.constBegin();
	...
	while (i != m_statusMap.constEnd()) {
		...
		if (s.m_item == "missing" || (s.m_item == "none" && s.m_repoStatus == "added")) {
			...
			if (path.getDots() == "" || path.getType() == "tnz") {
				QString currentFolderName = QString::fromStdWString(getPath().getWideName());
...
void DvDirVersionControlNode::getChildrenNames(std::vector<wstring> &names) const
{
...
DvDirModelNode *DvDirVersionControlNode::makeChild(wstring name)
{
...
void DvDirVersionControlNode::insertVersionControlStatus(const QString &fileName, SVNStatus status)
{
...
SVNStatus DvDirVersionControlNode::getVersionControlStatus(const QString &fileName)
{
	if (m_statusMap.contains(fileName))
		return m_statusMap.value(fileName);
...
QStringList DvDirVersionControlNode::refreshVersionControl(const QList<SVNStatus> &status)
{
	...
	for (int i = 0; i < listSize; i++) {
		...
		if (nodePath.getType() == "")
			insertVersionControlStatus(fileName, s);
		...
		if (type == "tlv" || type == "pli") {
			if (s.m_item != "unversioned" && s.m_item != "missing" && s.m_repoStatus != "added")
...
DvDirVersionControlRootNode *DvDirVersionControlNode::getVersionControlRootNode()
{
	DvDirModelNode *parent = getParent();
	if (parent)
		return parent->getVersionControlRootNode();
	return 0;
}
...
void DvDirVersionControlProjectNode::getChildrenNames(std::vector<wstring> &names) const
{
...
DvDirModelProjectNode::DvDirModelProjectNode(DvDirModelNode *parent, const TFilePath &path)
...
void DvDirModelProjectNode::getChildrenNames(std::vector<wstring> &names) const
{
	DvDirModelFileFolderNode::getChildrenNames(names);
	TProjectManager *pm = TProjectManager::instance();
	TProject *project = new TProject();
...
DvDirModelNode *DvDirModelProjectNode::makeChild(wstring name)
{
	if (name != L"" && name[0] == L'+') {
		TProjectManager *pm = TProjectManager::instance();
		TProject *project = new TProject();
		project->load(getProjectPath());
		string folderName = toString(name.substr(1));
...
DvDirModelDayNode::DvDirModelDayNode(DvDirModelNode *parent, wstring dayDateString)
...
void DvDirModelMyComputerNode::refreshChildren()
{
	...
	for (it = fps.begin(); it != fps.end(); ++it) {
		DvDirModelNode *child = new DvDirModelFileFolderNode(this, it->getWideString(), *it);
		addChild(child);
	}
}
...
void DvDirModelNetworkNode::refreshChildren()
{
	...
	HANDLE enumHandle;
	DWORD err = WNetOpenEnum(RESOURCE_CONTEXT, RESOURCETYPE_DISK, 0, NULL, &enumHandle);
	assert(err == NO_ERROR);

	DWORD count = -1, bufferSize = 16 << 10; // Use 16 kB as recommended on Windows references
	...
	do {
		...
		if (err == NO_ERROR) {
			for (int i = 0; i < count; ++i) {
				// Only list disk-type devices - in any case, the remote (UNC) name should exist
...
DvDirModelRootNode::DvDirModelRootNode()
	: DvDirModelNode(0, L"Root"), m_myComputerNode(0), m_networkNode(0), m_sandboxProjectNode(0)
{
	m_nodeType = "Root";
}
...
void DvDirModelRootNode::add(wstring name, const TFilePath &path)
{
...
void DvDirModelRootNode::refreshChildren()
{
	...
	if (m_children.empty()) {
		...
		for (i = 0; i < (int)projectRoots.size(); i++) {
			...
			wstring roothDir = projectRoot.getWideString();
...
void DvDirModel::onFolderChanged(const TFilePath &path)
{
	refreshFolder(path);
}
...
void DvDirModel::refreshFolder(const TFilePath &folderPath, const QModelIndex &i)
{
	...
	if (node->isFolder(folderPath))
		...
		int r;
		for (r = 0; r < count; r++)
			refreshFolder(folderPath, index(r, 0, i));
	}
}
...
void DvDirModel::refreshFolderChild(const QModelIndex &i)
{
	...
	int r;
	for (r = 0; r < count; r++)
		refreshFolderChild(index(r, 0, i));
}
...
QModelIndex DvDirModel::index(int row, int column, const QModelIndex &parent) const
{
	if (column != 0)
		return QModelIndex();
...
QModelIndex DvDirModel::childByName(const QModelIndex &parent, const wstring &name) const
{
	if (!parent.isValid())
		return QModelIndex();
...
*/
bool DvDirModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
	if (!index.isValid())
		return false;
	DvDirModelNode *node = getNode(index);
...
bool DvDirModel::removeRows(int row, int count, const QModelIndex &parentIndex)
{
...
{
	return getIndexByPath(TProjectManager::instance()->getCurrentProjectPath().getParentDir());
}
