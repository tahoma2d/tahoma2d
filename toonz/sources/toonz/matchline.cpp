...
void doMatchlines(const vector<MergeCmappedPair> &matchingLevels, int inkIndex, int inkPrevalence)
{
	...
	TPalette *matchPalette = matchingLevels[0].m_mcell->getImage(false)->getPalette();
	...
	for (i = 0; i < (int)matchingLevels.size(); i++) {
		...
		TToonzImageP match = (TToonzImageP)matchingLevels[i].m_mcell->getImage(false);
		if (!img || !match)
		...
		TRop::applyMatchLines(r, matchRas, palette, inkIndex, inkPrevalence, usedInks);
	...
	if (usedInks.size() > 0) {
		...
		TApp::instance()->getPaletteController()->getCurrentPalette()->notifyPaletteChanged();
	}
}
...
int MatchlinesDialog::exec(TPalette *plt)
{
	...
	if (styleIndex > plt->getStyleCount())
		styleIndex = 1;

	if (styleIndex != -1)
		m_inkIndex->setText(QString::number(styleIndex));
...
int MatchlinesDialog::getInkPrevalence()
{
	return m_inkPrevalence->getValue();
}
	...
	: Dialog(TApp::instance()->getMainWindow(), true, true, "Matchlines"), m_pltHandle(new TPaletteHandle()), m_currentXsheet(0)
{
	...
	QIcon lup_noGapIcon(QPixmap(":Resources/match_lup_nogap.png").scaled(104, 104));
	QIcon rup_noGapIcon(QPixmap(":Resources/match_rup_nogap.png").scaled(104, 104));
	...
	connect(m_button2, SIGNAL(toggled(bool)), this, SLOT(onChooseInkClicked(bool)));
	connect(okBtn, SIGNAL(clicked()), this, SLOT(accept()));
	connect(cancelBtn, SIGNAL(clicked()), this, SLOT(reject()));
	connect(lineStackButtonGroup, SIGNAL(buttonPressed(int)), SLOT(onLineStackButtonPressed(int)));
	connect(m_inkPrevalence, SIGNAL(valueChanged(bool)), SLOT(onInkPrevalenceChanged(bool)));
}
...
void doMatchlines(int column, int mColumn, int index, int inkPrevalence, int MergeCmappedSessionId)
{
...
QString indexes2string(const set<TFrameId> fids)
{
	...
	while (it != fids.end()) {
		...
		if (lastVal != it->getNumber())
			str += "-" + QString::number(lastVal);
		if (it1 == fids.end())
			return str;
...
void DeleteInkDialog::setRange(const QString &str)
{
	m_frames->setText(str);
}
...
void doDeleteMatchlines(TXshSimpleLevel *sl, const set<TFrameId> &fids, bool chooseInk)
{
	...
	if (chooseInk) {
		...
		if (LastMatchlineIndex == -1)
			LastMatchlineIndex = ph->getStyleIndex();
