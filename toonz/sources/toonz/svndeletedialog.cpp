	...
	: Dialog(TApp::instance()->getMainWindow(), true, false), m_deleteSceneContentsCheckBox(0), m_workingDir(workingDir), m_files(files), m_targetTempFile(0), m_isFolder(isFolder), m_sceneIconAdded(sceneIconAdded)
{
	...
	if (m_isFolder) {
		...
		m_textLabel->setText(tr("Delete %1 items.").arg(m_files.size() - m_sceneIconAdded));
	...
	connect(m_deleteLocalButton, SIGNAL(clicked()), this, SLOT(onDeleteLocalButtonClicked()));

	m_deleteServerButton = new QPushButton();
	m_deleteServerButton->setText(tr("Delete on Server "));
	connect(m_deleteServerButton, SIGNAL(clicked()), this, SLOT(onDeleteServerButtonClicked()));
...
{
	if (m_deleteSceneContentsCheckBox)
		m_deleteSceneContentsCheckBox->hide();
	m_commentTextEdit->hide();
	m_commentLabel->hide();
	m_waitingLabel->hide();
...
void SVNDeleteDialog::onDeleteServerButtonClicked()
{
	...
	disconnect(m_deleteServerButton, SIGNAL(clicked()), this, SLOT(onDeleteServerButtonClicked()));
	m_deleteServerButton->setText(tr("Delete"));
	connect(m_deleteServerButton, SIGNAL(clicked()), this, SLOT(deleteFiles()));

	m_textLabel->setText(tr("You are deleting items also on repository. Are you sure ?"));
	m_commentLabel->show();
...
void SVNDeleteDialog::deleteFiles()
{
	...
	if (m_keepLocalCopyCheckBox->isChecked())
		args << "--keep-local";
...
void SVNDeleteDialog::commitDeletedFiles()
{
	...
	if (!m_commentTextEdit->toPlainText().isEmpty())
		...
		args << QString("-m").append(VersionControl::instance()->getUserName() + " delete files.");
	m_thread.disconnect(SIGNAL(done(const QString &)));
