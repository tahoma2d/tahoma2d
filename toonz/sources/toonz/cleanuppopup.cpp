...
enum Resolution {
	...
	OVERWRITE,	 //!< Does not delete old cleanupped levels, but overwrites found frames.
	WRITE_NEW,	 //!< Like above, but does not overwrite. Just adds not cleanupped frames.
...
inline QString suffix(int num)
{
	return QString("_") + QString::number(num);
}
...
void loadCleanupParams(CleanupParameters *params, TXshSimpleLevel *sl)
{
	...
	CleanupSettingsModel::loadSettings(params, CleanupSettingsModel::getClnPath(sl));
}
...
{
	if (abs(viewSize.width() - imageSize.width()) > abs(viewSize.height() - imageSize.height())) {
						...
						bool keepOriginalPalette)
{
	try {
		...
		if (keepOriginalPalette)
			lw->setOverwritePaletteFlag(false);
	...
	: QDialog(TApp::instance()->getMainWindow()), m_params(new CleanupParameters), m_updater(new LevelUpdater), m_originalLevelPath(), m_originalPalette(0)
{
	...
	bool ret = true;
	ret = ret && connect(m_progressBar, SIGNAL(valueChanged(int)), this, SLOT(onValueChanged(int)));

	// NOTE: On MAC it seems that QAbstractButton's pressed() signal is reemitted at
	// every mouseMoveEvent when the button is pressed...
	// This is why clicked() substitutes pressed() below.

	ret = ret && connect(m_cleanupButton, SIGNAL(clicked()), this, SLOT(onCleanupFrame()));
...
void CleanupPopup::closeEvent(QCloseEvent *ce)
{
	reset();
}
...
void CleanupPopup::reset()
{
	...
	/*---タイトルバーを元の表示に戻す---*/
	MainWindow *mainWin = qobject_cast<MainWindow *>(TApp::instance()->getMainWindow());
	if (mainWin)
		mainWin->changeWindowTitle();
}
...
void CleanupPopup::buildCleanupList()
{
	...
	std::map<TXshSimpleLevel *, FramesList> cleanupList; // List of frames to be cleanupped
	std::vector<TXshSimpleLevel *> levelsList;			 // List of levels in the cleanup list,
	...
	if (selection) {
		...
		for (c = c0; c <= c1; ++c) {
			for (r = r0; r <= r1; ++r) {
				...
				if (!(sl && locals::supportsCleanup(sl)))
					continue;
				/*---もし新しいLevelなら、Levelのリストに登録---*/
				std::map<TXshSimpleLevel *, FramesList>::iterator it = cleanupList.find(sl);
	...
	/*--- カラム選択の場合 ---*/
		...
		if (columnSel->isEmpty() || frameCount <= 0)
			return;
		/*--- 選択された各カラムについて ---*/
...
bool CleanupPopup::analyzeCleanupList()
{
	...
	*/
	...
	for (clt = m_cleanupLevels.begin(); clt != clEnd; ++clt) {
		...
		loadCleanupParams(m_params.get(), sl); // Needed to retrieve output level resolution
...
{
	if (pos.first < 0 || int(m_cleanupLevels.size()) <= pos.first)
		return false;

	const CleanupLevel &cl = m_cleanupLevels[pos.first];

	if (pos.second < 0 || int(cl.m_frames.size()) <= pos.second)
		return false;
...
void CleanupPopup::execute()
{
	...
	// In case some cleaned up level already exists, let the user decide what to do
	...
	if (image) {
		...
		// Set the zoom factor depending on image and viewer sizes, so that the image fits
		// the preview area.
		...
		TPointD delta(0, 0);
		TAffine viewAff = TTranslation(delta) * TScale(factor) * TTranslation(-delta);
...
QString CleanupPopup::setupLevel()
{
	...
	// Frames are cleaned-up at full-sampling. Thus, clear subsampling on the level.
	...
	if (lineProcessing) {
		...
		TPaletteP palette = TCleanupper::instance()->createToonzPaletteFromCleanupPalette();
	...
	} else if (!m_params->getPath(scene).isEmpty()) // Should never be empty, AFAIK...
	{
		...
		if (notLineProcessed) {
			// Just change paths
			if (sl->getScannedPath().isEmpty())
				sl->setScannedPath(sl->getPath());
...
QString CleanupPopup::resetLevel()
{
	...
	const TFilePath &outputPath = scene->decodeFilePath(cl.m_outputPath),
					&inputPath = scene->decodeFilePath(CleanupSettingsModel::getInputPath(sl));

	if (inputPath == outputPath)
		return QString(); // Cannot remove source file - however, it can be overwritten
...
void CleanupPopup::closeLevel()
{
	...
	// Close the level updater. Silence any exception (ok, something went bad, old story now).
	try {
		m_updater->close();
	} catch (...) {
	}
...
void CleanupPopup::cleanupFrame()
{
	...
	// Perform image cleanup
	{
		...
		if (params->m_lineProcessingMode == lpNone) {
			...
			cl->setCustomDpi((dpix == 0 && dpiy == 0) ? sl->getProperties()->getDpi() : TPointD());
			...
			sl->setFrameStatus(fid, sl->getFrameStatus(fid) | TXshSimpleLevel::Cleanupped);
...
void CleanupPopup::onCancelCleanup()
{
	close();
}
	...
	: DVGui::ValidatedChoiceDialog(TApp::instance()->getMainWindow())
{
	...
	bool ret = connect(m_buttonGroup, SIGNAL(buttonClicked(int)), SLOT(onButtonClicked(int)));
	assert(ret);
	...
	radioButton->setText(tr("This is Re-Cleanup. Overwrite only to the no-paint files."));
	radioButton->setFixedHeight(20);
...
void CleanupPopup::updateTitleString()
{
	...
	QString str = QString::number(m_completion.first) + "/" + QString::number(m_completion.second) +
