...
bool TColumnSelection::isEmpty() const
{
...
{
	ColumnCmd::pasteColumns(m_indices);
}
...
void TColumnSelection::selectColumn(int col, bool on)
{
	...
	for (++it; it != m_indices.end(); ++it)
		if (!canMergeColumns(firstCol, *it, false))
			break;
	...
	it = m_indices.begin();
	for (++it; it != m_indices.end(); ++it)
		if (!canMergeColumns(firstCol, *it, true))
			break;
...
void TColumnSelection::selectNone()
{
	m_indices.clear();
}
...
{
	if (m_indices.size() == 1)
		ColumnCmd::resequence(*m_indices.begin());
}
...
{
	if (m_indices.size() == 1)
		ColumnCmd::cloneChild(*m_indices.begin());
}
