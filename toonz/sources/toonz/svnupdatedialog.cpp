	...
	: Dialog(TApp::instance()->getMainWindow(), true, false), m_updateSceneContentsCheckBox(0), m_workingDir(workingDir), m_files(files), m_updateToRevision(updateToRevision), m_nonRecursive(nonRecursive), m_sceneIconsCount(sceneIconsCount), m_someSceneIsMissing(false)
{
	...
	if (m_updateToRevision)
		connect(m_updateButton, SIGNAL(clicked()), this, SLOT(onUpdateToRevisionButtonClicked()));
	else
...
void SVNUpdateDialog::onStatusRetrieved(const QString &xmlResponse)
{
	...
	if (m_updateToRevision) {
		...
		if (m_filesWithConflict.count() > 0) {
			...
			adjustSize();
			connect(m_conflictWidget, SIGNAL(allConflictSetted()), this, SLOT(onConflictSetted()));
		} else
			updateFiles();
	}
}
...
void SVNUpdateDialog::onUpdateButtonClicked()
{
	...
	if (files.size() > 0) {
		...
		m_thread.disconnect(SIGNAL(done(const QString &)));
		connect(&m_thread, SIGNAL(done(const QString &)), SLOT(onUpdateToMineDone()));
...
void SVNUpdateDialog::onUpdateToMineDone()
{
	...
	if (files.size() > 0) {
		...
		m_thread.disconnect(SIGNAL(done(const QString &)));
		connect(&m_thread, SIGNAL(done(const QString &)), SLOT(onConflictResolved()));
		m_thread.executeCommand(m_workingDir, "svn", args);
	} else
		onConflictResolved();
}
