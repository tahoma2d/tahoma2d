...
void CObjectTracker::ObjeckTrackerHandlerByUser(TRaster32P *frame)
{
	...
	if (m_sTrackingObject.Status) {
		if (!m_sTrackingObject.assignedAnObject) {
			...
			output<<m_sTrackingObject.half_pixelx <<" "<<m_sTrackingObject.half_pixely<<endl;
			if (m_visible=="")
			{
				output<<"VISIBLE"<<endl;
			}
...
UBYTE8 CObjectTracker::CheckEdgeExistance(TRaster32P *frame, short _x, short _y)
{
	...
	if (abs((GrayCenter - GrayLeft) / 10) > EDGE_DETECT_THRESHOLD)
		E = 1;

	if (abs((GrayCenter - GrayRight) / 10) > EDGE_DETECT_THRESHOLD)
		E = 1;

	if (abs((GrayCenter - GrayUp) / 10) > EDGE_DETECT_THRESHOLD)
		E = 1;

	if (abs((GrayCenter - GrayDown) / 10) > EDGE_DETECT_THRESHOLD)
		E = 1;
...
float CObjectTracker::Matching(TRaster32P *frame, TRaster32P *frame_temp)
{
	...
	if ((ok_u > 0) && (ok_v > 0)) {
		...
		for (int i = dimy_int; i <= ((dimx_int - 1) * dimy_int - 1); i += 2) {
			...
			pixel_temp[i].g = (pixel_temp[i - dimy_int].g + pixel_temp[i + dimy_int].g) / 2;
			pixel_temp[i].b = (pixel_temp[i - dimy_int].b + pixel_temp[i + dimy_int].b) / 2;

			if (i % dimy_int == dimy_int - 1)
				i += dimy_int - 1;
		}
		...
		if (track) {
			...
			if (u_sup % 2 != 0)
				if (v_sup % 2 != 0) {
					...
					if (mat_dist[(u_sup - 2 * u_min - 1) * (2 * ok_v - 1) + (v_sup - 2 * v_min)] <
						mat_dist[(u_sup - 2 * u_min + 1) * (2 * ok_v - 1) + (v_sup - 2 * v_min)]) {
						m_sTrackingObject.half_pixelx = -1;
						m_sTrackingObject.half_pixely = 0;
						u_sup -= 1;
					} else {
				...
				if (mat_dist[(u_sup - 2 * u_min) * (2 * ok_v - 1) + (v_sup - 2 * v_min - 1)] <
					mat_dist[(u_sup - 2 * u_min) * (2 * ok_v - 1) + (v_sup - 2 * v_min + 1)]) {
					m_sTrackingObject.half_pixelx = 0;
					m_sTrackingObject.half_pixely = -1;
					v_sup -= 1;
				} else {
...
void CObjectTracker::SetPositionByNeighbours(void)
{
	if (!track) {
		...
		A[1][0] = 2 * (x[2] - x[0]);
		A[1][1] = 2 * (y[2] - y[0]);
		b[0] = d2[0] - d2[1] + x[1] * x[1] - x[0] * x[0] + y[1] * y[1] - y[0] * y[0];
		b[1] = d2[0] - d2[2] + x[2] * x[2] - x[0] * x[0] + y[2] * y[2] - y[0] * y[0];
		float detA, invA[2][2];
