...
void RenderController::setMovieExt(string ext)
{
	m_movieExt = ext;
}
...
void RenderController::myCancel()
{
	m_cancelled = true;
}
...
void RenderController::generateMovie(TFilePath outPath, bool emitSignal)
{
	...
	if (outPath.getName() == "")
		outPath = outPath.withName("untitled");
...
void RenderController::resetClipList()
{
	m_clipList.clear();
}
...
ClipListViewer::ClipListViewer(QWidget *parent)
	: DvItemViewer(parent), m_dropInsertionPoint(-1), m_loadSceneFromExportPanel(false)
{
	resetList();
	setModel(this);
	setAcceptDrops(true);
...
{
	if (!m_loadSceneFromExportPanel)
		getController()->resetClipList();
}
...
{
	if (getPanel()->getSelection()->isEmpty())
		return;
	int i;
...
QVariant ClipListViewer::getItemData(int index, DataType dataType, bool isSelected)
{
	if (index < 0 || index >= getController()->getClipCount())
		return QVariant();
...
void ClipListViewer::dragMoveEvent(QDragMoveEvent *event)
{
	...
	setDropInsertionPoint(event->pos());
	if (oldDropInsertionPoint != m_dropInsertionPoint)
		update();
}
...
void ClipListViewer::dropEvent(QDropEvent *event)
{
	if (event->mimeData()->hasUrls()) {
		...
		event->setDropAction(Qt::CopyAction);
		event->acceptProposedAction();
	} else if (event->mimeData()->hasFormat(m_mimeFormat) && m_dropInsertionPoint >= 0) {
...
{
	selection->enableCommand(this, MI_Clear, &ClipListViewer::removeSelectedClips);
}
...
void ClipListViewer::loadScene()
{
	...
	if (!IoCmd::saveSceneIfNeeded(tr("Load Scene")))
		return;

	// Nella clip list assegno un path esplicito all'eventuale scena corrente
	// (se la scena corrente non e' stata salvata dall'utente elimino il riferimento)
	ToonzScene *scene = TApp::instance()->getCurrentScene()->getScene();
	TFilePath scenePath;
	if (!scene->isUntitled())
...
#endif
	: TPanel(parent), m_clipListViewer(0), m_saveInFileFld(0), m_fileNameFld(0), m_useMarker(0)
{
...
//ClipList
	...
	m_fileNameFld = new DVGui::LineEdit(QString::fromStdWString(sceneName), settingsBox);
...
// Use Marker checkbox
	...
	if (m_clipListViewer)
		mainLayout->addWidget(m_clipListViewer);
	mainLayout->addWidget(new DVGui::Separator("", this));
	mainLayout->addWidget(settingsBox, 0);
	mainLayout->addWidget(new DVGui::Separator("", this));
	mainLayout->addWidget(exportBox, 0);

	connect(RenderController::instance(), SIGNAL(movieGenerated()), this, SLOT(onMovieGenerated()));
