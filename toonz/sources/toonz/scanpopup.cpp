...
void fillOutputType(QComboBox *t, TScannerParameters *params)
{
	...
	if (params->isSupported(TScannerParameters::BW))
		t->addItem(BlackAndWhite);
	if (params->isSupported(TScannerParameters::GR8))
		t->addItem(Graytones);
	if (params->isSupported(TScannerParameters::RGB24))
		t->addItem(Rgbcolors);
}
...
/*! \class ScanSettingsPopup
		\brief The ScanSettingsPopup class provides a dialog to change scan settings.
...
void ScanSettingsPopup::updateUI()
{
	...
	if (params->m_brightness.m_supported) {
		if (params->m_brightness.m_max > params->m_brightness.m_min) {
			m_brightness->setRange(params->m_brightness.m_min, params->m_brightness.m_max);
...
void MyScannerListener::onNextPaper()
{
	...
	if (TScanner::instance()->m_isTwain)
		MsgBox(INFORMATION, tr("Please, place the next paper drawing on the scanner flatbed, then select the relevant command in the TWAIN interface."));
	else {
		QString question(tr("Please, place the next paper drawing on the scanner flatbed, then click the Scan button."));
		int ret = MsgBox(question, QObject::tr("Scan"), QObject::tr("Cancel"), 0);
		if (ret == 2 || ret == 0)
			m_isCanceled = true;
	}
}
...
bool MyScannerListener::isCanceled()
{
	return m_isCanceled;
}
...
void doScan()
{
	if (!checkScannerDefinition())
		return;
	ScanList scanList;
	if (scanList.areScannedFramesSelected()) {
		int ret = MsgBox(QObject::tr("Some of the selected drawings were already scanned. Do you want to scan them again?"),
...
OpenPopupCommandHandler<DefineScannerPopup> openDefineScannerPopup(MI_DefineScanner);

OpenPopupCommandHandler<ScanSettingsPopup> openScanSettingsPopup(MI_ScanSettings);
...
public:
	...
	void execute()
	{
		...
		if (!cropboxCheck->isEnabled()) {
			if (m_currentTool)
				app->getCurrentTool()->setTool(QString::fromStdString(m_currentTool->getName()));
			return;
		}
...
public:
	...
	void mouseMove(const TPointD &p, const TMouseEvent &e)
	{
		...
		if (TRectD(cropBox.getP00() - size, cropBox.getP00() + size).contains(p))
			m_scaling = e00;
		else if (TRectD(cropBox.getP01() - size, cropBox.getP01() + size).contains(p))
			m_scaling = e01;
		else if (TRectD(cropBox.getP11() - size, cropBox.getP11() + size).contains(p))
			m_scaling = e11;
		else if (TRectD(cropBox.getP10() - size, cropBox.getP10() + size).contains(p))
			m_scaling = e10;
		else if (isCloseToSegment(p, TSegment(cropBox.getP00(), cropBox.getP10()), maxDist))
			m_scaling = eM0;
		else if (isCloseToSegment(p, TSegment(cropBox.getP10(), cropBox.getP11()), maxDist))
			m_scaling = e1M;
		else if (isCloseToSegment(p, TSegment(cropBox.getP11(), cropBox.getP01()), maxDist))
			m_scaling = eM1;
		else if (isCloseToSegment(p, TSegment(cropBox.getP01(), cropBox.getP00()), maxDist))
			m_scaling = e0M;
	...
	void leftButtonDrag(const TPointD &pos, const TMouseEvent &e)
	{
		...
		if (m_scaling != eNone && m_scaling != eMove) {
			...
			if (e.isShiftPressed()) {
				...
				if (m_scaling == e11) {
					if (cropBox.y0 - delta.x < cropBox.y1)
						cropBox.y0 -= delta.x;
					if (cropBox.x0 - delta.y < cropBox.x1)
						cropBox.x0 -= delta.y;
				} else if (m_scaling == e00) {
					if (cropBox.y1 - delta.x > cropBox.y0)
						cropBox.y1 -= delta.x;
					if (cropBox.x1 - delta.y > cropBox.x0)
						cropBox.x1 -= delta.y;
...
private:
	TRectD rect2pix(const TRectD &rect)
	{
		...
		double cbOffsetx = (((rect.x0 - scanAreaCenter.x) * previewDPI) / f) * scaleFactor;
		double cbOffsety = (((rect.y0 - scanAreaCenter.y) * previewDPI) / f) * scaleFactor;
