...
{
	if (m_r0 == 0 && m_c0 == 0 && m_r1 == -1 && m_c1 == -1)
		return;
	TApp *app = TApp::instance();
	for (int j = m_c0; j <= m_c1; j++)
		app->getCurrentXsheet()->getXsheet()->removeCells(m_r1 + 1, j, m_upTo - (m_r1 + 1) + 1);
...
void DuplicateUndo::redo() const
{
	if (m_r0 == 0 && m_c0 == 0 && m_r1 == -1 && m_c1 == -1)
		return;
	TApp *app = TApp::instance();
	app->getCurrentXsheet()->getXsheet()->duplicateCells(m_r0, m_c0, m_r1, m_c1, m_upTo);
	app->getCurrentXsheet()->notifyXsheetChanged();
}
	...
	: QDialog(TApp::instance()->getMainWindow()), m_count(0), m_upTo(0)
{
	...
	bool ret = true;
	ret = ret && connect(m_countFld, SIGNAL(editingFinished()), this, SLOT(updateValues()));
	ret = ret && connect(m_upToFld, SIGNAL(editingFinished()), this, SLOT(updateValues()));
