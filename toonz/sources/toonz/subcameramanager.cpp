...
TPointD PreviewSubCameraManager::winToCamera(SceneViewer *viewer, const QPoint &pos) const
{
...
bool PreviewSubCameraManager::mousePressEvent(SceneViewer *viewer, QMouseEvent *event)
{
	if (viewer->is3DView())
		return true;
...
bool PreviewSubCameraManager::mouseMoveEvent(SceneViewer *viewer, QMouseEvent *event)
{
	if (viewer->is3DView())
		return true;
...
bool PreviewSubCameraManager::mouseReleaseEvent(SceneViewer *viewer, QMouseEvent *event)
{
	if (viewer->is3DView())
		return true;
...
UCHAR PreviewSubCameraManager::getSubCameraDragEnum(SceneViewer *viewer, const QPoint &mousePos)
{
	TCamera *camera = TApp::instance()->getCurrentScene()->getScene()->getCurrentCamera();
	TRect subCamera = camera->getInterestRect();

	if (subCamera.getLx() <= 0 || subCamera.getLy() <= 0)
		return NODRAG;
	...
	if (cameraPosBox.y0 < subCameraD.y1)
		dragType |= INNER_TOP;
	if (cameraPosBox.y1 > subCameraD.y0)
		dragType |= INNER_BOTTOM;
	if (cameraPosBox.x0 < subCameraD.x1)
		dragType |= INNER_RIGHT;
	if (cameraPosBox.x1 > subCameraD.x0)
		dragType |= INNER_LEFT;

	if (cameraPosBox.y1 > subCameraD.y1)
		dragType |= OUTER_TOP;
	if (cameraPosBox.y0 < subCameraD.y0)
		dragType |= OUTER_BOTTOM;
	if (cameraPosBox.x1 > subCameraD.x1)
		dragType |= OUTER_RIGHT;
	if (cameraPosBox.x0 < subCameraD.x0)
		dragType |= OUTER_LEFT;
