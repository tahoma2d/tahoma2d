...
/*!
  ...
  \details  Vectorization interface is segmented among 3 main classes: \p VectorizerPopup,
            \p Vectorizer, and \p VectorizerCore. \p VectorizerPopup represents the
            outermost layer, providing a suitable user interface to choose between
            available vectorization modes and options, and displays a progress dialog
            to inform about the status of the underlying process.

            In order to launch a suitable vectorization process, an Xsheet or scene
            cast selection must be active; then the \p show() method should be called to
            let the user choose the preferred vectorization options. Each parameter
            set is passed down to a \p VectorizerConfiguration class, so see there for
            more detailed explanations. The \p apply() method can be alternatively
            called for default options, avoiding user interaction.

            During the process, the progress bar dialog is set on modal behaviour, in
            order to prevent the user from modifying or removing the frames to be
            vectorized.

  \sa       The \p VectorizerConfiguration, \p Vectorizer and \p VectorizerCore classes.
*/
...
public:
	...
	bool isLevelToConvert(TXshSimpleLevel *level); //!< Verifies if specified level supports vectorization.
...
private:
	...
	void setType(bool outline); //!< Changes the window appearance depending on \p m_type.

	void loadConfiguration(bool isOutline); //!< Loads VectorizerConfiguration into the popup.
...
private slots:
	...
	void onTypeChange(int type); //!< Stores the active vectorization \p type name, and calls \p setType().

	void onFrameName(QString frameName);
	void onFrameDone(int frameCount);

	void onPartialDone(int partial, int total); //!< Partial progress \p partial of \p total was done for current frame.
	void onFinished();
	...
	void onValueEdited(const TPixel32 &, bool isDrag)
	{
		onValueEdited(isDrag);
	}
...
/*!
  ...
  \details  The \p Vectorizer class serves as a medium between Xsheets and the low-level
            vectorization methods: each frame of the input level is processed by
            \p VectorizerCore methods and the output is then stored on free xsheet cells.

            Signals are also excanghed through this class, ensuring basic communications
            between the invoker (typically \p VectorizerPopup) and the \p VectorizerCore
            class.

            Since vectorization is typically a complex and resource-consuming task,
            \p Vectorizer also inherits a \p QThread in order to avoid system freezes.

  \sa       The \p VectorizerPopup, \p VectorizerConfiguration and \p VectorizerCore
            classes.
*/
...
public:
	...
	TXshSimpleLevel *getVectorizedLevel() const
	{
		return m_vLevel.getPointer();
	}

	void setLevel(const TXshSimpleLevelP &level); //!< Stores current level and creates a new vector level.

	void setParameters(const VectorizerParameters &params) //!  Stores vectorizer parameters.
	{
		m_params = params;
	}

	void setFids(const std::vector<TFrameId> &fids) //!  Stores the frame ids to be processed.
	{
		m_fids = fids, std::sort(m_fids.begin(), m_fids.end());
	}

	bool isCanceled() { return m_isCanceled; } //!< Tells whether vectorization was canceled.
...
private:
	TXshSimpleLevelP m_level;	  //!< Input level to vectorize (only one level at a time is dealt here).
	...
	bool m_isCanceled, //!< User cancels set this flag to true
		m_dialogShown; //!< Whether \p m_dialog was shown for current vectorization.
