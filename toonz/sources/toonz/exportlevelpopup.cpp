...
public:
	...
	void buildString()
	{
		m_pb.setLabelText(msg().arg(QString::number(m_pb.maximum())).arg(m_processedName));
	}
};
...
{
	if (ShortcutZoomer(this).exec(ke))
		return;
	...
	: FileBrowserPopup(tr("Export Level"), Options(CUSTOM_LAYOUT))
{
	...
	ret = connect(m_exportOptions, SIGNAL(optionsChanged()), SLOT(updatePreview())) && ret;
	assert(ret);
}
...
ExportLevelPopup::~ExportLevelPopup()
{
	...
	// This is by FrameNavigator's implicit contract that the supplied frame handle's
...
void ExportLevelPopup::showEvent(QShowEvent *se)
{
	...
	//
	//          The same applies to all the other popups in filebrowserpopup.cpp...
	...
	bool ret = true;
	{
		ret = connect(app->getCurrentSelection(), SIGNAL(selectionChanged(TSelection *)),
...
void ExportLevelPopup::onformatChanged(const QString &text)
{
	checkAlpha();
}
...
void ExportLevelPopup::onOptionsClicked()
{
	...
	if (DVGui::Dialog *dialog = openFormatSettingsPopup(this, ext, props, m_browser->getFolder())) {
		bool ret;
		ret = connect(dialog, SIGNAL(dialogClosed()), SLOT(checkAlpha())), assert(ret);
	}
}
...
bool ExportLevelPopup::execute()
{
	...
	if (colSelection && colSelection->getIndices().size() > 1) {
		fp = TFilePath(m_browser->getFolder() + TFilePath("a")).withType(ext).withFrame();
	...
	: QFrame(parent)
{
...

	{
		...
		m_bgColorField = new DVGui::ColorField(0, false, TPixel32::Black, 35, false);
		...
		layout->addWidget(m_pliOptions, row++, 0, 1, 4);
		{
...

			{
				...
				exportBoxLayout->addWidget(new QLabel(tr("Width:")), 0, 0, Qt::AlignRight);
				exportBoxLayout->addWidget(m_widthFld, 0, 1, Qt::AlignLeft);
				exportBoxLayout->addWidget(new QLabel(tr("Height:")), 0, 2, Qt::AlignRight);
				exportBoxLayout->addWidget(m_heightFld, 0, 3, Qt::AlignLeft);
				exportBoxLayout->addWidget(m_dpiLabel, 0, 4, Qt::AlignRight);
				exportBoxLayout->addWidget(new QLabel(tr("H Resolution:")), 1, 0, Qt::AlignRight);
				exportBoxLayout->addWidget(m_hResFld, 1, 1, Qt::AlignLeft);
				exportBoxLayout->addWidget(new QLabel(tr("V Resolution:")), 1, 2, Qt::AlignRight);
				exportBoxLayout->addWidget(m_vResFld, 1, 3, Qt::AlignLeft);
				exportBoxLayout->addWidget(new QLabel(tr("Scale:")), 1, 4, Qt::AlignRight);
...
IoCmd::ExportLevelOptions ExportLevelPopup::ExportOptions::getOptions() const
{
	...
	opts.m_camera.setSize(TDimensionD(m_widthFld->getValue(), m_heightFld->getValue()));
...
OpenPopupCommandHandler<ExportLevelPopup> exportLevelPopupCommand(MI_ExportLevel);
