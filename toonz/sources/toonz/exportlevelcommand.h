...
// Event Callbacks - accessories to some IoCmds that can be implemented externally
...
struct ExportLevelOptions {
	...
	//! \remark   In single frame exports, the transform applied to the frame is <I>the first</I>.
	double m_thicknessTransform[2][2]; //!< Vector images' thickness transformation at the start and
...
public:
	ExportLevelOptions()
		: m_props(0), m_bgColor(TPixel32::Transparent), m_forRetas(false), m_noAntialias(false)
	{
...
/*!
  \brief    Intended for preview purposes, computes the image that will be exported
...
TImageP exportedImage(
	const std::string &ext,								  //!< File extension - like \a tga, \a tlv, \a png, etc...
...
/*!
  ...
  \details  This function performs level export depending on the specified path's extension,
            and optional parameters. If no input level is specified for the export, this function
            attempts the export of currently active level (if any).

            This function allows external callbacks to deal with overwrite and progress notifications.
            If no callback is supplied to the function, default ones will be used.
*/

bool exportLevel(
	const TFilePath &path,							//!< File path to export the level to.
	TXshSimpleLevel *sl = 0,						//!< Level to export; if \p 0, current level will be used.
	ExportLevelOptions opts = ExportLevelOptions(), //!< Export options.
	OverwriteCallbacks *overwriteCB = 0,			//!< External callbacks to overwrite requests.
	ProgressCallbacks *progressCB = 0				//!< External callbacks to progress notifications.
	);
