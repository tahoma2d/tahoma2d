...
QScriptValue evaluateOnMainThread(QScriptContext *context, QScriptEngine *engine)
{
...
void def(ScriptEngine *teng, const QString &name, QScriptEngine::FunctionSignature fun)
{
	...
	: TPanel(parent)
{
  ...
  */
	...
	resize(800, 300);
	connect(m_scriptConsole, SIGNAL(selectionChanged()), this, SLOT(selectNone()));
}
