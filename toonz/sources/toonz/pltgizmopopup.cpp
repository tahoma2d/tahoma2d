...
void modifyColor(const T &modifier)
{
	...
	for (int i = 0; i < (int)styleIds.size(); i++) {
		...
		if (gname != L"" && cs->getOriginalName() != L"")
			cs->setIsEditedFlag(true);
	}
	if (areAllStyleLincked) {
		delete undo;
		return;
	}
	TApp::instance()->getPaletteController()->getCurrentPalette()->notifyColorStyleChanged(false);
...
void PltGizmoPopup::adjustH(double p)
{
	modifyColor(HueModifier((int)(p)));
}
...
void PltGizmoPopup::shiftH(double p)
{
	modifyColor(HueShifter((int)(p)));
}
...
void PltGizmoPopup::zeroMatte()
{
	modifyColor(TransparencyShifter(-255));
}
void PltGizmoPopup::fullMatte()
{
	modifyColor(TransparencyShifter(255));
}
