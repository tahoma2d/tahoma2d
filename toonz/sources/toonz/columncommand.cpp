...
void cloneNotColumnLinkedFxsAndOutputsFx(TXsheet *xsh, TXsheet *newXsh)
{
	...
	for (i = 0; i < notColumnLinkedClonedFxs.size(); i++) {
		...
		for (j = 0; j < fx->getOutputConnectionCount(); j++) {
			...
			int k, index = 0;
			if (TOutputFx *outFx = dynamic_cast<TOutputFx *>(outputFx))
				continue;

			index = 0;
			for (k = 0; k < outputFx->getInputPortCount(); k++) {
				if (outputFx->getInputPort(k) == outputPort)
					index = k;
			}
	...
	for (i = 0; i < fxDag->getOutputFxCount(); i++) {
		...
		else if (TLevelColumnFx *levelFx = dynamic_cast<TLevelColumnFx *>(inputFx)) {
			int index = levelFx->getColumnIndex();
			TFx *newInputFx = newXsh->getColumn(index)->getFx();
			newOutputFx->getInputPort(0)->setFx(newInputFx);
		} else if (TZeraryColumnFx *zeraryFx = dynamic_cast<TZeraryColumnFx *>(inputFx)) {
...
bool pasteColumnsWithoutUndo(std::set<int> *indices, bool doClone, const StageObjectsData *data)
{
	if (!data)
		data = dynamic_cast<const StageObjectsData *>(QApplication::clipboard()->mimeData());
...
void deleteColumnsWithoutUndo(std::set<int> *indices, bool onlyColumns = false)
{
	TApp *app = TApp::instance();
	TXsheet *xsh = app->getCurrentXsheet()->getXsheet();
	...
	for (it = indices->rbegin(); it != indices->rend(); ++it) {
		...
		if (column && column->getSoundColumn())
			soundColumnRemoved = true;
		if (column && column->getFx())
			app->getCurrentFx()->setFx(0);
		xsh->removeColumn(*it);
	}
...
{
	const StageObjectsData *data = dynamic_cast<const StageObjectsData *>(mimeData);
	if (!data)
		return;
	TApp *app = TApp::instance();
	TXsheet *xsh = app->getCurrentXsheet()->getXsheet();
...
// Clones the TXshChildLevel, but NOT any further nested TXshChildLevel inside it.
TXshChildLevel *cloneChildLevel(TXshChildLevel *cl)
{
	...
	std::set<int> indices;
	for (int i = 0; i < childXsh->getColumnCount(); i++)
		indices.insert(i);
		...
		for (it = m_indices.begin(); it != m_indices.end(); ++it) {
			...
			if (TFx *fx = column->getFx()) {
				for (int i = 0; i < fx->getOutputConnectionCount(); ++i) {
					// Only links whose output-connected fx is in the xsheet (ie not temporary)
					// are stored. Temporaries may be erased without an undo notification...
...
void InsertEmptyColumnsUndo::redo() const
{
	...
	std::vector<std::pair<int, int>>::const_reverse_iterator bt, bEnd = m_columnBlocks.rend();
	for (bt = m_columnBlocks.rbegin(); bt != bEnd; ++bt)
		for (int n = 0; n != bt->second; ++n)
			xsh->insertColumn(bt->first);
...
void InsertEmptyColumnsUndo::undo() const
{
	...
	std::vector<std::pair<int, int>>::const_iterator bt, bEnd = m_columnBlocks.end();
	for (bt = m_columnBlocks.begin(); bt != bEnd; ++bt)
		for (int n = 0; n != bt->second; ++n)
			xsh->removeColumn(bt->first);
...
void ColumnCmd::insertEmptyColumns(const std::set<int> &indices)
{
	...
	std::auto_ptr<ColumnCommandUndo> undo(new InsertEmptyColumnsUndo(positiveIndices));
...
void ColumnCmd::pasteColumns(std::set<int> &indices, const StageObjectsData *data)
{
...
void ColumnCmd::deleteColumns(std::set<int> &indices, bool onlyColumns, bool withoutUndo)
{
	if (indices.empty())
		return;
...
public:
	...
	void redo() const
	{
		...
		for (int r = 0; r < frameCount; r++)
			xsh->setCell(r, m_columnIndex, TXshCell(m_childLevel.getPointer(), TFrameId(r + 1)));
		TApp::instance()->getCurrentXsheet()->notifyXsheetChanged();
	}
...
public:
	...
	void execute()
	{
		...
		for (int i = 0; i < xsh->getColumnCount(); i++) {
			...
			if (cmd & (CMD_ENABLE_CAMSTAND | CMD_DISABLE_CAMSTAND | CMD_TOGGLE_CAMSTAND)) {
