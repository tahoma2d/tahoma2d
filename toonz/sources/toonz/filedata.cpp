...
void FileData::setFiles(std::vector<TFilePath> &files)
{
	m_files = files;
}
...
void FileData::getFiles(TFilePath folder, std::vector<TFilePath> &newFiles) const
{
	int i;
	for (i = 0; i < (int)m_files.size(); i++) {
		...
		if (!TSystem::doesExistFileOrLevel(oldPath)) {
			DVGui::MsgBox(DVGui::WARNING, tr("It is not possible to find the %1 level.").arg(QString::fromStdWString(oldPath.getWideString())));
			return;
		}
