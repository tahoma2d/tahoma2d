								  ...
								  const QModelIndex &index) const
{
	...
	DvDirVersionControlProjectNode *vcpNode = dynamic_cast<DvDirVersionControlProjectNode *>(node);
	DvDirModelFileFolderNode *fnode = dynamic_cast<DvDirModelFileFolderNode *>(node);
	DvDirVersionControlNode *vcNode = dynamic_cast<DvDirVersionControlNode *>(node);
...
void DvDirTreeViewDelegate::commitAndCloseEditor()
{
	...
	FileBrowser *fileBrowser = dynamic_cast<FileBrowser *>(m_treeView->parentWidget());
	if (fileBrowser)
...
void DvDirTreeViewDelegate::updateEditorGeometry(QWidget *editor,
												 const QStyleOptionViewItem &option,
												 const QModelIndex &index) const
{
}
...
void DvDirTreeView::resizeToConts(void)
{
...
void DvDirTreeView::dragMoveEvent(QDragMoveEvent *e)
{
	...
	DvDirModelFileFolderNode *folderNode =
		dynamic_cast<DvDirModelFileFolderNode *>(DvDirModel::instance()->getNode(index));
	DvDirModelNode *node = DvDirModel::instance()->getNode(index);
	if (!node->isFolder())
		return;
...
void DvDirTreeView::contextMenuEvent(QContextMenuEvent *e)
{
	...
	if (vcNode) {
		...
		if (vcNode->isUnderVersionControl()) {
			...
			connect(action, SIGNAL(triggered()), this, SLOT(updateCurrentVersionControlNode()));
		}
		if (fileExists) {
		...
		if (pathType != "tnz" && vcNode->isUnderVersionControl()) {
			...
			connect(action, SIGNAL(triggered()), this, SLOT(refreshCurrentVersionControlNode()));
...
void DvDirTreeView::createMenuAction(QMenu &menu, QString name, const char *slot, bool enable)
{
...
QSize DvDirTreeView::sizeHint() const
{
...
void DvDirTreeView::currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
...
bool DvDirTreeView::edit(const QModelIndex &index, EditTrigger trigger, QEvent *event)
{
...
{
	DvDirVersionControlRootNode *rootNode = dynamic_cast<DvDirVersionControlRootNode *>(node);
	if (rootNode) {
	...
	// Perform a normal update (on an arbitrary node)
		...
		// Check if the path exist, otherwise, it is a missing folder / file that has to be get.
...
void DvDirTreeView::putVersionControl(DvDirVersionControlNode *node)
{
	...
	if (node->isUnderVersionControl() && !isSceneFile) {
		...
		vc->commit(this, toQString(path.getParentDir()),
				   QStringList(QString::fromStdWString(node->getName())), !isSceneFile);
}
...
void DvDirTreeView::listVersionControl(DvDirVersionControlNode *lastExistingNode,
...
void DvDirTreeView::refreshVersionControl(DvDirVersionControlNode *node, const QStringList &files)
{
	if (!refreshVersionControlEnabled())
		return;
	...
	if (rootNode) {
		...
		if (!isSceneFile) {
			// Check if there is a .svn folder inside... and set automatically the version control
			nodePath.append("/");
			QDir dir(nodePath);
		...
		// Missing node that isSceneFile doesn't needs any update
			...
			if (rootNode) {
				...
				// get the last existing parent node, and store in the meantime, its relative path
				...
				setRefreshVersionControlEnabled(false);
				listVersionControl(dynamic_cast<DvDirVersionControlNode *>(n), relativePath);
			}
		}
	}
}
...
{
	DvDirVersionControlNode *vcNode = dynamic_cast<DvDirVersionControlNode *>(getCurrentNode());
	if (!vcNode)
		return;
	...
	if (path.getType() == "tnz") {
		...
		if (!currentProject)
			return;
		TFilePath sceneFolder = currentProject->decode(currentProject->getFolder(TProject::Scenes));
...
#else
		files.append(dir.relativeFilePath(toQString(scenesDescPath)).replace("/", "\\"));
#endif
...
void DvDirTreeView::onCheckOutDone(const QString &text)
{
	...
	if (isVisible()) {
		...
		m_currentRefreshedNode->restoreName();
		QModelIndex index = DvDirModel::instance()->getIndexByNode(m_currentRefreshedNode);
...
void DvDirTreeView::onInfoDone(const QString &xmlResponse)
{
	...
	disconnect(&m_thread, SIGNAL(done(const QString &)), this, SLOT(onInfoDone(const QString &)));
	if (!m_currentRefreshedNode)
		return;

	connect(&m_thread, SIGNAL(done(const QString &)), this, SLOT(onListDone(const QString &)));
	m_thread.executeCommand(toQString(m_currentRefreshedNode->getPath()), "svn", args);
}
...
void DvDirTreeView::checkPartialLock(const QString &workingDir, const QStringList &files)
{
	QStringList args;
	args << "propget";
	args << "partial-lock";
...
void DvDirTreeView::onCheckPartialLockDone(const QString &xmlResults)
{
	...
	for (int i = 0; i < count; i++) {
		...
		SVNStatus s = m_currentRefreshedNode->getVersionControlStatus(lock.m_fileName);
	...
	const TFilePath &fp)
{
	...
	if (s.m_path == name) {
		...
		unsigned int missingCount = 0, normalCount = 0, lockedCount = 0, unversionedCount = 0,
					 readOnlyCount = 0, modifiedCount = 0;
