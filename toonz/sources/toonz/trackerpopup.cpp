...
TFrameId getFidFromFrame(int frame)
{
	...
	if (frame > cellColumn->getRowCount())
		return TFrameId();
...
TRaster32P loadFrame(int frame, const TAffine &affine)
{
	...
	TImageP img = TApp::instance()->getCurrentLevel()->getSimpleLevel()->getFrame(id, false);
	if (!img)
		return (TRaster32P)0;
...
bool TrackerPopup::apply()
{
	...
	if (m_tracker->getLastError() != 0) {
		DVGui::MsgBox(WARNING, m_tracker->getLastError());
		return false;
	}
...
bool Tracker::setup()
{
	...
	TFrameId frameStartId = xsh->getCell(m_indexFrameStart - 1, columnIndex).getFrameId();
	...
	if (!m_trackerObjectsSet || m_trackerObjectsSet->getTrackerObjectsCount() == 0) {
		m_lastErrorCode = 2;
		return false;
	}
	...
	for (i = 0; i < (int)m_trackerObjectsSet->getTrackerObjectsCount(); i++) {
		int j = 0;
		for (j = 0; j < m_trackerObjectsSet->getObjectFromIndex(i)->getHooksCount(); j++) {
			id[posInitObject + j] = m_trackerObjectsSet->getObjectFromIndex(i)->getId();
			...
			// controllo che le dimensioni (altezza) della regioni siano contenute nell'immagine
		...
		posInitObject += m_trackerObjectsSet->getObjectFromIndex(i)->getHooksCount();
	}
...
bool Tracker::trackCurrentFrame()
{
	...
	for (i = 0; i < app1; i++) {
		...
		Hook *hook = m_trackerObjectsSet->getObject(objectId)->getHook(m_trackerRegionIndex);
		hook->setAPos(getFidFromFrame(m_currentFrame), m_affine.inv() * TPointD(globalx, globaly));
		m_trackerRegionIndex++;
		if (m_trackerRegionIndex >= m_trackerObjectsSet->getObject(objectId)->getHooksCount()) {
			m_trackerRegionIndex = 0;
		}
	}
...
MyThread::MyThread(Tracker *tracker)
{
	m_tracker = tracker;
}
