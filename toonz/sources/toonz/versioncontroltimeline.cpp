	...
	: Dialog(TApp::instance()->getMainWindow(), true, true), m_workingDir(workingDir), m_auxFiles(auxFiles), m_fileName(fileName), m_currentExportIndex(0), m_currentAuxExportIndex(0)
{
	...
	connect(&m_thread, SIGNAL(done(const QString &)), SLOT(onLogDone(const QString &)));
...
void SVNTimeline::onLogDone(const QString &xmlResponse)
{
	...
	QString fileNameType = QString::fromStdString(TFilePath(m_fileName.toStdWString()).getType());
...
void SVNTimeline::onExportDone()
{
	...
	if (m_currentExportIndex < itemCount) {
		...
		if (!m_auxTempFiles.isEmpty()) {
			m_thread.disconnect(SIGNAL(done(const QString &)));
			connect(&m_thread, SIGNAL(done(const QString &)), SLOT(onExportAuxDone()));
...
void SVNTimeline::exportToTemporaryFile(int index, bool isAuxFile)
{
	...
	if (isAuxFile && fi.completeSuffix() == "png") {
		args << "sceneIcons/" + fileName + " ." + extension + "@" + m_log.at(index).m_revision;
		args << fi.absoluteFilePath();
	} else {
		args << fileName + "." + fi.completeSuffix() + "@" + m_log.at(index).m_revision;
		args << fi.absoluteFilePath();
	}
...
{
	int selectedItemCount = m_timelineWidget->getListWidget()->selectedItems().count();
...
QIcon SVNTimeline::createIcon(const QString &fileName)
{
	...
	if (filePixmap.isNull()) {
		if (type == "tnz" || type == "tab")
			filePixmap = rasterToQPixmap(IconGenerator::generateSceneFileIcon(path, iconSize, 0));
		else if (type == "pli")
			filePixmap = rasterToQPixmap(IconGenerator::generateVectorFileIcon(path, iconSize, 1));
		else if (type == "tpl")
			...
			filePixmap = rasterToQPixmap(IconGenerator::generateRasterFileIcon(path, iconSize, 1));
		else if (type == "mpath")
...
void SVNTimeline::onUpdateToRevisionButtonClicked()
{
	...
	for (int i = 0; i < auxFilesSize; i++)
		args << m_auxFiles.at(i);

	int sceneResourcesSize = m_sceneResources.count();
	for (int i = 0; i < sceneResourcesSize; i++)
...
void SVNTimeline::onStatusRetrieved(const QString &xmlResponse)
{
	...
	if (!list.empty()) {
		...
		if (s.m_item == "none" || s.m_item == "missing" || s.m_repoStatus == "modified") {
			if (list.size() == 1)
				...
				m_textLabel->setText(tr("Getting %1 items...").arg(m_sceneResources.size() + 1));
	...
	m_timelineWidget->show();
	if (m_fileName.endsWith(".tnz"))
		m_sceneContentsCheckBox->show();
	m_waitingLabel->hide();
	m_textLabel->hide();
}
