	...
	: DVGui::Dialog(TApp::instance()->getMainWindow(), false, false, "NotePopup"), m_viewer(viewer), m_noteIndex(noteIndex), m_currentColorIndex(0), m_isOneButtonPressed(false)
{
	...
	ret = ret && connect(addNoteButton, SIGNAL(pressed()), this, SLOT(onNoteAdded()));
	...
	ret = ret && connect(discardNoteButton, SIGNAL(pressed()), this, SLOT(onNoteDiscarded()));
...
void NotePopup::setCurrentNoteIndex(int index)
{
	m_noteIndex = index;
}
...
void NotePopup::showEvent(QShowEvent *)
{
	...
	bool ret = connect(xsheetHandle, SIGNAL(xsheetSwitched()), this, SLOT(onXsheetSwitched()));
	assert(ret);
}
...
{
	TXsheetHandle *xsheetHandle = TApp::instance()->getCurrentXsheet();
	disconnect(xsheetHandle, SIGNAL(xsheetSwitched()), this, SLOT(onXsheetSwitched()));
}
...
TXshNoteSet *NotePopup::getNotes()
{
	return m_viewer->getXsheet()->getNotes();
}
...
void NotePopup::onColorFieldEditingChanged(const TPixel32 &color, bool isEditing, int index)
{
	if (!isEditing)
		return;
	m_textEditor->setStyleSheet(computeBackgroundStyleSheetString(QColor(color.r, color.g, color.b, color.m)));
...
void NotePopup::onXsheetSwitched()
{
	...
	if (notes->getCount() == 0 || m_noteIndex == -1)
		return;
	m_currentColorIndex = getNotes()->getNoteColorIndex(m_noteIndex);
	TPixel32 color = getColors().at(m_currentColorIndex);
	m_textEditor->setStyleSheet(computeBackgroundStyleSheetString(QColor(color.r, color.g, color.b, color.m)));
	...
	: QFrame(parent), m_newNotePopup(0), m_viewer(parent)
{
	...
	addNoteIcon.addFile(QString(":Resources/newmemo_disabled.png"), QSize(), QIcon::Disabled);
	...
	precNoteIcon.addFile(QString(":Resources/prevkey_disabled.png"), QSize(), QIcon::Disabled);
	...
	nextNoteIcon.addFile(QString(":Resources/nextkey_disabled.png"), QSize(), QIcon::Disabled);
	m_nextNoteButton->setIcon(nextNoteIcon);

	QStringList frameDisplayStyles;
	frameDisplayStyles << tr("Frame") << tr("Sec Frame") << tr("6sec Sheet") << tr("3sec Sheet");
...
void NoteArea::updateButtons()
{
	...
	if (currentNoteIndex == -1 || count == 1)
		...
		if (count > currentNoteIndex + 1)
			m_nextNoteButton->setEnabled(true);
		if (currentNoteIndex > 0)
			m_precNoteButton->setEnabled(true);
	}
}
