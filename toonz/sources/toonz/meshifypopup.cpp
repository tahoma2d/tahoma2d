...
struct MeshifyOptions //!  Available options for the meshification process.
	{
	double m_edgesLength;	  //!< The desired mesh edges length, in standard world coordinates
				  ...
				  TPointD &worldOriginPos)
{
	...
	if (bboxD.x0 >= bboxD.x1 || bboxD.y0 >= bboxD.y1)
		return TRaster32P();
	...
	// Draw the image
	TVectorRenderData rd(TTranslation(worldOriginPos) * TScale(scale), TRect(bbox.getSize()),
						 vi->getPalette(), 0, true, true);
...
TXshSimpleLevel *createMeshLevel(TXshLevel *texturesLevel)
{
	...
	// Build a suitable path (avoid duplicate names from the file system AND scene's level set)
	...
	if (codedDstPath == codedOrigPath && origSl) {
		...
		// Create a new level
		result = dynamic_cast<TXshSimpleLevel *>(scene->createNewLevel(MESH_XSHLEVEL, levelName));
		assert(result);
...
void makeMeshColumn(TXsheet &xsh, int texColId, int meshColIdx)
{
	...
	TStageObject *meshColObj = xsh.getStageObject(TStageObjectId::ColumnId(meshColIdx));
...
TStageObjectId firstChildLevelColumn(const TXsheet &xsh, const TStageObject &obj)
{
	...
	if (col) {
		...
		if (meshCol) {
			...
			if (ml && ml->getType() == MESH_XSHLEVEL) {
				...
				if (xl && (xl->getType() & LEVELCOLUMN_XSHLEVEL)) // Includes sub-xsheet type
					result = xl;
			}
		} else {
			const TXshCell &cell = xsh->getCell(r, c);
			...
			if (xl && (xl->getType() & LEVELCOLUMN_XSHLEVEL))
				result = xl;
		}
	}
...
void MeshifyPopup::showEvent(QShowEvent *se)
{
	...
	connect(columnHandle, SIGNAL(columnIndexSwitched()), this, SLOT(onCellSwitched()));
...
void MeshifyPopup::apply()
{
	...
	{
		TUndoManager::manager()->reset(); // Since this operation CURRENTLY HAS NO UNDO, and yet
...
TMeshImageP meshify(const TXshCell &cell, const MeshifyOptions &options)
{
	...
	if (TXshSimpleLevel *sl = cell.getSimpleLevel()) {
		TImageP img(sl->getFullsampledFrame(cell.getFrameId(), ImageManager::dontPutInCache));
		...
		// Due to a Toonz bug when loading a PLI, slDpi may actually aquire the camera dpi -
		// but it's always shown to be at the STANDARD world DPI, Stage::inch - plus, the
							...
							const MeshifyOptions &options)
{
	struct locals {
		...
		static inline int meshColumnIdx(const TXsheet &xsh, int r0, int r1, int c)
		{
			...
			const TXshMeshColumn *meshCol = xsh.getColumn(meshColIdx)->getMeshColumn();
			if (!meshCol)
				return -1;
	...
	int r, c;
	for (c = c1; c >= c0; --c) // Reverse iteration since we're interleaving the results
	{
		TXshLevelColumn *column = dynamic_cast<TXshLevelColumn *>(xsh->getColumn(c));
	...
	// Set current column to meshColIdx - this allows the users to skip selecting it
							...
							const MeshifyOptions &options)
{
	...
	for (int c = c0; c <= c1; ++c) {
		...
		for (int r = r0; r <= r1; ++r) {
			...
			// NOTE: The same mesh cell could be found multiple times. In this case, subsequent ones
			// will be ignored.
	...
	// Now, we have to re-meshify the map's former mesh references against the latter textures
	...
	// Update icons. Okay, I'm doing this in a separate cycle, because I suspect it's
	// not safe to re-render icons as we're manipulating the level frames map. It *could* be
					  ...
					  const MeshifyOptions &options)
{
	...
	if (TColumnSelection *colSelection = emptySelection ? (TColumnSelection *)0 : dynamic_cast<TColumnSelection *>(selection)) {
		...
		for (it = indices.begin(); it != iEnd; ++it) {
			TXshCellColumn *column = dynamic_cast<TXshCellColumn *>(xsh->getColumn(*it));
...
// Ensure that the specified range has homogeneous columns with respect to meshification
int columnTypes(TXsheet *xsh, int c0, int c1)
{
	...
	for (int c = c0; c <= c1; ++c) {
		...
		if (xc->getLevelColumn())
			columnBits |= HAS_LEVEL_COLUMNS;

		if (xc->getMeshColumn())
			columnBits |= HAS_MESH_COLUMNS;
	}
