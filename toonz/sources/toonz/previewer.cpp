...
/*! \class Previewer
The Previewer class is a singleton which deals with Preview renders of isolated frames.
The main purpose of this class, as of Toonz 6.x, is that of managing preview requests coming
from Toonz's Scene Viewers; however, it is designed to support requests from a generic
\b Previewer::Listener class.
The Previewer is a singleton, meaning that it centralizes requests from many possible listeners
in a single collection of previewed frames.
\n \n
The most important method of the class is \b getRaster(), which attempts retrieval of a certain
...
void Previewer::Listener::requestTimedRefresh()
{
...
//! Removes the associated raster from TImageCache, and listeners are made aware.
void Previewer::Imp::onRenderFailure(const RenderData &renderData, TException &e)
{
...
void Previewer::Imp::doOnRenderRasterFailed(const RenderData &renderData)
{
	...
	if (renderData.m_renderId != it->second.m_renderId)
		return;
...
void Previewer::Imp::saveFrame()
{
	...
	int frameCount = TApp::instance()->getCurrentXsheet()->getXsheet()->getFrameCount();
	...
	: m_imp(new Imp(this))
{
	...
	ret = ret && connect(&xsheetChangedTimer, SIGNAL(timeout()), this, SLOT(onXsheetChanged()));
	ret = ret && connect(&objectChangedTimer, SIGNAL(timeout()), this, SLOT(onObjectChanged()));
...
void Previewer::addListener(Listener *listener)
{
	...
	connect(&listener->m_refreshTimer, SIGNAL(timeout()), this, SLOT(updateView()));
}
...
void Previewer::removeListener(Listener *listener)
{
	...
	disconnect(&listener->m_refreshTimer, SIGNAL(timeout()), this, SLOT(updateView()));
...
void Previewer::saveRenderedFrames()
{
	...
	if (!savePopup)
		savePopup = new SavePreviewedPopup;
...
bool Previewer::isActive() const
{
	return !m_imp->m_listeners.empty();
}
...
void Previewer::onChange(const TFxChange &change)
{
	onObjectChanged();
}
