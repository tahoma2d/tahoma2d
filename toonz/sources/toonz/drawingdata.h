...
{
public:
	std::map<TFrameId, QString> m_imageSet; //images are in the cache, the QString is the cache id
	...
	*/
	...
	DrawingData(const DrawingData *src)
		: m_imageSet(src->m_imageSet), m_level(src->m_level), m_levelHooks(src->m_levelHooks)
	{
	}
	...
	bool getLevelFrames(TXshSimpleLevel *sl, std::set<TFrameId> &frames,
						...
						bool isRedo = false) const;

	// Image must be put in cache before call this.
	void setFrames(const std::map<TFrameId, QString> &imageSet, TXshSimpleLevel *level, const HookSet &levelHooks);
...
protected:
	TImageP getImage(QString imageId, TXshSimpleLevel *sl, const std::map<int, int> &styleTable) const;
};
