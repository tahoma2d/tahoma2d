	...
	: Dialog(TApp::instance()->getMainWindow(), true, false), m_workingDir(workingDir), m_files(files), m_revertSceneContentsCheckBox(0), m_folderOnly(folderOnly), m_sceneIconAdded(sceneIconAdded)
{
	...
	connect(m_revertButton, SIGNAL(clicked()), this, SLOT(onRevertButtonClicked()));
...
void SVNRevertDialog::revertFiles()
{
	...
	if (totalFilesToRevert > 0) {
		...
		for (int i = 0; i < fileToRevertSize; i++)
			args << m_filesToRevert.at(i);
...
void SVNRevertFrameRangeDialog::onStatusRetrieved(const QString &xmlResponse)
{
	...
	if (fileToRevertCount == 0) {
		...
		m_thread.disconnect(SIGNAL(statusRetrieved(const QString &)));
		m_textLabel->setText(tr("Reverting %1 items...").arg(QString::number(fileToRevertCount)));

		QStringList args;
		args << "revert";
		for (int i = 0; i < fileToRevertCount; i++)
			args << m_filesToRevert.at(i);
