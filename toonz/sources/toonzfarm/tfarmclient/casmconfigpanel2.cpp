		...
		try {
			...
			try {
				...
				if (fs.doesExist() && !fs.isDirectory())
					...
					if (task)
						task->m_setupFile = toString(m_text);
				}
			}
...
SubmitPageTask *CasmConfigPanel2::getTask() const
{
	return m_task;
}
...
void CasmConfigPanel2::onNumField(double v, int type)
{
	...
	case M_MCHUNK:
		...
		break;
	case M_THICK:
		if (m_lineart->isSelected())
			m_task->m_lineart = (int)v;
		if (m_edgeAliasing->isSelected())
			m_task->m_edgeAliasing = (int)v;
		break;
	case M_GAMMA:
		m_task->m_gamma = v;
		break;
...
void CasmConfigPanel2::browseSetupFiles()
{
	...
	if (!popup) {
		...
		popup->setOkAction(
			new TFileBrowserPopupAction<CasmConfigPanel2>(csp, &CasmConfigPanel2::loadSetup));
	}
...
void CasmConfigPanel2::loadSetup(const TFilePath &fp)
{
	...
	if (toString(m_setupTextField->getText()) != uncString)
		try {
			uncString = convertToUncString(fp);
		} catch (TException &e) {
