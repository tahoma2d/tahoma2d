	...
	: m_page(page), m_task(0)
{
	...
	m_taskType->setAction(
		new TOptionMenuAction<SubmitPage::Data>(this, &SubmitPage::Data::onTaskType));

	m_configPanel = new CasmConfigPanel2(page);
	m_configPanels.insert(make_pair(string("Casm"), m_configPanel));
	m_configPanels.insert(make_pair(string("Script"), new ScriptConfigPanel(page)));
...
void SubmitPage::Data::browseCasmFiles()
{
	...
	if (m_taskType->getText() == "Casm") {
		if (!puCasm) {
			...
			puCasm->setOkAction(
				new TFileBrowserPopupAction<SubmitPage::Data>(this, &SubmitPage::Data::load));
		}
		popup = puCasm;
...
void SubmitPage::Data::load(const TFilePath &fp)
{
	...
	if (toString(m_filepathTextField->getText()) != uncString)
		try {
...
void SubmitPage::Data::submitCasm(CasmTask2 *casm)
{
	...
	for (; itDep != dep.end(); ++itDep)
		task.m_dependencies->add((*itDep).first);

	int ra = casm->m_start;

	int subCount = casm->m_end / casm->m_taskChunksize;
	if (casm->m_end % casm->m_taskChunksize)
		subCount++;
...
void SubmitPage::Data::submitScript(ScriptTask *script)
{
	...
	string user = TSystem::getUserName();
	string host = TSystem::getHostName();
	TFarmTask task(scriptName, nativeCmdLine, user, host, 1, (int)m_priority->getValue());
...
SubmitPageTask *SubmitPage::getTask() const
{
