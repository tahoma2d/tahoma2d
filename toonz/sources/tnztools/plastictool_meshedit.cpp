...
bool borderVertex(const TTextureMesh &mesh, int v)
{
	...
	tcg::vertex_traits<TTextureVertex>::edges_const_iterator et, eEnd(vx.edgesEnd());
...
bool testCollapseEdge(const TTextureMesh &mesh, int e)
{
	struct Locals {
		...
		bool testAdjacency()
		{
			...
			tcg::vertex_traits<vertex_type>::edges_const_iterator et, eEnd = vx0.edgesEnd();
...
std::pair<double, int> closestEdge(const TTextureMesh &mesh, const TPointD &pos)
{
	Closer closer = {mesh, pos};
...
std::pair<double, MeshIndex> closestVertex(const TMeshImage &mi, const TPointD &pos)
{
	std::pair<double, MeshIndex> closest((std::numeric_limits<double>::max)(), MeshIndex());
...
std::pair<double, MeshIndex> closestEdge(const TMeshImage &mi, const TPointD &pos)
{
	std::pair<double, MeshIndex> closest((std::numeric_limits<double>::max)(), MeshIndex());
