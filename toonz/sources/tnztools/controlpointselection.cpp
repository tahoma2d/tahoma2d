...
bool ControlPointEditorStroke::setControlPointsLinear(std::set<int> points, bool isLinear)
{
...
void ControlPointSelection::deleteControlPoints()
{
	...
	if (m_controlPointEditorStroke->getControlPointCount() == 0) {
		...
		if (!isCurrentObjectSpline) {
			UndoControlPointEditor *cpEditorUndo = dynamic_cast<UndoControlPointEditor *>(undo);
			if (cpEditorUndo)
