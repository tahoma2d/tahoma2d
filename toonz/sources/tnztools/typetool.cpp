...
void paintChar(const TVectorImageP &image, int styleId)
{
	...
	for (j = 0; j < strokeNum; j++)
		image->getStroke(j)->setStyle(styleId);
	...
	for (j = 0; j < regNum; j++)
		paintRegion(image->getRegion(j), styleId, true);
}
	...
	void undo() const
	{
		...
		if (m_fillInformationBefore) {
			...
			for (i = 0; i < size; i++) {
				...
				assert(reg);
				if (reg)
					reg->setStyle((*m_fillInformationBefore)[i].m_styleId);
			}
		}
	...
	void redo() const
	{
		...
		if (m_fillInformationAfter) {
			...
			for (i = 0; i < size; i++) {
				...
				assert(reg);
				if (reg)
					reg->setStyle((*m_fillInformationAfter)[i].m_styleId);
			}
		}
		application->getCurrentXsheet()->notifyXsheetChanged();
		notifyImageChanged();
...
void TypeTool::loadFonts()
{
	...
	m_scale = TScale(m_dimension / (double)(TFontManager::instance()->getMaxHeight()));
}
...
void TypeTool::setSize(std::wstring strSize)
{
	...
	for (UINT i = 0; i < m_string.size(); i++) {
		if (TVectorImageP vi = m_string[i].m_char)
			vi->transform(TScale(ratio));
...
void TypeTool::updateCharPositions(int updateFrom)
{
	...
	TFontManager *instance = TFontManager::instance();
	if (updateFrom > 0) {
		if ((int)m_string.size() <= updateFrom - 1)
			return;
		...
		if (m_isVertical && !instance->hasVertical()) {
			...
			if (m_string[updateFrom - 1].isReturn())
				...
				currentOffset = currentOffset + TPointD(m_string[updateFrom - 1].m_offset, 0);
		}
...
void TypeTool::addTextToVectorImage(const TVectorImageP &currentImage, std::vector<const TVectorImage *> &images)
{
	...
	UndoTypeTool *undo = new UndoTypeTool(fillInformationBefore, fillInformationAfter, level,
...
void TypeTool::setCursorIndexFromPoint(TPointD point)
{
	...
	if (!m_isVertical)
		retNum = tround((m_startPoint.y + m_dimension - point.y) / m_dimension - 0.5);
	else
...
void TypeTool::replaceText(std::wstring text, int from, int to)
{
	...
	for (unsigned int i = 0; i < (unsigned int)text.size(); i++) {
		...
		if (vi) {
			...
			TToonzImageP newTImage(new TToonzImage(newRasterCM, newRasterCM->getBounds()));
...
void TypeTool::addReturn()
{
	...
	if ((UINT)m_cursorIndex == m_string.size())
		...
		m_string.insert(m_string.begin() + m_cursorIndex, StrokeChar(vi, -1., TK_Return, 0));
...
void TypeTool::onInputText(std::wstring preedit, std::wstring commit, int replacementStart, int replacementLen)
{
	...
	int b = tcrop(m_preeditRange.first + replacementStart + replacementLen, a, stringLength);
	replaceText(commit, a, b);
	int index = a + commit.length();

	// inserisco la nuova preedit string
	if (preedit.length() > 0)
		replaceText(preedit, index, index);
