...
{
	vector<TFilledRegionInf> m_oldFillInformation, m_newFillInformation;

	int m_row;
	int m_column;

public:
	map<int, VIStroke *> m_originalStrokes;
	map<int, VIStroke *> m_newStrokes;
	...
	~UndoEraser()
	{
		map<int, VIStroke *>::const_iterator it;
	...
	void addOldStroke(int index, VIStroke *stroke)
	{
		...
		m_originalStrokes.insert(map<int, VIStroke *>::value_type(index, s));
	}

	void addNewStroke(int index, VIStroke *stroke)
	{
		VIStroke *s = cloneVIStroke(stroke);
		m_newStrokes.insert(map<int, VIStroke *>::value_type(index, s));
	}
	...
	void undo() const
	{
		...
		vector<int> newStrokeIndex(m_newStrokes.size());
		mapToVector(m_newStrokes, newStrokeIndex);

		image->removeStrokes(newStrokeIndex, true, false);

		map<int, VIStroke *>::const_iterator it = m_originalStrokes.begin();
		UINT i = 0;
		VIStroke *s;
	...
	void redo() const
	{
		...
		QMutexLocker lock(image->getMutex());
		vector<int> oldStrokeIndex(m_originalStrokes.size());
		mapToVector(m_originalStrokes, oldStrokeIndex);

		image->removeStrokes(oldStrokeIndex, true, false);

		map<int, VIStroke *>::const_iterator it = m_newStrokes.begin();
		UINT i = 0;
		VIStroke *s;
...
private:
	...
	vector<TPointD> m_polyline; //!< Polyline points.
