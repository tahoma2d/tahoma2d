...
{
	std::for_each(m_controls.begin(), m_controls.end(), tcg::deleter<ToolOptionControl>());
	...
	: ToolOptionsBox(parent), m_pg(pg), m_tool(tool), m_frameHandle(frameHandle), m_objHandle(objHandle), m_xshHandle(xshHandle)
{
	...
	// Lock E/W
	TBoolProperty *lockProp = dynamic_cast<TBoolProperty *>(m_pg->getProperty("Lock Position E/W"));
	if (lockProp)
	...
	// Lock E/W Center
	lockProp = dynamic_cast<TBoolProperty *>(m_pg->getProperty("Lock Center E/W"));
	if (lockProp)
		m_lockEWCenterCheckbox = new ToolOptionCheckbox(m_tool, lockProp, toolHandle, this);
	// Lock N/S Center
	lockProp = dynamic_cast<TBoolProperty *>(m_pg->getProperty("Lock Center N/S"));
	if (lockProp)
	...
	: ToolOptionsBox(parent), m_targetType(tool->getTargetType()), m_poligonSideLabel(0), m_hardnessLabel(0), m_hardnessField(0), m_poligonSideField(0), m_shapeField(0), m_pencilMode(0)
{
	...
	m_hardnessField = dynamic_cast<ToolOptionSlider *>(m_controls.value("Hardness:"));
	if (m_hardnessField)
	...
	m_poligonSideField = dynamic_cast<ToolOptionIntSlider *>(m_controls.value("Polygon Sides:"));
	if (m_poligonSideField)
		m_poligonSideLabel = m_labels.value(m_poligonSideField->propertyName());
	m_pencilMode = dynamic_cast<ToolOptionCheckbox *>(m_controls.value("Pencil Mode"));
	...
	: ToolOptionsBox(parent), m_tool(tool)
{
	...
	bool ret = true;
	ToolOptionCombo *fontField = dynamic_cast<ToolOptionCombo *>(m_controls.value("Font:"));
	ret &&connect(fontField, SIGNAL(currentIndexChanged(int)), this, SLOT(onFieldChanged()));

#ifndef MACOSX
	ToolOptionCombo *styleField = dynamic_cast<ToolOptionCombo *>(m_controls.value("Style:"));
	...
	: ToolOptionsBox(parent), m_targetType(tool->getTargetType()), m_fillDepthLabel(0), m_fillDepthField(0), m_segmentMode(0)
{
	...
	m_colorMode = dynamic_cast<ToolOptionCombo *>(m_controls.value("Mode:"));
	m_selectiveMode = dynamic_cast<ToolOptionCheckbox *>(m_controls.value("Selective"));
	m_fillDepthField = dynamic_cast<ToolOptionPairSlider *>(m_controls.value("Fill Depth"));
	if (m_fillDepthField)
		m_fillDepthLabel = m_labels.value(m_fillDepthField->propertyName());
	m_segmentMode = dynamic_cast<ToolOptionCheckbox *>(m_controls.value("Segment"));
	...
	: ToolOptionsBox(parent), m_smoothMode(0), m_joinStrokesMode(0), m_toolMode(0), m_autocloseLabel(0), m_autocloseField(0)
{
	...
	m_joinStrokesMode = dynamic_cast<ToolOptionCheckbox *>(m_controls.value("JoinStrokes"));
	...
	m_autocloseField = dynamic_cast<ToolOptionSlider *>(m_controls.value("Distance"));
	if (m_autocloseField)
	...
	: ToolOptionsBox(parent)
{
	...
	m_realTimePickMode = dynamic_cast<ToolOptionCheckbox *>(m_controls.value("Passive Pick"));
