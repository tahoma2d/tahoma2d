	...
	)
{
	...
	if (1 <= integer_part) {
		...
		if (0.0 < fractional_part) {
			accumulation +=
				((accumulation * transparent + src_value) -
				 accumulation) *
				fractional_part;
		}
	} else { /* 整数部分がゼロ以下のとき */
	...
	const double density)
{
	...
	for (int ii = 0; ii < pixsize; ++ii, image_array += channels) {
		...
		double rr2 = accum_by_trans_(rr1,
									 1.0 - aa1, integer_part, fractional_part);
		double gg2 = accum_by_trans_(gg1,
									 1.0 - aa1, integer_part, fractional_part);
		double bb2 = accum_by_trans_(bb1,
									 1.0 - aa1, integer_part, fractional_part);
		double aa2 = accum_by_trans_(aa1,
									 1.0 - aa1, integer_part, fractional_part);
	...
	const double density)
{
	...
	if ((std::numeric_limits<unsigned char>::digits == bits) && ((std::numeric_limits<unsigned char>::digits == ref_bits) || (0 == ref_bits))) {
		change_template_(
			image_array, height, width, channels, ref, ref_mode, density);
	} else if ((std::numeric_limits<unsigned short>::digits == bits) && ((std::numeric_limits<unsigned char>::digits == ref_bits) || (0 == ref_bits))) {
		change_template_(
			reinterpret_cast<unsigned short *>(image_array), height, width, channels, ref, ref_mode, density);
	} else if ((std::numeric_limits<unsigned short>::digits == bits) && (std::numeric_limits<unsigned short>::digits == ref_bits)) {
		change_template_(
			reinterpret_cast<unsigned short *>(image_array), height, width, channels, reinterpret_cast<const unsigned short *>(ref), ref_mode, density);
	} else if ((std::numeric_limits<unsigned short>::digits == bits) && (std::numeric_limits<unsigned short>::digits == ref_bits)) {
		change_template_(
			image_array, height, width, channels, reinterpret_cast<const unsigned short *>(ref), ref_mode, density);
	} else {
