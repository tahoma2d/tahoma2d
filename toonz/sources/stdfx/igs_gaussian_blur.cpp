...
void blur_1st_hori_(
	...
	,
	const int height_with_margin, const int width_with_margin, double *brush_sequence // &(std::vector<double *>).at(0)
	,
	const int int_radius, double **out_plane_with_margin // &(std::vector<double *>).at(0)
...
template <class T>
void get_(
	const T *in, const int height, const int width, const int channels, const int current_ch, double **out // &(std::vector<double *>).at(0)
	)
{
	in += current_ch;
	const double maxi = static_cast<double>(
...
void convert_hv_(
	...
	,
	const int int_radius, double **buffer_inn // &(std::vector<double *>).at(0)
	,
	...
	)
{
	...
	std::vector<double *> in_plane_with_margin_dp(
		height_with_margin);
	...
	std::vector<double *> out_plane_with_margin_dp(
		height_with_margin);
	for (int yy = 0; yy < height_with_margin; ++yy) {
	...
	if ((std::numeric_limits<unsigned char>::digits == bits) && ((std::numeric_limits<unsigned char>::digits == ref_bits) || (0 == ref_bits))) {
		convert_hv_(
			static_cast<const unsigned char *>(in_with_margin), static_cast<unsigned char *>(out_no_margin), height_with_margin, width_with_margin, channels, &filter_buf.at(0), int_radius, &in_plane_with_margin_dp.at(0), &out_plane_with_margin_dp.at(0)
		...
		convert_hv_(
			static_cast<const unsigned short *>(in_with_margin), static_cast<unsigned short *>(out_no_margin), height_with_margin, width_with_margin, channels, &filter_buf.at(0), int_radius, &in_plane_with_margin_dp.at(0), &out_plane_with_margin_dp.at(0)
		...
		convert_hv_(
			static_cast<const unsigned short *>(in_with_margin), static_cast<unsigned short *>(out_no_margin), height_with_margin, width_with_margin, channels, &filter_buf.at(0), int_radius, &in_plane_with_margin_dp.at(0), &out_plane_with_margin_dp.at(0)
																																																								   ...
																																																								   ,
			reinterpret_cast<const unsigned short *>(ref), ref_mode, real_radius, sigma);
	} else if ((std::numeric_limits<unsigned char>::digits == bits) && (std::numeric_limits<unsigned short>::digits == ref_bits)) {
		convert_hv_(
			static_cast<const unsigned short *>(in_with_margin), static_cast<unsigned short *>(out_no_margin), height_with_margin, width_with_margin, channels, &filter_buf.at(0), int_radius, &in_plane_with_margin_dp.at(0), &out_plane_with_margin_dp.at(0)

																																																								   ,
			reinterpret_cast<const unsigned char *>(ref), ref_mode, real_radius, sigma);
	} else {
