...
IGS_RADIAL_BLUR_EXPORT void convert(
	...
	,
	const int ref_bits, const int ref_mode // R,G,B,A,luminance or R,G,B,luminance
	...
	,
	const double xc, const double yc, const double twist_radian = 0.0, const double twist_radius = 0.0, const double intensity = 0.2 /* 強度。ゼロより大きく2以下 */
	,
...
IGS_RADIAL_BLUR_EXPORT int reference_margin(																																						/* Twist時正確でない... */
											const int height, const int width, const double xc, const double yc, const double twist_radian, const double twist_radius, const double intensity = 0.2 /* 強度。ゼロより大きく2以下 */
											/* radius円境界での平均値ぼかしゼロとするためintensityは2以下 */
											,
											const double radius = 0.0 /* 平均値ぼかしの始まる半径 */
											,
