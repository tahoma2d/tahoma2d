...
bool DespeckleFx::doGetBBox(double frame, TRectD &bBox, const TRenderSettings &info)
{
...
void DespeckleFx::doCompute(TTile &tile, double frame, const TRenderSettings &ri)
{
	if (!m_input.isConnected())
		return;
	...
	TRop::despeckle(tile.getRaster(), size, false, m_transparenceType->getValue() == 1);
}

//------------------------------------------------------------------

void DespeckleFx::doDryCompute(TRectD &rect, double frame, const TRenderSettings &info)
{
	if (!m_input.isConnected())
		return;
