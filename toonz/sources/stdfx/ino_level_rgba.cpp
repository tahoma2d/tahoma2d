	...
	TTile &tile, double frame, const TRenderSettings &rend_sets)
{
	...
	double red_gamma = this->m_red_gamma->getValue(frame) /
					   ino::param_range();
	...
	double gre_gamma = this->m_gre_gamma->getValue(frame) /
					   ino::param_range();
	...
	double blu_gamma = this->m_blu_gamma->getValue(frame) /
					   ino::param_range();
	...
	double alp_gamma = this->m_alp_gamma->getValue(frame) /
					   ino::param_range();
	...
	if (log_sw) {
		...
		os << "params"
		   << "  red_in_min " << v_red_in.first
		   << "  red_in_max " << v_red_in.second
		   << "  red_out_min " << v_red_out.first
		   << "  red_out_max " << v_red_out.second
		   << "  red_gamma " << red_gamma
		   << "  gre_in_min " << v_gre_in.first
		   << "  gre_in_max " << v_gre_in.second
		   << "  gre_out_min " << v_gre_out.first
		   << "  gre_out_max " << v_gre_out.second
		   << "  gre_gamma " << gre_gamma
		   << "  blu_in_min " << v_blu_in.first
		   << "  blu_in_max " << v_blu_in.second
		   << "  blu_out_min " << v_blu_out.first
		   << "  blu_out_max " << v_blu_out.second
		   << "  blu_gamma " << blu_gamma
		   << "  alp_in_min " << v_alp_in.first
		   << "  alp_in_max " << v_alp_in.second
		   << "  alp_out_min " << v_alp_out.first
		   << "  alp_out_max " << v_alp_out.second
		   << "  alp_gamma " << alp_gamma
	...
	try {
		...
		igs::levels::change(
			...
			,
			v_red_in.first, v_red_in.second, v_gre_in.first, v_gre_in.second, v_blu_in.first, v_blu_in.second, v_alp_in.first, v_alp_in.second, red_gamma, gre_gamma, blu_gamma, alp_gamma, v_red_out.first, v_red_out.second, v_gre_out.first, v_gre_out.second, v_blu_out.first, v_blu_out.second, v_alp_out.first, v_alp_out.second
