...
public:
	noise_reference(
		const int ww, const int hh, const double hue_range, const double lig_range, const double sat_range, const double alp_range, const unsigned long random_seed, const double near_blur
...
/* RGB値にノイズをのせる */
void pixel_rgb(
	const double red_in, const double gre_in, const double blu_in, const double alp_in, const double hue_noise, const double lig_noise, const double sat_noise, control_term_within_limits &lig_term, control_term_within_limits &sat_term, double &red_out, double &gre_out, double &blu_out);
/* Alpha値にノイズをのせる */
void pixel_a(
...
void igs::hls_noise_in_camera::pixel_a(
	const double alp_in, const double alp_noise, control_term_within_limits &alp_term, double &alp_out)
{
...
void change_template_(
	T *image_array, const int ww, const int hh, const int ch, noise_reference &noise, const double hue_range, control_term_within_limits &lig_term, control_term_within_limits &sat_term, control_term_within_limits &alp_term)
{
		...
		for (int yy = 0; yy < hh; ++yy) {
			for (int xx = 0; xx < ww; ++xx) {
				if (
					...
					pixel_rgb(
						static_cast<double>(image_array[red]) / div_val, static_cast<double>(image_array[gre]) / div_val, static_cast<double>(image_array[blu]) / div_val, static_cast<double>(image_array[alp]) / div_val, noise.hue_value(xx, yy), noise.lig_value(xx, yy), noise.sat_value(xx, yy), lig_term, sat_term, rr, gg, bb);
		...
		if (
			...
			for (int yy = 0; yy < hh; ++yy) {
				for (int xx = 0; xx < ww; ++xx, image_array += ch) {
					pixel_rgb(
						static_cast<double>(image_array[red]) / div_val, static_cast<double>(image_array[gre]) / div_val, static_cast<double>(image_array[blu]) / div_val, 1.0, noise.hue_value(xx, yy), noise.lig_value(xx, yy), noise.sat_value(xx, yy), lig_term, sat_term, rr, gg, bb);
					image_array[red] = static_cast<T>(rr * mul_val);
					image_array[gre] = static_cast<T>(gg * mul_val);
					image_array[blu] = static_cast<T>(bb * mul_val);
				}
	...
	const double lig_effective, const double lig_center, const int lig_type, const double sat_effective, const double sat_center, const int sat_type, const double alp_effective, const double alp_center, const int alp_type)
{
	...
	if (std::numeric_limits<unsigned char>::digits == bits) {
		change_template_(
			static_cast<unsigned char *>(image_array), width, height, channels, noise, hue_range, lig_term, sat_term, alp_term);
		noise.clear(); /* ノイズ画像メモリ解放 */
	} else if (std::numeric_limits<unsigned short>::digits == bits) {
		change_template_(
			static_cast<unsigned short *>(image_array), width, height, channels, noise, hue_range, lig_term, sat_term, alp_term);
		noise.clear(); /* ノイズ画像メモリ解放 */
	} else {
