...
{
//--------------------------------------------------------------------
double clamp_min1_ch_(const double val)
{
...
double multiply_(const double dn, const double up)
{
	return dn * up;
}
double multiply_ch_(
	...
	double &dn_r, double &dn_g, double &dn_b, double &dn_a, const double up_r, double up_g, double up_b, double up_a, const double up_opacity)
{
	...
	if (dn_a <= 0) { /* downが透明のときはup値を表示 */
		dn_set_up_opacity_(dn_r, dn_g, dn_b, dn_a, up_r, up_g, up_b, up_a, up_opacity);
		return;
	}
