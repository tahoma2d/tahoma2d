...
public:
	ino_warp_hv()
		: m_h_maxlen(0.0 * ino::param_range()), m_v_maxlen(0.0 * ino::param_range()), m_h_ref_mode(new TIntEnumParam(2, "Red")), m_v_ref_mode(new TIntEnumParam(2, "Red")), m_alpha_rendering(true), m_anti_aliasing(true)
	{
		...
		this->m_h_maxlen->setValueRange(
			0.0 * ino::param_range(), 100.0 * ino::param_range());
		this->m_v_maxlen->setValueRange(
			0.0 * ino::param_range(), 100.0 * ino::param_range());
...
{
template <class T>
void data_set_template_(
	const TRasterPT<T> in_ras // with margin
	,
...
void fx_(
	...
	,
	const double h_maxlen, const double v_maxlen, const int h_ref_mode, const int v_ref_mode, const bool alpha_rendering_sw, const bool anti_aliasing_sw)
{
	if (0 != hori_ras) {
	...
	if ((TRaster32P)in_ras) {
		data_set_template_<TPixel32>(
			in_ras, margin, out_ras);
	} else if ((TRaster64P)in_ras) {
		data_set_template_<TPixel64>(
			in_ras, margin, out_ras);
	}
}
}
//------------------------------------------------------------
void ino_warp_hv::doCompute(
	TTile &tile, double frame, const TRenderSettings &rend_sets)
{
	/*------ 接続していなければ処理しない ----------------------*/
	...
	double h_maxlen = 0;
	double v_maxlen = 0;
	this->get_render_real_hv(
		frame, rend_sets.m_affine, h_maxlen, v_maxlen);
	/*------ 表示の範囲を得る ----------------------------------*/
	TRectD bBox = TRectD(
	...
	if (hori_cn_is) {
		this->m_hori->allocateAndCompute(
			hori_tile, bBox.getP00(), TDimensionI(/* Pixel単位のdouble -> intにしてセット */
												  (int)(bBox.getLx() + 0.5), (int)(bBox.getLy() + 0.5)),
			tile.getRaster(), frame, rend_sets);
	}
	if (vert_cn_is) {
		this->m_vert->allocateAndCompute(
			vert_tile, bBox.getP00(), TDimensionI(/* Pixel単位のdouble -> intにしてセット */
												  (int)(bBox.getLx() + 0.5), (int)(bBox.getLy() + 0.5)),
			tile.getRaster(), frame, rend_sets);
	}
	/*------ 保存すべき画像メモリを塗りつぶしクリア ------------*/
	...
	if (log_sw) {
		...
		if (hori_cn_is) {
			os
				<< "   htile w " << hori_tile.getRaster()->getLx()
				<< "  h " << hori_tile.getRaster()->getLy()
				<< "  pixbits " << ino::pixel_bits(hori_tile.getRaster());
		}
		if (vert_cn_is) {
			os
				<< "   vtile w " << vert_tile.getRaster()->getLx()
				<< "  h " << vert_tile.getRaster()->getLy()
				<< "  pixbits " << ino::pixel_bits(vert_tile.getRaster());
		}
		os
			<< "   frame " << frame
	...
	try {
		...
		fx_(
			...
			,
			h_maxlen, v_maxlen, this->m_h_ref_mode->getValue(), this->m_v_ref_mode->getValue(), alp_rend_sw, anti_ali_sw);
		if (vert_cn_is) {
