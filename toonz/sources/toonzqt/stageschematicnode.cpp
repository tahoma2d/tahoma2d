...
namespace
{
void drawCamera(QPainter *painter, const QColor &color, const QPen &pen, double width, double height)
{
	QPointF points[3];
	...
	painter->drawRect(QRectF(width * 0.25, height * 0.5, width * 0.5, height * 0.5));
...
ColumnPainter::ColumnPainter(StageSchematicColumnNode *parent, double width, double height, const QString &name)
	: QGraphicsItem(parent), m_parent(parent), m_width(width), m_height(height), m_name(name)
{
...
void ColumnPainter::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	StageSchematicScene *stageScene = dynamic_cast<StageSchematicScene *>(scene());
	if (!stageScene)
		return;
...
GroupPainter::GroupPainter(StageSchematicGroupNode *parent, double width, double height, const QString &name)
	: QGraphicsItem(parent), m_parent(parent), m_width(width), m_height(height), m_name(name)
{
	setFlag(QGraphicsItem::ItemIsMovable, false);
	setFlag(QGraphicsItem::ItemIsSelectable, false);
...
void GroupPainter::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	StageSchematicScene *stageScene = dynamic_cast<StageSchematicScene *>(scene());
	if (!stageScene)
		return;
...
PegbarPainter::PegbarPainter(StageSchematicPegbarNode *parent, double width,
	...
	: QGraphicsItem(parent), m_width(width), m_height(height), m_parent(parent), m_name(name)
{
}
...
void PegbarPainter::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
CameraPainter::CameraPainter(StageSchematicCameraNode *parent, double width, double height, const QString &name)
	: QGraphicsItem(parent), m_width(width), m_height(height), m_parent(parent), m_name(name)
{
	StageSchematicScene *stageScene = dynamic_cast<StageSchematicScene *>(scene());
	TXsheet *xsh = stageScene->getXsheet();
	m_isActive = xsh->getStageObjectTree()->getCurrentCameraId() == parent->getStageObject()->getId();
}
...
void CameraPainter::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
void CameraPainter::contextMenuEvent(QGraphicsSceneContextMenuEvent *cme)
{
	...
	QAction *cameraSettings = CommandManager::instance()->getAction("MI_CameraStage");
	...
	bool isDeactivated = m_parent->getStageObject()->getId() != stageScene->getXsheet()->getStageObjectTree()->getCurrentCameraId();
...
TablePainter::TablePainter(StageSchematicTableNode *parent, double width, double height)
...
void TablePainter::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
SplinePainter::SplinePainter(StageSchematicSplineNode *parent, double width, double height,
							 const QString &name)
	: QGraphicsItem(parent), m_parent(parent), m_width(width), m_height(height), m_name(name)
{
}
...
void SplinePainter::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
void SplinePainter::contextMenuEvent(QGraphicsSceneContextMenuEvent *cme)
{
	...
	connect(removeSpline, SIGNAL(triggered()), stageScene, SLOT(onRemoveSpline()));
...
StageSchematicNodePort::StageSchematicNodePort(StageSchematicNodeDock *parent, int type)
...
void StageSchematicNodePort::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	...
	if (getType() == eStageChildPort || getType() == eStageChildGroupPort)
		painter->drawPixmap(QRect(0, 0, 18, 18), (isHighlighted()) ? redPm_HL : redPm);
	else
		painter->drawPixmap(QRect(0, 0, 18, 18), (isHighlighted()) ? bluePm_HL : bluePm);
}
...
bool StageSchematicNodePort::linkTo(SchematicPort *port, bool checkOnly)
{
	...
	if (this == port && !srcNode->getStageObject()->isGrouped()) {
		if (!checkOnly)
			TStageObjectCmd::setParent(srcNode->getStageObject()->getId(), TStageObjectId::NoneId,
									   "", stageScene->getXsheetHandle());
		return true;
	}
	StageSchematicNode *dstNode = dynamic_cast<StageSchematicNode *>(port->getNode());
	if (!dstNode)
		return false;
	if (srcNode == dstNode)
		return false;
	...
	if (getType() == eStageParentPort && port->getType() == eStageChildPort) {
		...
		childPort = this;
		parentPort = port;
	} else if (getType() == eStageChildPort && port->getType() == eStageParentPort) {
		...
		return false;
	if (pegbar->getId().isTable())
		return false;
	if (pegbar->getId().isPegbar() &&
		!parentPegbar->getId().isTable() && !parentPegbar->getId().isPegbar() && !parentPegbar->getId().isCamera())
		return false;

	if (parentPegbar == pegbar || parentPegbar->isAncestor(pegbar))
		return false;
	if (parentPegbar->isGrouped() || pegbar->isGrouped())
		return false;
	if (checkOnly)
		return true;

	StageSchematicNodePort *dstPort = dynamic_cast<StageSchematicNodePort *>(parentPort);
	if (!dstPort)
		return false;
	TStageObjectCmd::setParent(pegbar->getId(), parentPegbar->getId(),
							   dstPort->getHandle().toStdString(), stageScene->getXsheetHandle());
	return true;
}
...
SchematicPort *StageSchematicNodePort::searchPort(const QPointF &scenePos)
{
	...
	for (i = 0; i < items.size(); i++) {
		StageSchematicNodePort *linkingTo = dynamic_cast<StageSchematicNodePort *>(items[i]);
		if (linkingTo) {
			StageSchematicNode *node = dynamic_cast<StageSchematicNode *>(linkingTo->getNode());
			if (!node->getStageObject()->isGrouped())
				return linkingTo;
		}
	}
	return 0;
...
void StageSchematicNodePort::hideSnappedLinks()
{
	if (!m_linkingTo)
		return;
	if (getType() == eStageChildPort && m_linkingTo->getType() == eStageParentPort && m_linkingTo->getLinkCount() == 1)
		m_linkingTo->getLink(0)->hide();
	if (getType() == eStageParentPort && m_linkingTo->getType() == eStageChildPort && getLinkCount() == 1)
		getLink(0)->hide();
}
...
StageSchematicSplinePort::StageSchematicSplinePort(StageSchematicSplineDock *parent, int type)
...
void StageSchematicSplinePort::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	//ChildPm : connected to the pegbar
	QPixmap childPm = QPixmap(":Resources/spline_child_port.png");
	QPixmap childPm_disconnect = QPixmap(":Resources/spline_child_port_disconnect.png");
...
bool StageSchematicSplinePort::linkTo(SchematicPort *port, bool checkOnly)
{
	...
	StageSchematicSplineNode *splineNode = dynamic_cast<StageSchematicSplineNode *>(getNode());
	if (node) {
		StageSchematicSplineNode *targetSplineNode = dynamic_cast<StageSchematicSplineNode *>(port->getNode());
		if (!targetSplineNode && port != this)
			return false;
		if (!checkOnly && targetSplineNode) {
			TStageObjectSpline *spline = targetSplineNode->getSpline();
			TStageObjectCmd::setSplineParent(spline, node->getStageObject(), stageScene->getXsheetHandle());
		} else if (!checkOnly && port == this)
			node->getStageObject()->setSpline(0);
		return true;
	} else if (splineNode) {
		StageSchematicNode *targetNode = dynamic_cast<StageSchematicNode *>(port->getNode());
		if (!targetNode || port->getType() != eStageSplinePort)
			return false;
		if (!checkOnly) {
			TStageObjectSpline *spline = splineNode->getSpline();
			TStageObjectCmd::setSplineParent(spline, targetNode->getStageObject(), stageScene->getXsheetHandle());
		}
		return true;
	}
	return false;
...
SchematicPort *StageSchematicSplinePort::searchPort(const QPointF &scenePos)
{
	...
	int i;
	for (i = 0; i < items.size(); i++) {
		StageSchematicSplinePort *linkingTo = dynamic_cast<StageSchematicSplinePort *>(items[i]);
		if (linkingTo)
			return linkingTo;
	}
	return 0;
...
StageSchematicNodeDock::StageSchematicNodeDock(StageSchematicNode *parent, bool isParentPort,
	...
	: QGraphicsItem(parent), m_parent(parent), m_isParentPort(isParentPort)
{
	...
	connect(m_port, SIGNAL(isReleased(const QPointF &)), this, SLOT(onPortReleased(const QPointF &)));
...
void StageSchematicNodeDock::hoverLeaveEvent(QGraphicsSceneHoverEvent *he)
{
	...
	int i;
	for (i = 0; i < m_port->getLinkCount(); i++)
		m_port->getLink(i)->updatePath();
}
	...
	for (i = 0; i < port->getLinkCount(); i++) {
		...
		if (startPort == port) {
			...
			if (port->getType() == 102 && startPos.x() < endPos.x())
				startPos = mapToScene(boundingRect().width(), boundingRect().height() * 0.5);
			link->updatePath(startPos, endPos);
		} else {
			if (port->getType() == 101 && startPos.x() < endPos.x())
				endPos = mapToScene(0, boundingRect().height() * 0.5);
			if (port->getType() == 102 && startPos.x() > endPos.x())
				endPos = mapToScene(boundingRect().width(), boundingRect().height() * 0.5);
			link->updatePath(startPos, endPos);
		}
	}
}
...
StageSchematicSplineDock::StageSchematicSplineDock(SchematicNode *parent, bool isParentPort,
												   eStageSchematicPortType type)
	: QGraphicsItem(parent), m_parent(parent), m_isParentPort(isParentPort)
{
	setFlag(QGraphicsItem::ItemIsSelectable, false);
...
void StageSchematicSplineDock::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
}
...
StageSchematicNode::StageSchematicNode(StageSchematicScene *scene, TStageObject *obj,
									   int width, int height, bool isGroup)
	: SchematicNode(scene), m_stageObject(obj), m_isGroup(isGroup)
{
	...
	if (m_isGroup)
		m_splineDock = new StageSchematicSplineDock(this, false, eStageSplineGroupPort);
	else
	...
	m_pathToggle = new SchematicToggle_SplineOptions(this,
													 QPixmap(":Resources/schematic_spline_aim_rhomb.svg"),
													 QPixmap(":Resources/schematic_spline_aim_square.svg"),
													 0);
	m_cpToggle = new SchematicToggle_SplineOptions(this,
												   QPixmap(":Resources/schematic_spline_cp.svg"),
												   0);
	...
	connect(m_pathToggle, SIGNAL(stateChanged(int)), scene, SLOT(onPathToggled(int)));
	...
	if (m_isGroup)
		m_parentDock = new StageSchematicNodeDock(this, true, eStageParentGroupPort);
	else
...
StageSchematicNode::~StageSchematicNode()
{
...
void StageSchematicNode::setSchematicNodePos(const QPointF &pos) const
{
	if (!m_stageObject->isGrouped() || m_stageObject->isGroupEditing())
		...
		const StageSchematicGroupNode *groupNode = dynamic_cast<const StageSchematicGroupNode *>(this);
		assert(groupNode);
...
StageSchematicNodePort *StageSchematicNode::makeChildPort(const QString &label)
{
...
StageSchematicNodePort *StageSchematicNode::makeParentPort(const QString &label)
{
...
void StageSchematicNode::updateChildDockPositions()
{
	...
	double height = size * m_childDocks.at(0)->getPort()->boundingRect().height();
	double y = (m_height * 0.5) + (height * 0.5);
	for (i = 0; i < size; i++) {
...
void StageSchematicNode::onHandleReleased()
{
	...
	if (updated) {
		...
		i = 0;
		while (labels.contains(QString(1, (char)('A' + i))))
			i++;
	...
	: StageSchematicNode(scene, pegbar, 90, 18)
{
	...
	QString toolTip = name == id ? m_name : m_name + " (" + QString::fromStdString(id) + ")";
	setToolTip(toolTip);
}
...
void StageSchematicPegbarNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
void StageSchematicPegbarNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *me)
{
	QRectF nameArea(5, 0, m_width - 10, 14);
...
void StageSchematicPegbarNode::updateChildDockPositions()
{
	int i, size = m_childDocks.size();
	double height = size * m_childDocks.at(0)->getPort()->boundingRect().height();
	double y = 9 + (height * 0.5);
	for (i = 0; i < size; i++) {
		y -= m_childDocks.at(0)->getPort()->boundingRect().height();
		m_childDocks[i]->setPos(m_width - 18, y);
		m_childDocks[i]->getPort()->updateLinksGeometry();
	}
}
...
StageSchematicTableNode::StageSchematicTableNode(StageSchematicScene *scene, TStageObject *pegbar)
	: StageSchematicNode(scene, pegbar, 90, 18)
{
...
void StageSchematicTableNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	StageSchematicNode::paint(painter, option, widget);
}
...
void StageSchematicTableNode::updateChildDockPositions()
{
	int i, size = m_childDocks.size();
	double height = size * m_childDocks.at(0)->getPort()->boundingRect().height();
	double y = 0;
...
StageSchematicColumnNode::StageSchematicColumnNode(StageSchematicScene *scene, TStageObject *pegbar)
	: StageSchematicNode(scene, pegbar, 90, 32), m_isOpened(true)
{
	...
	m_renderToggle = new SchematicToggle(this, QPixmap(":Resources/schematic_prev_eye.png"), SchematicToggle::eIsParentColumn);
	ret = ret && connect(m_renderToggle, SIGNAL(toggled(bool)), this, SLOT(onRenderToggleClicked(bool)));
	if (scene) {
		TXshColumn *column = scene->getXsheet()->getColumn(pegbar->getId().getIndex());
		if (column)
		...
		m_cameraStandToggle = new SchematicToggle(this,
												  QPixmap(":Resources/schematic_table_view.png"),
	...
	TXshColumn *column = scene->getXsheet()->getColumn(pegbar->getId().getIndex())->getLevelColumn();
	if (column) {
		TXshSimpleLevel *simpleLevel = column->getLevelColumn()->getCell(column->getLevelColumn()->getFirstRow()).getSimpleLevel();
		if (simpleLevel) {
			HookSet *hookSet = simpleLevel->getHookSet();
			for (int i = 0; i < hookSet->getHookCount(); i++) {
				Hook *hook = hookSet->getHook(i);
				if (hook && !hook->isEmpty()) {
					StageSchematicNodeDock *hookDock = new StageSchematicNodeDock(this, false, eStageChildPort);
					addPort(i + 2, hookDock->getPort());
					m_childDocks.append(hookDock);
					hookDock->getPort()->setHandle(QString::fromStdString("H" + toString(hook->getId() + 1)));
				}
			}
		}
	}
	...
	qreal y = 14;
	m_parentDock->setPos(0, y);
	for (int i = 0; i < m_childDocks.size(); i++) {
		m_childDocks[i]->setPos(m_width - m_childDocks[i]->boundingRect().width(),
								y - i * m_childDocks[i]->boundingRect().height());
	}
...
void StageSchematicColumnNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	StageSchematicNode::paint(painter, option, widget);
	TStageObjectId id = m_stageObject->getId();
	assert(id.isColumn());
	...
	QPointF pos(m_cameraStandToggle->pos() - QPointF(srcWidth + 1, -srcHeight + 3));
		...
		if (r1 >= r0) {
			...
			if (xl) {
				...
				if (Preferences::instance()->getColumnIconLoadingPolicy() == Preferences::LoadOnDemand)
					onDemand = stageScene->getColumnHandle()->getColumnIndex() != columnIndex;
				return IconGenerator::instance()->getIcon(xl, cell.m_frameId, false, onDemand);
			}
		}
	}
	return QPixmap();
...
void StageSchematicColumnNode::getLevelTypeAndName(int &ltype, QString &levelName)
{
	StageSchematicScene *stageScene = dynamic_cast<StageSchematicScene *>(scene());
	if (!stageScene) {
		ltype = NO_XSHLEVEL;
...
void StageSchematicColumnNode::updateChildDockPositions()
{
	int i, size = m_childDocks.size();
	double height = size * m_childDocks.at(0)->getPort()->boundingRect().height();
	double y = 14;
	for (i = 0; i < size; i++) {
		m_childDocks[i]->setPos(m_width - m_childDocks.at(0)->getPort()->boundingRect().width(), y);
		y += m_childDocks.at(0)->getPort()->boundingRect().height();
...
void StageSchematicColumnNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *me)
{
	StageSchematicScene *stageScene = dynamic_cast<StageSchematicScene *>(scene());
	if (!stageScene)
		return;
...
void StageSchematicColumnNode::onNameChanged()
{
	...
	TXshZeraryFxColumn *fxColumn = dynamic_cast<TXshZeraryFxColumn *>(column);
	if (fxColumn)
		TFxCommand::renameFx(fxColumn->getZeraryColumnFx(), m_name.toStdWString(), stageScene->getXsheetHandle());
	else {
		TStageObjectCmd::rename(id, m_name.toStdString(), stageScene->getXsheetHandle());
		update();
	}
...
void StageSchematicColumnNode::onRenderToggleClicked(bool isActive)
{
	...
	if (!stageScene)
		return;
	TXshColumn *column = stageScene->getXsheet()->getColumn(m_stageObject->getId().getIndex());
	if (column) {
...
StageSchematicCameraNode::StageSchematicCameraNode(StageSchematicScene *scene, TStageObject *pegbar)
	: StageSchematicNode(scene, pegbar, 90, 18)
{
...
void StageSchematicCameraNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	StageSchematicNode::paint(painter, option, widget);
}
...
void StageSchematicCameraNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *me)
{
	...
	if (nameArea.contains(me->pos())) {
		...
		QAction *cameraSettingsPopup = CommandManager::instance()->getAction("MI_CameraStage");
...
void StageSchematicCameraNode::updateChildDockPositions()
{
	int i, size = m_childDocks.size();
	double height = size * m_childDocks.at(0)->getPort()->boundingRect().height();
	double y = 0;
...
StageSchematicSplineNode::StageSchematicSplineNode(StageSchematicScene *scene, TStageObjectSpline *spline)
...
void StageSchematicSplineNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
void StageSchematicSplineNode::onClicked()
{
	if (m_dock->getPort()->getLinkCount() > 0) {
		StageSchematicNode *parentNode = dynamic_cast<StageSchematicNode *>(m_dock->getPort()->getLinkedNode(0));
...
void StageSchematicSplineNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *me)
{
	QRectF nameArea(14, 0, m_width - 15, 14);
	if (nameArea.contains(me->pos())) {
...
StageSchematicGroupNode::StageSchematicGroupNode(StageSchematicScene *scene, TStageObject *root,
												 const QList<TStageObject *> groupedObj)
	: StageSchematicNode(scene, root, 90, 18, true), m_root(root), m_groupedObj(groupedObj)
{
	int i;
...
void StageSchematicGroupNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
}

//--------------------------------------------------------

void StageSchematicGroupNode::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *me)
{
	QRectF nameArea(14, 0, m_width - 15, 14);
	if (nameArea.contains(me->pos())) {
...
void StageSchematicGroupNode::onNameChanged()
{
	...
	TStageObjectCmd::renameGroup(m_groupedObj, m_name.toStdWString(), false, stageScene->getXsheetHandle());
	update();
}

//--------------------------------------------------------

int StageSchematicGroupNode::getGroupId()
{
	return m_root->getGroupId();
}
