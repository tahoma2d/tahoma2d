...
TFxP cloneInputPort(const TFxP &currentFx)
{
  ...
  for (i=0; i<getInputPortCount(); ++i)
  {
		...
		if(inputFx)
		{
			if(TLevelColumnFx* affFx = dynamic_cast<TLevelColumnFx* >(inputFx))
...
void updateMaximumPageSize(QGridLayout *layout, int &maxLabelWidth, int &maxWidgetWidth, int &fieldsHeight)
{
	...
	for (int r = 0; r < layout->rowCount(); r++) {
		...
		QLabel *label = dynamic_cast<QLabel *>(layout->itemAtPosition(r, 0)->widget());
		QGroupBox *gBox = dynamic_cast<QGroupBox *>(layout->itemAtPosition(r, 0)->widget());
		if (label) {
		...
		if (pF) {
			...
			if (hLay) {
				...
				for (int i = 0; i < hLay->count(); i++) {
					...
					ParamField *hPF = dynamic_cast<ParamField *>(hLay->itemAt(i)->widget());
					if (hLabel) {
...
QSize ParamsPage::getPreferedSize()
{
	...
	return QSize(maxLabelWidth + maxWidgetWidth + 5 /* Spacing */ + 24 /* Margin２つ分 */,
...
void ParamsPageSet::setFx(const TFxP &currentFx, const TFxP &actualFx, int frame)
{
...
{
	QScrollArea *scrollAreaPage = dynamic_cast<QScrollArea *>(m_pagesList->currentWidget());
...
{
	QScrollArea *scrollAreaPage = dynamic_cast<QScrollArea *>(m_pagesList->widget(index));
	assert(scrollAreaPage);
	return dynamic_cast<ParamsPage *>(scrollAreaPage->widget());
}
...
void ParamViewer::setFx(const TFxP &currentFx, const TFxP &actualFx, int frame, ToonzScene *scene)
{
	if (!actualFx) {
