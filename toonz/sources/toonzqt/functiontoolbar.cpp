...
void FunctionToolbar::onFrameSwitched()
{
...
{
	if (m_frameHandle)
		m_frameHandle->setFrame(m_frameNavigator->getFrame());
}
...
void FunctionToolbar::setSelection(FunctionSelection *selection)
{
	if (m_selection != selection) {
		if (m_selection)
			disconnect(m_selection, SIGNAL(selectionChanged()), this, SLOT(onSelectionChanged()));
		m_selection = selection;
		if (m_selection)
			connect(m_selection, SIGNAL(selectionChanged()), this, SLOT(onSelectionChanged()));
	}
}
