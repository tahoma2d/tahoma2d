...
TStageObject *getRoot(const QList<TStageObject *> &objs, TStageObjectTree *pegTree)
{
	int i;
...
void keepSubgroup(QMap<int, QList<SchematicNode *>> &editedGroup)
{
	...
	for (it = editedGroup.begin(); it != editedGroup.end(); it++) {
		...
		for (i = 0; i < groupedNodes.size(); i++) {
			StageSchematicNode *node = dynamic_cast<StageSchematicNode *>(groupedNodes[i]);
			if (!node)
				continue;
...
void StageSchematicScene::onSelectionSwitched(TSelection *oldSel, TSelection *newSel)
{
		...
		if (!pegbar->isGrouped() || pegbar->isGroupEditing()) {
			...
			if (parent && parent->isGrouped()) {
				...
				if (m_GroupTable.contains(parentGroupId)) {
					StageSchematicGroupNode *groupeParentNode = m_GroupTable[parentGroupId];
...
StageSchematicGroupNode *StageSchematicScene::addStageGroupNode(QList<TStageObject *> objs)
{
	if (objs.isEmpty())
		return 0;
...
void StageSchematicScene::updatePosition(StageSchematicNode *node, const TPointD &pos)
{
...
void StageSchematicScene::updateEditedGroups(const QMap<int, QList<SchematicNode *>> &editedGroup)
{
	...
	for (it = editedGroup.begin(); it != editedGroup.end(); it++) {
		...
		while (it2 != editedGroup.end()) {
			StageSchematicNode *placedObj = dynamic_cast<StageSchematicNode *>(it2.value()[0]);
			StageSchematicNode *obj = dynamic_cast<StageSchematicNode *>(it.value()[0]);
			if (!placedObj || !obj) {
				it2++;
				continue;
			}
		...
		StageSchematicGroupEditor *node = addEditedGroupedStageSchematicNode(it.key(), it.value());
...
void StageSchematicScene::updateNestedGroupEditors(StageSchematicNode *node, const QPointF &newPos)
{
	if (!node)
		return;
	QStack<int> groupIdStack = node->getStageObject()->getGroupIdStack();
	if (groupIdStack.isEmpty())
		return;
	int i;
	QRectF rect;
...
void StageSchematicScene::updatePositionOnResize(TStageObject *obj, bool maximizedNode)
{
...
void StageSchematicScene::updateSplinePositionOnResize(TStageObjectSpline *spl, bool maximizedNode)
{
...
StageSchematicSplineNode *StageSchematicScene::addSchematicSplineNode(TStageObjectSpline *spline)
{
...
void StageSchematicScene::reorderScene()
{
	placeNodes();
}
...
void StageSchematicScene::placeSplineNode(StageSchematicSplineNode *splineNode)
{
	double xFirstPos = m_firstPos.x - 500;
	double yFirstPos = m_firstPos.y + 500;
	int hStep = (m_showLetterOnPortFlag) ? 150 : 120;
...
void StageSchematicScene::onRemoveSpline()
{
...
TXsheet *StageSchematicScene::getXsheet()
{
...
void StageSchematicScene::onCurrentObjectChanged(const TStageObjectId &id, bool isSpline)
{
	...
	if (m_frameHandle->isEditingLevel())
		return;
...
void StageSchematicScene::contextMenuEvent(QGraphicsSceneContextMenuEvent *cme)
{
...
void StageSchematicScene::onSelectionChanged()
{
	...
	for (it = slcItems.begin(); it != slcItems.end(); it++) {
		StageSchematicGroupNode *groupNode = dynamic_cast<StageSchematicGroupNode *>(*it);
		StageSchematicNode *node = dynamic_cast<StageSchematicNode *>(*it);
		if (groupNode) {
			QList<TStageObject *> objs = groupNode->getGroupedObjects();
			for (i = 0; i < objs.size(); i++)
				m_selection->select(objs[i]->getId());
			highlightLinks(groupNode, true);
		} else if (node) {
			m_selection->select(node->getStageObject()->getId());
			highlightLinks(node, true);
		}
		StageSchematicSplineNode *spline = dynamic_cast<StageSchematicSplineNode *>(*it);
		if (spline) {
...
void StageSchematicScene::onEditGroup()
{
	...
	for (i = 0; i < objs.size(); i++) {
		...
		if (obj && obj->isGrouped() && !obj->isGroupEditing())
			obj->editGroup();
	}
	updateScene();
}
