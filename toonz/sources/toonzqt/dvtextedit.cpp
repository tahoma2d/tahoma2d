...
void DvTextEdit::createActions()
{
	...
	m_italicAction = new QAction(createQIconOnOffPNG("italic"), tr("Italic"), this);
	m_italicAction->setCheckable(true);
	connect(m_italicAction, SIGNAL(triggered()), this, SLOT(setTextItalic()));

	m_underlineAction = new QAction(createQIconOnOffPNG("underline"), tr("Underline"), this);
	m_underlineAction->setCheckable(true);
	connect(m_underlineAction, SIGNAL(triggered()), this, SLOT(setTextUnderline()));
...
void DvTextEdit::mousePressEvent(QMouseEvent *e)
{
	...
	if (m_miniToolBarEnabled && m_miniToolBar->isVisible())
		hideMiniToolBar();
}
...
{
	m_fontComboBox->setCurrentIndex(m_fontComboBox->findText(QFontInfo(f).family()));
	m_sizeComboBox->setCurrentIndex(m_sizeComboBox->findText(QString::number(f.pointSize())));
...
void DvTextEdit::onCursorPositionChanged()
{
	alignmentChanged(alignment());
}
