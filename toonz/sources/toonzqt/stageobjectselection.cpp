...
StageObjectSelection::StageObjectSelection()
	: m_xshHandle(0), m_objHandle(0), m_fxHandle(0), m_pastePosition(TConst::nowhere)
{
}
...
{
	TStageObjectCmd::deleteSelection(m_selectedObjects.toVector().toStdVector(),
									 m_selectedLinks.toStdList(), m_selectedSplines.toStdList(),
									 m_xshHandle, m_objHandle, m_fxHandle);
}
...
void StageObjectSelection::select(int id)
{
	m_selectedSplines.append(id);
}
...
void StageObjectSelection::unselect(int id)
{
	...
	if (index >= 0)
		m_selectedSplines.removeAt(index);
}
...
{
	QPair<TStageObjectId, TStageObjectId> boundingObjects = getBoundingObjects(link);
...
QPair<TStageObjectId, TStageObjectId> StageObjectSelection::getBoundingObjects(SchematicLink *link)
{
	...
	if (link) {
		...
		if (port->getType() == 100 && link->getEndPort()->getType() == 100) {
			StageSchematicNode *node = dynamic_cast<StageSchematicNode *>(port->getNode());
			if (node)
				boundingObjects = getBoundingObjects(port, port);
			else
				boundingObjects = getBoundingObjects(link->getEndPort(), link->getEndPort());
																			   ...
																			   SchematicPort *outputPort)
{
	...
	StageSchematicNode *outputNode = dynamic_cast<StageSchematicNode *>(outputPort->getNode());
...
bool StageObjectSelection::isConnected() const
{
	...
	for (i = 0; i < m_selectedObjects.size(); i++) {
		...
		canBeGrouped = canBeGrouped && obj->getEditingGroupId() == parentObj->getEditingGroupId();
	}
...
{
	if (m_selectedObjects.size() <= 1 || !isConnected())
		return;
...
void StageObjectSelection::ungroupSelection()
{
	...
	int i;
	for (i = 0; i < m_selectedObjects.size(); i++) {
		int groupId = objTree->getStageObject(m_selectedObjects[i], false)->getGroupId();
		if (groupId > 0)
			idSet.insert(objTree->getStageObject(m_selectedObjects[i], false)->getGroupId());
	}
...
void StageObjectSelection::pasteSelection()
{
	...
	const StageObjectsData *objData = dynamic_cast<const StageObjectsData *>(data);
	if (!objData)
		return;
	std::set<int> indexes;
