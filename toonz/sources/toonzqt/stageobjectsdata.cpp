...
//! Returns whether the fx is terminal or has a not-generable, terminal-upstream parent.
bool isColumnSelectionTerminalFx(TFx *fx, TFxSet *terminalFxs, const std::set<TFx *> &columnFxs)
{
...
public:
	...
	//! Restores the object data in the xsheet (TStageObjectDataElement -> TXsheet) and
...
//! Class used to clone a column stage object's relational data inside the stage schematic.
...
TColumnDataElement *TColumnDataElement::clone() const
{
	...
	if (element->m_column)
		element->m_column = element->m_column->clone();
...
//! Class used to clone a camera stage object's relational data inside the stage schematic.
...
void TSplineDataElement::storeSpline(TStageObjectSpline *spline, int flag)
{
	...
	m_spline->addRef();
	if (flag & eResetFxDagPositions)
		m_spline->setDagNodePos(TConst::nowhere);
}
...
TStageObjectSpline *TSplineDataElement::restoreSpline(int flag) const
{
	...
	if (flag & eResetFxDagPositions)
		spline->setDagNodePos(TConst::nowhere);
...
bool StageObjectsData::checkCircularReferences(TXsheet *xsh) const
{
	int i, elementsCount = m_elements.size();
	for (i = 0; i < elementsCount; ++i) {
		TColumnDataElement *columnElement = dynamic_cast<TColumnDataElement *>(m_elements[i]);
		if (!columnElement)
			continue;
...
void StageObjectsData::storeColumns(const std::set<int> &columnIndexes, TXsheet *xsh, int fxFlags)
{
...
void StageObjectsData::storeSplines(const std::list<int> &splineIds, TXsheet *xsh, int fxFlags)
{
