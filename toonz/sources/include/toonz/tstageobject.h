...
/*!
  TStageObject is the Toonz class representing animatable scene objects like columns, pegbars,
  tables and cameras (pretty much every object in Toonz's stage schematic).

\par Description

  A TStageObject contains a common set of editable parameters (coordinates, angles and more)
  that define the object's state along an xsheet timeline. These parameters are in the form of
  animatable TDoubleParam instances and can be accessed directly through the getParam() method.
  \n\n
  A notable responsibility of the TStageObject class is that of building the keyframes data that
  will be shown in xsheet viewes (the key icons). The object's keyframes are built and cached
  each time one of its associated parameters is changed.
  \n\n
  Furthermore, a TStageObject instance is typically a node in a tree-like hierarchycal structure
  of class TStageObjectTree which is used to define relative object placement between objects.

\par Creation

  Client code should access TStageObject instances through its TXsheet owner, like:
  ...
  The actual creation of new TStageObject instances is currently a responsibility of the
...
public:
  ...
  */
	...
	enum Channel {
		...
		T_Scale,
		T_Path,   //!< point position along the spline, as a percentage of the length of the spline
...
public:
  ...
  */
	...
	//! Returns the \a id of the object's parent. Use the owner xsheet to access the parent object.
  ...
  */
	...
	inline bool isAimEnabled() const { return (m_status & STATUS_MASK) == PATH_AIM; }
	inline bool isPathEnabled() const
	{
		return (m_status & STATUS_MASK) == PATH_AIM || (m_status & STATUS_MASK) == PATH;
	}
  ...
  */
	...
	// n.b. accesso diretto alla variabili interne center e offset. serve per undo/redo
  ...
  */
	...
	/*
	This function checks if a list of keyframes can be moved without deleting an existing keyframe.
  */
  ...
  */
	...
	void assignParams(const TStageObjectParams *src, bool doParametersClone = true);
