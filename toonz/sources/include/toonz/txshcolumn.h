...
\n This is an abstract base class inherited by the concrete classes
   ...
   TXshColumn allows to manage generic column status getStatusWord() and setStatusWord.
   Particularly it's possible to know if column is a mask isMask(), is lock isLocked(),
   or if it's view is in modality preview or camera stand, isCamstandVisible() and
   isPreviewVisible(). It's besides possible ot change this settings using setMask(),
...
\n This is an abstract base class inherited by the concrete classes
   ...
   isEmpty(), to know column range getRange(), row count getRowCount(), last and first
   not empty frame getMaxFrame(), getFirstRow().
   Specally class allows to change cells, getCells(), setCells(), or individually cell,
   getCell(), setCell(); allows to insert insertEmptyCells(), remove removeCells(),
