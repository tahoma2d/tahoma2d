...
public:
	...
	TProject *getProject() const; //!< Returns a pointer to the project holding a scene instance.
	void setProject(TProject *);  //!< Associates a scene to a project.

	void setScenePath(const TFilePath &p);				   //!< Sets the scene's file path.
	TFilePath getScenePath() const { return m_scenePath; } //!< Returns the scene's file path.
	...
	bool isUntitled() const; //!< Returns whether the scene path is empty, or the scene
							 //!  is explicitly untitled.  \sa  Member function setUntitled().

	TXsheet *getXsheet() const;	//!< Returns a pointer to the scene's \a current xsheet.
	TXsheet *getTopXsheet() const; //!< Returns the \a topmost xsheet in the scene's xsheet hierarchy.

	ChildStack *getChildStack() const { return m_childStack; } //!< Returns a pointer to the xsheets hierarchy.

	TSceneProperties *getProperties() const //!< Returns a pointer to the scene's properties.
	{
		return m_properties;
	}

	TLevelSet *getLevelSet() const { return m_levelSet; } //!< Returns a pointer to the scene's <I>Level Set</I>.

	void clear(); //!< Clears the scene.

	void save(const TFilePath &path, TXsheet *subxsheet = 0);		   //!< Saves the scene (or a sub-xsheet) at the
																	   //!  specified path.
	void loadTnzFile(const TFilePath &path);						   //!< Loads scene data from file, \a excluding the
																	   //!  associated project and the scene resources.
	void loadNoResources(const TFilePath &path);					   //!< Loads a scene \a without loading its resources.
	void loadResources(bool withProgressDialog = false);			   //!< Loads the scene resources.
	void load(const TFilePath &path, bool withProgressDialog = false); //!  Loads a scene from file.

	/*! \return   The \a coded path to be used for import. */

	TFilePath getImportedLevelPath(const TFilePath path) const; //!< Builds the path to be used during a level import
																//!< operation.
	...
	bool convertLevelIfNeeded(TFilePath &levelPath); //!< Checks if the given levelPath is a file path of an
	...
	TXshLevel *loadLevel(const TFilePath &actualPath,
						 const LevelOptions *options = 0, //!< Loads a level from the specified \a decoded path.
						 std::wstring levelName = L"",
						 const std::vector<TFrameId> &fIds = std::vector<TFrameId>());

	/*!
    Performs a camera-stand render of the specified xsheet (or the main one in case none was
    specified), on its currently selected camera, and returns the result on top of the specified
    32-bit raster.
  */
	void renderFrame(const TRaster32P &ras, int row, const TXsheet *xsh = 0, bool checkFlags = true) const;

	/*!
    Performs a camera-stand render of the specified xsheet in the specified placedRect,
    with known world/placed reference change - and returns the result on top of the specified
    32-bit raster.
  */
