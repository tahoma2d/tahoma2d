...
DVAPI TRect addInkStroke(const TToonzImageP &ti, TStroke *stroke,
						 int inkId, bool selective, bool filled, TRectD clip = TRectD(), bool doAntialiasing = true);
...
DVAPI TRect changeColorStroke(const TToonzImageP &ti, const ChangeColorStrokeSettings &settings);

//-------------------------------------------------------------------

// Cancella gli ink in corrispondenza della stroke.
// Se maskInkId!=-1, cancella solo dove il vecchio ink e' uguale a maskInkId.
//void eraseInkStroke(const TToonzImageP &ti, TStroke *stroke, int maskInkId, TRectD clip=TRectD());

// Cancella l'ink e/o il paint in corrispondenza della stroke, se maskStyleId!=-1, solo se
// sono uguali a maskStykeId (indipendentemente).
//DVAPI  void eraseInkAndPaint(const TToonzImageP &ti, TStroke *stroke,
//                               int maskId, bool inkOnly, bool paintOnly, TRectD clip=TRectD());

// Cambia tutti gli ink della clip area con inkId.
//void changeInkRect    (const TToonzImageP &ti, const TRectD &clip, int inkId);
