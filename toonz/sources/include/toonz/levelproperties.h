...
public:
	...
	int m_subsampling,	 //!< <TT>[default: 1]</TT> Image subsampling value (see
		...
		m_antialias;	   //!< <TT>[default: 0]</TT> Antialias amount (\p 0 meaning
						   //!  no antialias).
	DpiPolicy m_dpiPolicy; //!< <TT>[default: DP_ImageDpi]</TT> Dpi policy to be adpoted.
...
public:
	...
	void setImageRes(const TDimension &d); //!< Sets the level's known image resolution.
	TDimension getImageRes() const;		   //!< Returns the level's known image resolution.

	void setBpp(int bpp); //!< Sets the level's known bits-per-pixel.
	int getBpp() const;   //!< Returns the level's known bits-per-pixel.

	void setHasAlpha(bool hasAlpha) //!  Sets whether the level has an alpha channel.
	{
		m_hasAlpha = hasAlpha;
	}
	...
	void setCreator(const std::string &creator) //!  Sets the level's creator string.
	{
		m_creator = creator;
	}
	...
	bool doPremultiply() const //!  Returns whether premultiplication must be applied.
	{
