...
//! Stage::Visitor is the abstract class model related to <I> scene visiting <\I>.
/*!
  Iterating a scene to perform some task on its content is a typical task in Toonz.
  Scene visiting is here intended as an implementation of the <I> visitor-visitable <\I>
  pattern applied to Toonz scenes, where the \a visitable objects are of type Stage::Player,
  and visitors are reimplemented from the Stage::Visitor class.
\n\n
  The Stage::Player class is the rough equivalent of a level at a specific scene frame (the term
  \a Player being more legacy rather than having a definite sense I guess), which in particular
  can be seen as an \a image occurring in scene compositing. Stage::Player actually have a lot
  more insights about onion skinning, image placement, and other stuff - be sure to check it
  thorough.
\n\n
  When visiting a scene, the Players are supplied to Stage::Visitor instances sorted in compositing
...
public:
	...
	virtual void onImage(const Player &player) = 0; //!< The \a visitation function.

	// I've not checked the actual meaning of the methods below. They are unused in Toonz, but *are*
	// used in Toonz derivative works such as Tab or LineTest. They deal with OpenGL stencil buffer.
...
// This class (sadly?) disappears from Stage::Visitor's visibility - it's just a visit() option.
...
//! Stage::RasterPainter is the object responsible for drawing scene contents on a standard
//! Toonz SceneViewer panel.
/*!
  This class performs standard <I> camera-stand <\I> scene renderization.
  It renders every supported Toonz image type (including Plastic's texturized mesh-based
...
private:
	...
	struct VisualizationOptions {
		bool m_singleColumnEnabled; //!< Whether only current column should be rendered
