...
//! Stage::Player is the class that gets visited during standard (simplified) scene drawing.
/*!
  Whenever a Toonz scene needs to be drawn on screen (a simplified form of scene rendering,
  without fxs and other stuff), a SceneBuilder instance traverses the scene content and
  compiles a sequence of Stage::Player objects (ordered by Z and stacking order).
\n\n
  A Stage::Player consists roughly of all the data necessary to draw an image of a level, in
  particular the image itself.
  Further data include the placement affine and the scaling dpi affine that can be composed
  to get the actual affine the image must be rendered with. Other data is available; check
  the public class members for details.
\n\n
  Once the sequence has been completed, a heir of the Stage::Visitor class can then visit
...
public:
	...
	int m_frame;	//!< The player's xhseet row (could be not the current, due to onion skin?)

	bool m_isPlaying; //!< Whether this player is in a 'play >' status or rather 'paused'
