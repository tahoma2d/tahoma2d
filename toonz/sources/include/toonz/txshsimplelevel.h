...
public:
	...
	LevelProperties *getProperties() const //!< Returns a pointer to the \a owned level properties.
	{
		return m_properties.get();
	}
	void clonePropertiesFrom(const TXshSimpleLevel *oldSl);

	TPalette *getPalette() const;		//!< Returns a pointer to the \a shared level palette.
	...
	TFrameId getFirstFid() const; //!< Returns the first level frame's id; roughly equivalent to
								  //!  <TT>index2fid(0)</TT>.
	TFrameId getLastFid() const;  //!< Returns the last level frame's id; roughly equivalent to
  ...
  */
	...
	//! Analyzes the level's frames table and returns the alleged entries \a step - ie the distance
	//! from each entry to the next.
	int guessStep() const;

	void setFrame(const TFrameId &fid, const TImageP &img);

	TImageP getFrame(const TFrameId &fid, UCHAR imgManagerParamsMask, int subsampling) const;
	TImageP getFrame(const TFrameId &fid, bool toBeModified) const
	{
		return getFrame(fid, toBeModified ? ImageManager::toBeModified : ImageManager::none, 0);
	}
	TImageP getFullsampledFrame(const TFrameId &fid, UCHAR imgManagerParamsMask) const;
  ...
  */
	...
	TPointD getImageDpi(const TFrameId &fid = TFrameId::NO_FRAME, int frameStatus = -1);
