...
Inherits \b TSmartObject and \b TPersist.
		...
		A \b column \b set contains all xsheet column. A collection of function, concern column
		set, allows to manage xsheet column. It's possible to know xsheet column number,
		getColumnCount(), know first not empty column index, getFirstFreeColumnIndex(),
...
class DVAPI TXsheet : public TSmartObject, public TPersist
{
...
public:
	...
	*/
	...
	/*! Returns true if cells contained in rect delimited by first row \b \e r0, last row \b \e r1
		and first column \b \e c0, last column \b \e c1 are empty; otherwise, returns false.
	*/
	bool isRectEmpty(int r0, int c0, int r1, int c1) const;
	/*! Returns the \b TXshCell cell in row identified by index \b \e row and column identified by
		index \b \e col. If column \b TXshColumnP in \b \e col is empty return an empty cell.
	...
	*/
	...
	/*! If column identified by index \b \e col is not empty, is a \b TXshCellColumn and is not
		locked, this method removes \b \e rowCount cells starting from \b \e row, it calls
		TXshCellColumn::removeCells(). It removes cells without shift remaining cells.
...
*/ void clearCells(int row, int col, int rowCount = 1);
	...
	*/
	...
	/*! Sets the level set \b \e levels with all current xsheet levels used, it includes
		in \b \e levels sub-xsheet levels too.
		\sa isLevelUsed()
	*/
	void getUsedLevels(std::set<TXshLevel *> &levels) const;
	/*! Returns true if \b \e level is used in current xsheet or in sub-xsheet, otherwise returns
		false. It verifies if \b \e level is contained in level set \b getUsedLevels().
	*/
	bool isLevelUsed(TXshLevel *) const;
	/*! The method retrieves a pegbar \b TStageObject with the passed \b \e id \b TStageObjectId
	...
	*/
	...
	/*! The method retrieves the pegbar tree \b TStageObjectTree contained in struct \b TXsheetImp.
		\sa getStageObject()
	*/
	...
	*/
	...
	/*! The cells, contained in rect delimited by first row \b \e r0, last row \b \e r1 and
		first column \b \e c0, are appended at the end of the rect in a reversed order.
	...
	*/
	...
	\n	If returns true, the cells, contained in rect delimited by first row \b \e r0, last row
		\b \e r1 and first column \b \e c0, are repeated as if filling the numbering gap between
		two subsequence cell. For example if the command is applied to two cell with
	...
	*/
	...
	/*! The cells, contained in rect delimited by first row \b \e r0, last row \b \e r1 and
		first column \b \e c0, are repeated in order to have a step \b \e type. The duplicated
		cells will be inserted by shifting the other down.
	*/
  ...
  */
	...
	/*! Only one cell each step \b \e type, of the cells, contained in rect delimited by
		first row \b \e r0, last row \b \e r1 and first column \b \e c0, are preserved,  the
		others are deleted.
	*/
	...
	/*! Exposes level \b \e xl in xsheet starting from cell identified by \b \e row and \b \e col.
	...
	*/
	...
	/*! Clears xsheet calling \b clearAll() and sets all xsheet elements, contained
		in \b TXsheetImp, to information contained in \b TIStream \b \e is.
		\sa saveData()
	*/
	void loadData(TIStream &is);
	/*! Save all xsheet elements information, contained in \b TXsheetImp, in \b TOStream \b \e os.
		\sa loadData()
	*/
	void saveData(TOStream &os);
	/*! Inserts an empty column in \b \e index calling \b insertColumn().
	*/
	void insertColumn(int index, TXshColumn::ColumnType type = TXshColumn::eLevelType);
	/*! Insert \b \e column in column \b \e index. Insert column in the column set, in the
		pegbarTree \b TStageObjectTree contained in TXsheetImp and if column fx \b TFx is
	...
	*/
	...
	/*! Returns a pointer to the \b TXshColumn identified in xsheet by index \b \e index.
	*/
	TXshColumn *getColumn(int index) const;
	/*! Returns xsheet column count, i.e the number of xsheet column used, it calls
	...
	*/
	...
	/*! Returns a pointer to object \b FxDag contained in \b TXsheetImp, this object
		allows the user to manage all fx dag, i.e. all element of fx schematic.
	*/
	...
	*/
	...
	/*! Returns a pointer to \b ToonzScene contained in \b TXsheetImp, that is the scene to
