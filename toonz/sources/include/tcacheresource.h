...
<li> A cache resource has a textual description of its content that we'll call 'name'.
It is the associative key used to retrieve a resource among all the others stored
in the cache. <\li>
<li> The spatial domain of a cache resource is the entire plane, meaning that access
...
<li> For simplicity, the pixel geometry of a cache is coherent to the plane origin - that is,
pixel corners always have integer coordinates. <\li>
<li> Filled resources have a <b> raster type <\b>, specifying the type of rasters that are
...
<li> Cache resources provide reference counters to either the entire resource and each
piece of its internal memory model, in order to perform automatic release of unnecessary
data. <\li>
<li> The resource provides an accessible resume of its currently filled areas, in the form of
a QRegion. <\li>
