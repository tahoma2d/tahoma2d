...
typedef struct
	{
	LPIXEL *buffer; /* colormap buffer for <= 16 bits or for 32 bits (otherwise NIL) */
...
#define RASRASRAS(type1, type2, type3) \
	RASRAS(RASRAS((type1), (type2)), (type3))
 ...
 */
TNZAPI int create_subraster(RASTER *rin, RASTER *rout, int x0, int y0,
							int x1, int y1);
...
TNZAPI void rop_subimage_to_raster(IMAGE *img,
 ...
 */
TNZAPI void rop_copy(RASTER *rin, RASTER *rout,
 ...
 */
TNZAPI void rop_copy_extra(RASTER *rin, RASTER *rout,
						   int x1, int y1, int x2, int y2, int newx, int newy);
 ...
 */
TNZAPI void rop_copy_90(RASTER *rin, RASTER *rout,
						int x1, int y1, int x2, int y2, int newx, int newy,
...
TNZAPI void rop_shrink(RASTER *rin, RASTER *rout,
					   int x1, int y1, int x2, int y2, int newx, int newy,
					   int factor);
 ...
 */
TNZAPI void rop_zoom_out(RASTER *rin, RASTER *rout,
						 int x1, int y1, int x2, int y2, int newx, int newy,
						 int abs_zoom_level);
 ...
 */
TNZAPI void rop_zoom_out_90(RASTER *rin, RASTER *rout,
							int x1, int y1, int x2, int y2, int newx, int newy,
...
TNZAPI void rop_ovr_gr_2(RASTER *rbot, RASTER *rtop, RASTER *rovr, LPIXEL color);
 ...
 */
TNZAPI void rop_ovr_patches(RASTER *rbot, UCHAR botmask,
							RASTER *rtop, UCHAR topmask, RASTER *rovr);
 ...
 */
TNZAPI void rop_conv_3(RASTER *rin, RASTER *rout, int dx, int dy, double conv[]);
 ...
 */
TNZAPI void rop_conv_3_i(RASTER *rin, RASTER *rout, int dx, int dy, double conv[]);
 ...
 * onto (cxout,cyout)
 */
TNZAPI void rop_rotate(RASTER *rin, RASTER *rout, double angle,
					   double cxin, double cyin, double cxout, double cyout);
...
TNZAPI void get_flt_fun_rad(FLT_TYPE flt_type,
							double (**flt_fun)(double), double *flt_rad);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_resample(RASTER *rin, RASTER *rout,
						 AFFINE aff, FLT_TYPE flt_type, double blur);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_resample_tcm(RASTER *rin, RASTER *rout,
							 AFFINE aff, FLT_TYPE flt_type, double blur,
							 TCM_INFO tcm_info);
...
TNZAPI void rop_fill_cmap_ramp(RASTER *ras, TCM_INFO info,
...
TNZAPI void rop_fill_cmap_colramp(RASTER *ras, TCM_INFO info,
								  LPIXEL color, int color_index,
								  int already_premultiplied);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_fill_cmap_penramp(RASTER *ras, TCM_INFO info,
								  LPIXEL pencil, int pencil_index,
								  int already_premultiplied);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_custom_fill_cmap_ramp(RASTER *ras, TCM_INFO info,
									  LPIXEL color, LPIXEL pencil,
									  int color_index, int pencil_index,
...
TNZAPI void rop_fill_cmap_buffer(RASTER *ras, TCM_INFO info,
								 LPIXEL *color, LPIXEL *pencil,
								 int already_premultiplied);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_fill_cmap_colbuffer(RASTER *ras, TCM_INFO info,
									LPIXEL *color, int already_premultiplied);

/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

TNZAPI void rop_fill_cmap_penbuffer(RASTER *ras, TCM_INFO info,
...
TNZAPI void rop_lineart_fullcolor(RASTER *r, LPIXEL *colors, int num_colors, TBOOL only_edges);
