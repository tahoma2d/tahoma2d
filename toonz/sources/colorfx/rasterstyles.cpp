...
void TAirbrushRasterStyle::loadData(TInputStreamInterface &is)
{
	is >> m_blur;
}
...
{
	if (m_blur > 0.0)
		TRop::blur(params.m_r, params.m_r, m_blur, 0, 0, true);
...
namespace
{
bool inline doIsBorderPix(int index, TPixelCM32 *pix, TPixelGR8 *pixGr, int paint)
{
	return (pix->getPaint() != paint && (pix->isPurePaint() || pixGr->value != 0));
}
...
void TBlendRasterStyle::makeIcon(const TDimension &d)
{
	...
	static TRasterP normalIc;
	if (normalIc == TRasterP())
		TImageReader::load(dir + "blend.bmp", normalIc);
...
void TNoColorRasterStyle::makeIcon(const TDimension &d)
{
	...
	static TRasterP normalIc;
	if (normalIc == TRasterP())
		TImageReader::load(dir + "markup.bmp", normalIc);
