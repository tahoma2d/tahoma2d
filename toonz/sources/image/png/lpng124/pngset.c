				   ...
				   int num_text)
{
	...
	png_debug1(1, "in %s storage function\n", (png_ptr->chunk_name[0] == '\0' ? "text" : (png_const_charp)png_ptr->chunk_name));
...
{
	if (png_ptr != NULL && info_ptr != NULL && chunk >= 0 && chunk < (int)info_ptr->unknown_chunks_num)
...
png_permit_empty_plte(png_structp png_ptr, int empty_plte_permitted)
{
	...
	if (png_ptr == NULL)
		return;
	png_ptr->mng_features_permitted = (png_byte)((png_ptr->mng_features_permitted & ((png_byte)(~(PNG_FLAG_MNG_EMPTY_PLTE)))) |
												 ((empty_plte_permitted & (png_byte)PNG_FLAG_MNG_EMPTY_PLTE)));
}
#endif
