...
struct Border {
	...
	Border() : m_x0((std::numeric_limits<int>::max)()), m_y0(m_x0), m_x1(-m_x0), m_y1(m_x1) {}
	...
	{
		m_points.clear();
		m_x0 = (std::numeric_limits<int>::max)(), m_y0 = m_x0, m_x1 = -m_x0, m_y1 = m_x1;
	}
};
...
template <typename PixelSelector>
IsolatedReader<PixelSelector>::IsolatedReader(const PixelSelector &selector, int sizeTol)
...
template <typename PixelSelector>
void IsolatedReader<PixelSelector>::openContainer(const RasterEdgeIterator<PixelSelector> &it)
{
...
template <typename PixelSelector>
void IsolatedReader<PixelSelector>::addElement(const RasterEdgeIterator<PixelSelector> &it)
{
	if (!m_ok)
		return;
	m_ok = (it.leftColor() == m_selector.transparent());
...
template <typename PIXEL, typename CHANNEL>
void doDespeckleRGBM(const TRasterPT<PIXEL> &ras, int sizeThreshold, bool transparentIsWhite)
{
	InkSelectorRGBM<PIXEL, CHANNEL> selector(transparentIsWhite);
	IsolatedReader<InkSelectorRGBM<PIXEL, CHANNEL>> reader(selector, sizeThreshold);
	ReplacePainter<PIXEL> painter(ras, transparentIsWhite ? PIXEL::White : PIXEL::Transparent);
...
void doDespeckleCM32(const TRasterPT<TPixelCM32> &ras, int sizeThreshold, bool check)
{
...
template <typename PixelSelector>
void FillingReader<PixelSelector>::openContainer(const RasterEdgeIterator<PixelSelector> &it)
{
	DespecklingReader::openContainer(it.pos());
}
...
template <typename PixelSelector>
void FillingReader<PixelSelector>::addElement(const RasterEdgeIterator<PixelSelector> &it)
{
	DespecklingReader::addElement(it.pos());
}
