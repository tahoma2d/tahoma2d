...
void TAutocloser::Imp::compute(vector<Segment> &closingSegmentArray)
{
	...
	try {
		...
		m_bRaster = braux->extract(TRect(2, 2, braux->getLx() - 3, braux->getLy() - 3));
		m_bRaster->lock();
...
TAutocloser::TAutocloser(const TRasterP &r)
{
	m_imp = new Imp(r);
}
...
void TAutocloser::setInkIndex(int inkIndex)
{
	m_imp->m_inkIndex = inkIndex;
}

//-------------------------------------------------

int TAutocloser::getInkIndex() const
{
	return m_imp->m_inkIndex;
}
