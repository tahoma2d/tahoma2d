...
TStageObject *TStageObjectTree::getStageObject(const TStageObjectId &id, bool create)
{
	std::map<TStageObjectId, TStageObject *> &pegbars = m_imp->m_pegbarTable;
	...
	if (it != pegbars.end()) {
		...
		if (id.isColumn()) {
			...
			if (index > 0)
				getStageObject(TStageObjectId::ColumnId(index - 1));
...
void TStageObjectTree::insertColumn(int index)
{
	...
	for (int k = 0; k < index; k++)
		getStageObject(TStageObjectId::ColumnId(k));

	std::map<TStageObjectId, TStageObject *> &pegbars = m_imp->m_pegbarTable;

	std::vector<std::pair<TStageObjectId, TStageObject *>> tmp(pegbars.begin(), pegbars.end());
...
void TStageObjectTree::loadData(TIStream &is)
{
	...
	while (is.matchTag(tagName)) {
		if (tagName == "splines") {
			...
			TStageObject *pegbar = dynamic_cast<TStageObject *>(getStageObject(id, true));
			if (!pegbar)
...
int TStageObjectTree::getCameraCount() const
{
	return m_imp->m_cameraCount;
}

//-----------------------------------------------------------------------------

int TStageObjectTree::getNewGroupId()
{
	return ++m_imp->m_groupIdCount;
}
...
int TStageObjectTree::getSplineCount() const
{
	return m_imp->m_splines.size();
}
...
void TStageObjectTree::createGrammar(TXsheet *xsh)
{
	...
	for (it = m_imp->m_pegbarTable.begin();
		...
		if (const PlasticSkeletonDeformationP &sd = obj->getPlasticSkeletonDeformation())
...
{
	if (m_imp->m_grammar)
		param->setGrammar(m_imp->m_grammar);
}
