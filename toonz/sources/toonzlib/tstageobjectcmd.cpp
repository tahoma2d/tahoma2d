...
public:
	...
	{
		m_spline->addRef();
		TStageObjectTree *pegbarTree = m_xshHandle->getXsheet()->getStageObjectTree();
		for (int i = 0; i < pegbarTree->getStageObjectCount(); i++) {
			TStageObject *pegbar = pegbarTree->getStageObject(i);
			if (pegbar->getSpline() == m_spline)
...
public:
	...
	void undo() const
	{
		...
		if (!object)
			return;
		object->setSpline(m_spline);
		if (m_objHandle->getObjectId() == m_id)
			m_objHandle->setIsSpline(true);
		m_xshHandle->notifyXsheetChanged();
	}
...
public:
	...
	void undo() const
	{
		...
		for (it = m_columnFxConnections.begin(); it != m_columnFxConnections.end(); it++) {
				...
				TObjectHandle *objHandle, bool doUndo = true)
{
	...
	if (link.first == link.second) { //is a link connecting a spline with an object
		...
		assert(spline);
		object->setSpline(0);
		if (objHandle->getObjectId() == link.first)
			objHandle->setIsSpline(false);
		if (doUndo) {
			TUndo *undo = new RemoveSplineLinkUndo(link.first, spline, xshHandle, objHandle);
			TUndoManager::manager()->add(undo);
		}
	} else {
		TStageObject *object = objTree->getStageObject(link.first, false);
...
class ResetPositionUndo : public TUndo
{
	...
	void saveKeyframes(std::vector<TDoubleKeyframe> &keyframes, const TDoubleParam *param)
	{
		...
		if (n == 0)
			...
			keyframes.resize(n);
			for (int i = 0; i < n; i++)
				keyframes[i] = param->getKeyframe(i);
		}
	}
	...
	void restoreKeyframes(TDoubleParam *param, const std::vector<TDoubleKeyframe> &keyframes) const
	{
...
void TStageObjectCmd::rename(const TStageObjectId &id, std::string name, TXsheetHandle *xshHandle)
{
	...
	if (oldName == name)
		return;
	pegbar->setName(name);
	TUndoManager::manager()->add(new StageObjectRenameUndo(id, xshHandle, oldName, name));
}
...
void TStageObjectCmd::resetOffset(const TStageObjectId &id, TXsheetHandle *xshHandle)
{
...
void TStageObjectCmd::resetCenterAndOffset(const TStageObjectId &id, TXsheetHandle *xshHandle)
{
	TStageObject *peg = xshHandle->getXsheet()->getStageObject(id);
	if (!peg)
		return;
	...
	TUndoManager::manager()->add(new ResetCenterAndOffsetUndo(id, xshHandle, oldOffset));
	xshHandle->notifyXsheetChanged();
}
...
void TStageObjectCmd::resetPosition(const TStageObjectId &id, TXsheetHandle *xshHandle)
{
	...
	TXsheetHandle *xshHandle, bool doUndo)
{
	...
	if (doUndo) {
		...
		TUndoManager::manager()->add(
			new SetParentUndo(id, xshHandle, oldParentId, oldParentHandle, parentId, parentHandle));
	}
}
...
void TStageObjectCmd::addNewCamera(TXsheetHandle *xshHandle, TObjectHandle *objHandle, QPointF initialPos)
{
	...
	*newCameraPegbar->getCamera() = *currentCamera;
	...
	TUndoManager::manager()->add(new NewCameraUndo(cameraId, xshHandle, objHandle));
	xshHandle->notifyXsheetChanged();
}
...
void TStageObjectCmd::setAsActiveCamera(TXsheetHandle *xshHandle, TObjectHandle *objHandle)
{
	TXsheet *xsh = xshHandle->getXsheet();
...
void TStageObjectCmd::addNewSpline(TXsheetHandle *xshHandle, TObjectHandle *objHandle, TColumnHandle *colHandle, QPointF initialPos)
{
	...
	if (objId == TStageObjectId::NoneId) {
		...
		if (col >= 0)
			objId = TStageObjectId::ColumnId(col);
	}
	if (objId != TStageObjectId::NoneId) {
									  ...
									  bool doUndo)
{
	...
	for (it2 = objIds.begin(); it2 != objIds.end(); it2++) {
		if (it2->isColumn())
			columnIndexes.append(it2->getIndex());
		if (it2->isPegbar())
			pegbarIndexes.append(it2->getIndex());
		if (it2->isCamera())
			cameraIndexes.append(it2->getIndex());
	}
...
void TStageObjectCmd::group(const QList<TStageObjectId> ids, TXsheetHandle *xshHandle)
{
	...
	TUndoManager::manager()->add(new UndoGroup(ids, groupId, positions, xshHandle));
}
...
void TStageObjectCmd::renameGroup(const QList<TStageObject *> objs, const std::wstring &name, bool fromEditor, TXsheetHandle *xshHandle)
{
	...
	for (i = 0; i < objs.size(); i++) {
		if (i == 0)
			oldName = objs[i]->getGroupName(fromEditor);
		int position = objs[i]->removeGroupName(fromEditor);
		objs[i]->setGroupName(name, position);
		positions.push_back(position);
	}
	TUndoManager::manager()->add(new UndoRenameGroup(objs, positions, name, oldName, xshHandle));
}
...
void TStageObjectCmd::duplicateObject(const QList<TStageObjectId> ids, TXsheetHandle *xshHandle)
{
	TXsheet *xsh = xshHandle->getXsheet();
	TStageObjectTree *objTree = xsh->getStageObjectTree();
	...
	for (i = 0; i < objCount; i++) {
		...
		if (id.isPegbar() || id.isCamera()) {
			...
			delete params;
			if (id.isCamera())
				*(duplicatedObj->getCamera()) = *(obj->getCamera());
		}
	}
	xshHandle->notifyXsheetChanged();
}
...
void TStageObjectCmd::enableSplineAim(TStageObject *obj, int state, TXsheetHandle *xshHandle)
{
...
void TStageObjectCmd::enableSplineUppk(TStageObject *obj, bool toggled, TXsheetHandle *xshHandle)
{
	UndoStatusChange *undo = new UndoStatusChange(obj, xshHandle);
