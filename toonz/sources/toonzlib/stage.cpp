...
void updateOnionSkinSize(const PlayerSet &players)
{
	...
	for (i = 0; i < (int)players.size(); i++) {
		...
		if (player.m_onionSkinDistance == c_noOnionSkin)
			continue;
		if (player.m_onionSkinDistance > 0 && maxOnionSkinFrontValue < player.m_onionSkinDistance)
			maxOnionSkinFrontValue = player.m_onionSkinDistance;
		if (player.m_onionSkinDistance < 0 && maxOnionSkinBackValue > player.m_onionSkinDistance)
...
public:
	...
	\n	Analyze cell in row \b row and column \b col of xsheet \b xsh. If level
			containing this cell is a simple level, \b TXshSimpleLevel, create a Pleyer
			object with informations of the cell. Else if level is a child level,
			\b TXshChildLevel, recall \b addFrame().
	*/
	void addCell(PlayerSet &players, ToonzScene *scene, TXsheet *xsh, int row, int col, int level);

	/*! Verify if onion-skin is active and recall \b addCell().
	\n	Compute the distance between each cell with active onion-skin and current
...
StageBuilder::~StageBuilder()
{
	clearPointerContainer(m_maskPool);
}
	...
	TXsheet *xsh, int row, int col, int level)
{
	...
	struct locals {
		...
		static inline bool isMeshDeformed(TXsheet *xsh, TStageObject *obj, const ImagePainter::VisualSettings *vs)
		{
			...
			if (parentId.isColumn() && obj->getParentHandle()[0] != 'H') {
				...
				if (!parentObj->getPlasticSkeletonDeformation())
					return false;
	...
	if (storePlayer) {
		...
		if (m_subXSheetStack.empty()) {
			...
			player.m_placement = (m_camera3d ? parent.m_aff : parent.m_zaff) * columnZaff;
		}
		...
		TStageObjectId childCameraId = childXsheet->getStageObjectTree()->getCurrentCameraId();
		...
		if (!m_subXSheetStack.empty()) {
			...
			subXSheet.m_z = m_subXSheetStack.front().m_z; // Top-level xsheet cell's z
...
bool alreadyAdded(TXsheet *xsh, int row, int index, const vector<int> &rows, int col)
{
	...
	for (i = 0; i < index; i++)
		if (xsh->getCell(rows[i], col) == xsh->getCell(rows[index], col))
			return true;
	if (xsh->getCell(rows[index], col) == xsh->getCell(row, col))
		return true;
	return false;
}
	...
	TXsheet *xsh, int row, int col, int level)
{
	struct locals {
		...
		static inline bool doStandardOnionSkin(StageBuilder *sb, TXsheet *xsh, int level, int col)
		{
	...
	if (m_onionSkinMask.isShiftTraceEnabled() && col == m_currentColumnIndex) {
		...
		if (cell.getSimpleLevel() != 0) {
			...
			if (cell.getSimpleLevel() == 0 || otherCell.getSimpleLevel() == cell.getSimpleLevel()) {
	...
	for (int i = 0; i < columnCount; i++) {
		...
		if (column && !column->isEmpty()) {
			if (!column->isPreviewVisible() && checkPreviewVisibility)
				continue;
			if (column->isCamstandVisible() || includeUnvisible) // se l'"occhietto" non e' chiuso
			{
		...
		if (!isMask) {
			while (m_masks.size() > maskCount)
				m_masks.pop_back();
		}
	}
	if (level == 0)
		std::stable_sort(players.begin(), players.end(), PlayerLt());
}
