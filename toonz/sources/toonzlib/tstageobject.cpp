...
{
	typedef tcg::function<void (TStageObject::*)(LazyData &) const, &TStageObject::update> Func;
...
{
	return const_cast<LazyData &>(static_cast<const TStageObject &>(*this).lazyData());
}
...
void TStageObject::setCenterAndOffset(const TPointD &center, const TPointD &offset)
{
	m_center = center;
	m_offset = offset;
	invalidate();
...
TStageObject *TStageObject::getPinnedDescendant(int frame)
{
	if (getPinnedRangeSet()->isPinned(frame))
		return this;
	for (list<TStageObject *>::iterator it = m_children.begin(); it != m_children.end(); ++it) {
...
void TStageObject::updateKeyframes(LazyData &ld) const
{
	...
	for (ft = frames.begin(); ft != fEnd; ++ft) {
		...
		const std::map<QString, SkVD::Keyframe> &vdfs = stageKf.m_skeletonKeyframe.m_vertexKeyframes;
...
QStack<int> TStageObject::getGroupIdStack()
{
	return m_groupId;
}
...
QStack<wstring> TStageObject::getGroupNameStack()
{
	return m_groupName;
}
