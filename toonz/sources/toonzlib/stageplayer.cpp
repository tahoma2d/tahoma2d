...
TImageP Stage::Player::image() const
{
	...
	if (slType == PLI_XSHLEVEL && TXshSimpleLevel::m_rasterizePli) {
		if (!(m_isCurrentColumn && m_isCurrentXsheetLevel))
			id = id + "_rasterized";
	}
...
DrawableTextureDataP Stage::Player::texture() const
{
	...
	if (!cl)
		return DrawableTextureDataP(); // Should never happen, though

	// Fetch the xsheet data
	TXsheet *xsh = cl->getXsheet();
	int frame = cell.getFrameId().getNumber() - 1; // frame 1 internally stands for 0
