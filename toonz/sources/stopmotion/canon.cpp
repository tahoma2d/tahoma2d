...
EdsError Canon::initializeCanonSDK() {
    ...
    if (!m_error)
        m_error = EdsSetCameraAddedHandler(Canon::handleCameraAddedEvent,
            (EdsVoid*)this);
    return m_error;
}
...
void Canon::cameraAdded() {
    StopMotion::instance()->refreshCameraList();
}
...
bool Canon::downloadEVFData() {
    ...
    if (err == EDS_ERR_OK) {
        ...
        StopMotion::instance()->m_fullImageDimensions = TDimension(coordSys.width, coordSys.height);
...
void Canon::onImageReady(const bool& status) {
