...
static int
_tiffDummyCloseProc(thandle_t fd)
{
	return 0;
}
...
/*
 * Open a TIFF file descriptor for read/writing - but without close proc.
 */
TIFF*
TIFFFdOpenNoCloseProc(int fd, const char* name, const char* mode)
{
	TIFF* tif;
	...
	tif = TIFFClientOpen(name, mode,
	    (thandle_t) fd,
	    _tiffReadProc, _tiffWriteProc,
	    _tiffSeekProc, _tiffDummyCloseProc, _tiffSizeProc,
	    _tiffMapProc, _tiffUnmapProc);
	if (tif)
		tif->tif_fd = fd;
	return (tif);
