# Ztoryc — Changelog Archive (pre 2026-03-21)

> Voci archiviate per tenere CHANGELOG.md leggero.
> Queste sessioni sono storiche — il codice corrispondente è nel repo git.
...
### Added (planning — implementato nelle sessioni successive)
...
- **Task 12b diagnostica** — Soluzione: esporre `soundTrack()` da
...
### Added (planning — implementato il 2026-03-21)
...
---
## [2026-03-19] — FlipConsole play fix + audio animatic + BUG-05

### Fixed
- `ZtoryAnimaticViewer::onDrawFrame()` — scrive frame su `ctrl->frameHandle()` non su `TApp`
- `updateAnimaticFrameMarkers()` — quando `ancestorCount > 0` usa range pieno
- `refreshAnimaticSound()` — chiama `ctrl->mainXsheet()->makeSound()`
...
- Fix architetturale costruttore `TLevelWriterTzl`: reset a modalità nuovo-file se `frameCount==0`
...
---
## [Unreleased] + [0.1.0-backup] — 2026-03-14

### Fix contribuibili upstream Tahoma2D
- Per-xsheet In/Out markers (`subscenecommand.cpp`) ← PR candidate
- Save Sub-Scene As path corruption (`toonzscene.cpp`, `iocommand.cpp`) ← PR candidate
- Switch room programmatico (`mainwindow.h/.cpp`)
- Mesh sottoscene cartella sbagliata (`meshifypopup.cpp`) ← PR candidate
- New Scene senza save dialog (`iocommand.cpp`) ← PR candidate
- BaseViewerPanel::getPreviewButtonStates crash (`viewerpane.cpp`) ← PR candidate
